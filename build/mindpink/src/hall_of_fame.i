...
typedef char HallOfFameFreeSpace[(sizeof(struct HallofFameTeam) * 50 <= 3968 * 2) ? 1 : -1];;
...
static __attribute__((section("ewram_data"))) u32 sSelectedPaletteIndices = 0;
...
static void Task_Hof_InitMonData(u8 taskId)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        {
            sHofMonPtr[0].mon[i].species = GetMonData(&gPlayerParty[i], 65);
            sHofMonPtr[0].mon[i].tid = GetMonData(&gPlayerParty[i], 1);
            sHofMonPtr[0].mon[i].personality = GetMonData(&gPlayerParty[i], 0);
            sHofMonPtr[0].mon[i].lvl = GetMonData(&gPlayerParty[i], 56);
...
static void Task_Hof_InitTeamSaveData(u8 taskId)
{
    ...
    {
        if (lastSavedTeam[0].mon[0].species == 0)
            break;
    }
    if (i >= 50)
    {
...
static void Task_Hof_TryDisplayAnotherMon(u8 taskId)
{
    ...
    else
    {
        ...
        if (gTasks[taskId].data[1] < 6 - 1 && currMon[1].species != 0)
        {
            ...
            BeginNormalPaletteFade(sSelectedPaletteIndices, 0, 12, 12, ((22) | ((24) << 5) | ((29) << 10)));
...
static void Task_Hof_WaitBorderFadeAway(u8 taskId)
{
    ...
    {
        gTasks[taskId].data[3]++;
        SetGpuReg(0x52, 256 * gTasks[taskId].data[3]);
    }
}
...
static void Task_Hof_SpawnPlayerPic(u8 taskId)
{
    ...
    ShowBg(1);
    ShowBg(3);
    gTasks[taskId].data[4] = CreateTrainerPicSprite(PlayerGenderToFrontTrainerPicId(gSaveBlock2Ptr->playerGender, 1), 1, 0x78, 0x48, 6, 0xFFFF);
    AddWindow(&sWindowTemplate);
    LoadStdWindowGfx(1, 0x21D, (0x000 + ((13) * 16)));
    gTasks[taskId].data[3] = 120;
...
static void Task_Hof_WaitAndPrintPlayerInfo(u8 taskId)
{
    ...
    else
    {
        ...
        DrawDialogueFrame(0, 0);
        AddTextPrinterParameterized2(0, FONT_NORMAL, gText_LeagueChamp, 0, ((void *)0), 0x2, 0x1, 0x3);
        CopyWindowToVram(0, COPYWIN_FULL);
...
static void Task_Hof_HandlePaletteOnExit(u8 taskId)
{
    CpuSet(gPlttBufferFaded, gPlttBufferUnfaded, 0x00000000 | (((0x200 + 0x200))/(16/8) & 0x1FFFFF));
...
static void SetWarpsToRollCredits(void)
{
    ...
    SetWarpDestination(((9 | (3 << 8)) >> 8), ((9 | (3 << 8)) & 0xFF), -1, 11, 6);
    DoWarp();
...
static void Task_HofPC_CopySaveData(u8 taskId)
{
    ...
    {
        CpuSet(gDecompressionBuffer, sHofMonPtr, 0x00000000 | (((3968 + 128) * 2)/(16/8) & 0x1FFFFF));
...
static void Task_HofPC_DrawSpritesPrintText(u8 taskId)
{
    ...
    BlendPalettes(0xFFFF0000, 0xC, ((22) | ((24) << 5) | ((29) << 10)));

    ConvertIntToDecimalStringN(gStringVar1, gTasks[taskId].data[1], STR_CONV_MODE_LEFT_ALIGN, 3);
    StringExpandPlaceholders(gStringVar4, gText_HOFNumber);

    if (gTasks[taskId].data[0] <= 0)
        TopBarWindowPrintTwoStrings(gStringVar4, gText_UPDOWNPick_ABUTTONBBUTTONCancel, 0, 0, 1);
    else
        TopBarWindowPrintTwoStrings(gStringVar4, gText_UPDOWNPick_ABUTTONNext_BBUTTONBack, 0, 0, 1);
...
static void Task_HofPC_PrintMonInfo(u8 taskId)
{
    ...
    BlendPalettesUnfaded(sSelectedPaletteIndices, 0xC, ((22) | ((24) << 5) | ((29) << 10)));
...
static void Task_HofPC_PrintDataIsCorrupted(u8 taskId)
{
    TopBarWindowPrintString(gText_ABUTTONExit, 8, 1);
    DrawDialogueFrame(0, 0);
    AddTextPrinterParameterized2(0, FONT_NORMAL, gText_HOFCorrupted, 0, ((void *)0), 0x2, 0x1, 0x3);
    CopyWindowToVram(0, COPYWIN_FULL);
...
static void HallOfFame_PrintMonInfo(struct HallofFameMon* currMon, u8 unused1, u8 unused2)
{
    ...
    if (currMon->species != 479)
    {
        ...
        if (dexNumber != 0xFFFF)
        {
            text[0] = (dexNumber / 100) + 0xA1;
            text[1] = ((dexNumber %= 100) / 10) + 0xA1;
            text[2] = (dexNumber % 10) + 0xA1;
        }
        else
        {
            text[0] = text[1] = text[2] = 0xAC;
        }
        text[3] = 0xFF;
    ...
    i = 0;
    if (currMon->nick[0] != 0xFF)
    {
        for (i = 0; i < 10 && currMon->nick[i] != 0xFF; i++)
        {
...
static void HallOfFame_PrintPlayerInfo(u8 unused1, u8 unused2)
{
    ...
    PutWindowTilemap(1);
    DrawStdFrameWithCustomTileAndPalette(1, 0, 0x21D, 13);
...
static void ClearVramOamPltt_LoadHofPal(void)
{
    ...
    while (1)
    {
        { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(vramOffset); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((0x1000)/(16/8))); dmaRegs[2]; }; };
        vramOffset += 0x1000;
        vramSize -= 0x1000;
        if (vramSize <= 0x1000)
        {
            { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(vramOffset); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((vramSize)/(16/8))); dmaRegs[2]; }; };
            break;
        }
    }
    ...
    oamOffset = 0x7000000;
    oamSize = 0x400;
    { vu32 tmp = (vu32)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(oamOffset); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((oamSize)/(32/8))); dmaRegs[2]; }; };

    plttOffset = 0x5000000;
    plttSize = (0x200 + 0x200);
    { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(plttOffset); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((plttSize)/(16/8))); dmaRegs[2]; }; };

    ResetPaletteFade();
    LoadPalette(sHallOfFame_Pal, (0x000 + ((0) * 16)), sizeof(sHallOfFame_Pal));
}
...
{
    ResetBgsAndClearDma3BusyFlags(0);
    InitBgsFromTemplates(0, sHof_BgTemplates, (sizeof(sHof_BgTemplates) / sizeof((sHof_BgTemplates)[0])));
