# 0 "src/trainer_fan_club.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/trainer_fan_club.c"
...
{
    TryLoseFansFromPlayTimeAfterLinkBattle(((struct TrainerFanClub *)GetVarPointer(0x4038)));
}
...
static void TryLoseFansFromPlayTimeAfterLinkBattle(struct TrainerFanClub *fanClub)
{
    if (DidPlayerGetFirstFans(fanClub))
    {
        ...
        VarSet(0x4039, gSaveBlock2Ptr->playTimeHours);
    }
}

void Script_UpdateTrainerFanClubGameClear(void)
{
    UpdateTrainerFanClubGameClear(((struct TrainerFanClub *)GetVarPointer(0x4038)));
}
...
__attribute__((aligned(4))) const u8 sCounterIncrements[] = {2, 1, 2, 1};
...
static u8 PlayerGainRandomTrainerFan(struct TrainerFanClub *fanClub)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if (!((fanClub->fanFlags >> (sFanClubMemberIds[i]) & 1)))
        {
            idx = i;
            if (Random() % 2)
            {
                (fanClub->fanFlags |= 1 << (sFanClubMemberIds[i]));
                return sFanClubMemberIds[i];
            }
        }
    }

    (fanClub->fanFlags |= 1 << (sFanClubMemberIds[idx]));
...
static u8 PlayerLoseRandomTrainerFan(struct TrainerFanClub *fanClub)
{
    ...
    if ((fanClub->fanFlags >> (sFanClubMemberIds[idx]) & 1))
        (fanClub->fanFlags ^= 1 << (sFanClubMemberIds[idx]));
...
{
    return GetNumFansOfPlayerInTrainerFanClub(((struct TrainerFanClub *)GetVarPointer(0x4038)));
}
...
{
    TryLoseFansFromPlayTime(((struct TrainerFanClub *)GetVarPointer(0x4038)));
}
...
{
    return IsFanClubMemberFanOfPlayer(((struct TrainerFanClub *)GetVarPointer(0x4038)));
}

static bool16 IsFanClubMemberFanOfPlayer(struct TrainerFanClub *fanClub)
{
    return (fanClub->fanFlags >> (gSpecialVar_0x8004) & 1);
}

static void SetInitialFansOfPlayer(struct TrainerFanClub *fanClub)
{
    (fanClub->fanFlags |= 1 << (0));
    (fanClub->fanFlags |= 1 << (1));
    (fanClub->fanFlags |= 1 << (2));
}
...
static void BufferFanClubTrainerName(struct LinkBattleRecords *linkRecords, u8 whichLinkTrainer, u8 whichNPCTrainer)
{
    ...
    {
        str = gStringVar1;
        StringCopyN(str, linkTrainerName, 7);
        str[7] = 0xFF;
        if ( str[0] == 0xFC
               && str[1] == 0x15)
        {
...
{
    UpdateTrainerFansAfterLinkBattle(((struct TrainerFanClub *)GetVarPointer(0x4038)));
}
...
{
    SetPlayerGotFirstFans(((struct TrainerFanClub *)GetVarPointer(0x4038)));
}

static void SetPlayerGotFirstFans(struct TrainerFanClub *fanClub)
{
    fanClub->gotInitialFans = 1;
}

u8 Script_TryGainNewFanFromCounter(void)
{
    return TryGainNewFanFromCounter(((struct TrainerFanClub *)GetVarPointer(0x4038)), gSpecialVar_0x8004);
}
