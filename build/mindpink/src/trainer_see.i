...
extern const struct SpritePalette gObjectEventPal_NpcBlue;
...
static bool8 CheckTrainer(u8 trainerObjId)
{
    ...
    {
        if (script[1] == 4 && GetMonsStateToDoubles())
            return 0;
...
static u8 GetTrainerApproachDistanceSouth(struct ObjectEvent *trainerObj, s16 range, s16 x, s16 y)
{
    ...
    {
        if (range > 3 && GetFirstInactiveObjectEventId() == 16)
            return 0;
...
# 250 "src/trainer_see.c"
static void TrainerApproachPlayer(struct ObjectEvent * trainerObj, u8 approachDistance)
{
    u8 taskId = CreateTask(Task_RunTrainerSeeFuncList, 80);
    struct Task *task = &gTasks[taskId];
    task->data[1] = ((uintptr_t)trainerObj) >> 16;
    task->data[2] = (uintptr_t)trainerObj;
    task->data[3] = approachDistance;
}
...
static void Task_RunTrainerSeeFuncList(u8 taskId)
{
    ...
    do { (trainerObj) = (struct ObjectEvent *)(((task)->data[1] << 16) | ((u16)(task)->data[2])); } while (0);
...
static bool8 TrainerSeeFunc_StartExclMark(u8 taskId, struct Task *task, struct ObjectEvent * trainerObj)
{
    ...
    if (trainerObj->facingDirection == 1 && task->data[3] > 2)
    {
        task->data[0] = 12;
    }
    else
...
static bool8 TrainerSeeFunc_TrainerApproach(u8 taskId, struct Task *task, struct ObjectEvent * trainerObj)
{
    if (!ObjectEventIsMovementOverridden(trainerObj) || ObjectEventClearHeldMovementIfFinished(trainerObj))
    {
        ...
        {
            ObjectEventSetHeldMovement(trainerObj, 0x4A);
            task->data[0]++;
        }
    }
    return 0;
...
static bool8 TrainerSeeFunc_BeginRemoveDisguise(u8 taskId, struct Task *task, struct ObjectEvent * trainerObj)
{
    ...
    {
        ObjectEventSetHeldMovement(trainerObj, 0x67);
        task->data[0]++;
    }
    return 0;
}
...
static bool8 TrainerSeeFunc_OffscreenAboveTrainerCreateCameraObj(u8 taskId, struct Task *task, struct ObjectEvent *trainerObj)
{
    int specialObjectId;
    task->data[5] = 0;
    specialObjectId = SpawnSpecialObjectEventParameterized(18, 7, 0x7F, gSaveBlock1Ptr->pos.x + 7, gSaveBlock1Ptr->pos.y + 7, 3);
    gObjectEvents[specialObjectId].invisible = 1;
...
{
    u8 specialObjectId;
    TryGetObjectEventIdByLocalIdAndMap(0x7F, gSaveBlock1Ptr->location.mapNum, gSaveBlock1Ptr->location.mapGroup, &specialObjectId);
    ...
    if (task->data[5] != task->data[3] - 1)
    {
        ObjectEventSetHeldMovement(&gObjectEvents[specialObjectId], GetWalkFastMovementAction(2));
        task->data[5]++;
    }
    else
    {
...
static bool8 TrainerSeeFunc_OffscreenAboveTrainerCameraObjMoveDown(u8 taskId, struct Task *task, struct ObjectEvent * trainerObj)
{
    ...
    if (task->data[5] != task->data[3] - 1)
    {
        ObjectEventSetHeldMovement(&gObjectEvents[specialObjectId], GetWalkFastMovementAction(1));
        task->data[5]++;
    }
    else
    {
...
u8 FldEff_DoubleExclMarkIcon(void)
{
    ...
    spriteId = CreateSpriteAtEnd(&sSpriteTemplate_Emoticons, 0, 0, 0x52);
    if (spriteId != 64)
        SetIconSpriteData(&gSprites[spriteId], 66, 1);
...
u8 FldEff_XIcon(void)
{
    ...
    spriteId = CreateSpriteAtEnd(&sSpriteTemplate_Emoticons, 0, 0, 0x52);
    if (spriteId != 64)
        SetIconSpriteData(&gSprites[spriteId], 46, 2);
...
u8 FldEff_SmileyFaceIcon(void)
{
    ...
    spriteId = CreateSpriteAtEnd(&sSpriteTemplate_Emoticons, 0, 0, 0x52);
    if (spriteId != 64)
        SetIconSpriteData(&gSprites[spriteId], 64, 3);
...
u8 FldEff_QuestionMarkIcon(void)
{
    ...
    spriteId = CreateSpriteAtEnd(&sSpriteTemplate_Emoticons, 0, 0, 0x52);
    if (spriteId != 64)
        SetIconSpriteData(&gSprites[spriteId], 33, 4);
