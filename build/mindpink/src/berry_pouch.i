...
# 1 "include/text.h" 1
# 6 "include/new_menu_helpers.h" 2
...
# 30 "src/berry_pouch.c" 2
# 1 "include/constants/items.h" 1
# 31 "src/berry_pouch.c" 2
# 1 "include/constants/songs.h" 1
# 32 "src/berry_pouch.c" 2
...
struct BerryPouchStruct_203F36C
{
    ...
    u8 itemMenuIconId;
    u8 __attribute__((aligned(4))) bg1TilemapBuffer[0x800];
    s16 data[4];
};
...
static __attribute__((section("ewram_data"))) struct BerryPouchStruct_203F36C *sResources = ((void *)0);
static __attribute__((section("ewram_data"))) struct BerryPouchStruct_203F370 sStaticCnt = {};
static __attribute__((section("ewram_data"))) struct ListMenuItem *sListMenuItems = ((void *)0);
static __attribute__((section("ewram_data"))) u8 * sListMenuStrbuf = ((void *)0);
static __attribute__((section("ewram_data"))) const u8 * sContextMenuOptions = ((void *)0);
static __attribute__((section("ewram_data"))) u8 sContextMenuNumOptions = 0;
static __attribute__((aligned(4))) __attribute__((section("ewram_data"))) u8 sVariableWindowIds[14] = {};
static __attribute__((aligned(4))) __attribute__((section("ewram_data"))) u8 sBerryPouchSpriteId = 0;
...
static const struct SpriteTemplate sSpriteTemplate_BerryPouch = {
    100, 100, &sOamData, sSpriteAnimTable, ((void *)0), sSpriteAffineAnimTable, SpriteCallbackDummy
};
...
void InitBerryPouch(u8 type, void (*savedCallback)(void), u8 allowSelect)
{
    ...
    else
    {
        ...
        if (savedCallback != ((void *)0))
            sStaticCnt.savedCallback = savedCallback;
        sResources->exitCallback = ((void *)0);
...
static void BerryPouchInitBgs(void)
{
    ...
    memset(sResources->bg1TilemapBuffer, 0, 0x800);
...
static bool8 BerryPouchLoadGfx(void)
{
    ...
    case 1:
        ...
        break;
    case 2:
        LoadCompressedPalette(gBerryPouchBgPals, (0x000 + ((0) * 16)), 3 * ((16) * sizeof(u16)));
        if (gSaveBlock2Ptr->playerGender != 0)
            LoadCompressedPalette(gBerryPouchBgPal0FemaleOverride, (0x000 + ((0) * 16)), ((16) * sizeof(u16)));
        sResources->data[0]++;
        break;
    case 3:
...
{
    u16 i;
    struct BagPocket *pocket = &gBagPockets[5 - 1];
...
static void GetBerryNameAndIndexForMenu(u8 * dest, u16 itemId)
{
    ...
    ConvertIntToDecimalStringN(gStringVar1, itemId - 134 + 1, STR_CONV_MODE_LEADING_ZEROS, 2);
...
static void BerryPouchMoveCursorFunc(s32 itemIndex, bool8 onInit, struct ListMenu *list)
{
    ...
    if (sResources->listMenuNumItems != itemIndex)
        CreateBerryPouchItemIcon(BagGetItemIdByPocketPosition(5, itemIndex), sResources->itemMenuIconId);
    else
        CreateBerryPouchItemIcon(376, sResources->itemMenuIconId);
...
static void BerryPouchItemPrintFunc(u8 windowId, u32 itemId, u8 y)
{
    ...
    {
        unused = BagGetItemIdByPocketPosition(5, itemId);
        itemQuantity = BagGetQuantityByPocketPosition(5, itemId);
...
static void PrintSelectedBerryDescription(s32 itemIdx)
{
    ...
    if (itemIdx != sResources->listMenuNumItems)
        str = ItemId_GetDescription(BagGetItemIdByPocketPosition(5, itemIdx));
    else
...
static void Task_BerryPouchFadeToExitCallback(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        if (sResources->exitCallback != ((void *)0))
...
static void Task_BerryPouchMain(u8 taskId)
{
    s16 * data = gTasks[taskId].data;
    s32 menuInput;
    if (!gPaletteFade.active && IsActiveOverworldLinkBusy() != 1)
    {
        ...
        if (({(gMain.newKeys) & (0x0004);}) && sStaticCnt.allowSelect == 1)
        {
            PlaySE(5);
            gSpecialVar_ItemId = 0;
        ...
        else
        {
            ...
            default:
                ...
                if (sStaticCnt.type == BERRYPOUCH_FROMBERRYCRUSH)
                {
                    gSpecialVar_ItemId = BagGetItemIdByPocketPosition(5, menuInput);
                    BerryPouch_StartFadeToExitCallback(taskId);
                }
                ...
                else
                {
                    ...
                    data[1] = menuInput;
                    data[2] = BagGetQuantityByPocketPosition(5, menuInput);
                    gSpecialVar_ItemId = BagGetItemIdByPocketPosition(5, menuInput);
...
static void CreateNormalContextMenu(u8 taskId)
{
    ...
    else if (MenuHelpers_IsLinkActive() == 1 || InUnionRoom() == 1)
    {
...
{
    s16 * data = gTasks[taskId].data;
    u16 itemId = BagGetItemIdByPocketPosition(5, data[1]);
...
{
    s16 * data = gTasks[taskId].data;
    ConvertIntToDecimalStringN(gStringVar3, ItemId_GetPrice(BagGetItemIdByPocketPosition(5, data[1])) / 2 * data[8], STR_CONV_MODE_LEFT_ALIGN, 6);
...
static void Task_Sell_PrintSelectMultipleUI(u8 taskId)
{
    ...
    SellMultiple_UpdateSellPriceDisplay(ItemId_GetPrice(BagGetItemIdByPocketPosition(5, data[1])) / 2 * data[8]);
    PrintMoneyInWin2();
...
static void BerryPouchInitWindows(void)
{
    ...
    LoadUserWindowGfx(0, 0x001, (0x000 + ((14) * 16)));
    LoadMenuMessageWindowGfx(0, 0x013, (0x000 + ((13) * 16)));
    LoadStdWindowGfx(0, 0x00A, (0x000 + ((12) * 16)));
...
static u8 GetOrCreateVariableWindow(u8 winIdx)
{
    ...
    if (retval == 0xFF)
    {
        ...
        if (winIdx == 2 || winIdx == 6 || winIdx == 7 || winIdx == 8 || winIdx == 9)
            DrawStdFrameWithCustomTileAndPalette(sVariableWindowIds[winIdx], 0, 0x00A, 12);
        else
            DrawStdFrameWithCustomTileAndPalette(sVariableWindowIds[winIdx], 0, 0x001, 14);
        ScheduleBgCopyTilemapToVram(2);
...
static void DestroyVariableWindow(u8 winIdx)
{
    ClearStdWindowAndFrameToTransparent(sVariableWindowIds[winIdx], 0);
...
static void TryDestroyVariableWindow(u8 winIdx)
{
    ...
    {
        ClearDialogWindowAndFrameToTransparent(sVariableWindowIds[winIdx], 0);
        ClearWindowTilemap(sVariableWindowIds[winIdx]);
        RemoveWindow(sVariableWindowIds[winIdx]);
        PutWindowTilemap(1);
        ScheduleBgCopyTilemapToVram(0);
