# 0 "src/help_message.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/help_message.c"
...
# 4 "src/help_message.c" 2


static __attribute__((section("ewram_data"))) u8 sHelpMessageWindowId = 0;
...
void DestroyHelpMessageWindow(u8 a0)
{
    if (sHelpMessageWindowId != 0xFF)
    {
        FillWindowPixelBuffer(sHelpMessageWindowId, ((0) | ((0) << 4)));
        ...
        for (i = 0; i < height; i++)
        {
            for (j = 0; j < width; j++)
            {
                if (i == 0)
                    ...
                    tileId = 5;
                CpuSet(&ptr[tileId * 32], &buffer[(i * width + j) * 32], 0x04000000 | ((32)/(32/8) & 0x1FFFFF))
...
static const u8 sHelpMessageTextColors[3] = {0x0, 0xA, 0x2};
