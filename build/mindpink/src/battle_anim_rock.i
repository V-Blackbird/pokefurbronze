...
static const union AffineAnimCmd sAffineAnim_Whirlpool[] =
{
    ...
    {.frame = {.xScale = 0x2, .yScale = -0x3, .rotation = 0, .duration = 5}},
    {.frame = {.xScale = -0x2, .yScale = 0x3, .rotation = 0, .duration = 5}},
...
{
    { (sizeof(sFlyingSandSubsprites) / sizeof((sFlyingSandSubsprites)[0])), sFlyingSandSubsprites },
};
...
void AnimTask_LoadSandstormBackground(u8 taskId)
{
    ...
    LoadCompressedPalette(gBattleAnimSpritePal_FlyingDirt, (0x000 + ((animBg.paletteId) * 16)), ((16) * sizeof(u16)));
    if (IsContest())
...
static void AnimTask_Rollout_Step(u8 taskId)
{
    ...
    case 0:
        ...
        PlaySE12WithPanning(155, task->data[13]);
        break;
    case 1:
        if (--task->data[11] == 0)
            ++task->data[0];
    ...
    case 3:
        ...
        if (++task->data[9] >= task->data[10])
        {
            ...
            PlaySE12WithPanning(168, task->data[13]);
        }
        if (--task->data[8] == 0)
        {
            ++task->data[0];
