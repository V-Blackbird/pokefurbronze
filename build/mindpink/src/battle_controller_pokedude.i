# 0 "src/battle_controller_pokedude.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/battle_controller_pokedude.c"
...
# 2 "src/battle_controller_pokedude.c" 2
# 1 "include/gflib.h" 1
...
# 3 "src/battle_controller_pokedude.c" 2
# 1 "include/task.h" 1
# 13 "include/task.h"
...
# 4 "src/battle_controller_pokedude.c" 2
...
# 5 "src/battle_controller_pokedude.c" 2
...
# 6 "src/battle_controller_pokedude.c" 2
# 1 "include/data.h" 1
# 9 "include/data.h"
struct MonCoords
{
...
# 7 "src/battle_controller_pokedude.c" 2
# 1 "include/util.h" 1
...
# 8 "src/battle_controller_pokedude.c" 2
# 1 "include/m4a.h" 1
...
# 9 "src/battle_controller_pokedude.c" 2
# 1 "include/link.h" 1
# 110 "include/link.h"
enum {
    BLOCK_REQ_SIZE_NONE,
...
# 10 "src/battle_controller_pokedude.c" 2
...
void ClearTempFieldEventData(void);
# 11 "src/battle_controller_pokedude.c" 2
...
# 12 "src/battle_controller_pokedude.c" 2
...
extern const u8 gText_PokeSum_EggOrigin_Trade[];
# 13 "src/battle_controller_pokedude.c" 2
...
# 14 "src/battle_controller_pokedude.c" 2
...
# 15 "src/battle_controller_pokedude.c" 2
...
# 16 "src/battle_controller_pokedude.c" 2
...
# 17 "src/battle_controller_pokedude.c" 2
...
# 18 "src/battle_controller_pokedude.c" 2
...
# 19 "src/battle_controller_pokedude.c" 2
...
# 20 "src/battle_controller_pokedude.c" 2
# 1 "include/constants/songs.h" 1
# 21 "src/battle_controller_pokedude.c" 2
# 1 "include/constants/moves.h" 1
# 22 "src/battle_controller_pokedude.c" 2
# 1 "include/constants/pokemon.h" 1
# 23 "src/battle_controller_pokedude.c" 2
...
static void PokedudeBufferRunCommand(void)
{
    ...
    {
        if (gBattleBufferA[gActiveBattler][0] < (sizeof(sPokedudeBufferCommands) / sizeof((sPokedudeBufferCommands)[0])))
        {
...
static void Task_LaunchLvlUpAnim(u8 taskId)
{
    ...
    if (IsDoubleBattle() == 1 && monIndex == gBattlerPartyIndexes[battlerId ^ 2])
        battlerId ^= 2;
    InitAndLaunchSpecialAnimation(battlerId, battlerId, battlerId, 0);
...
static u32 CopyPokedudeMonData(u8 monId, u8 *dst)
{
    ...
    case REQUEST_ALL_BATTLE:
        battleMon.species = GetMonData(mon, 11);
        battleMon.item = GetMonData(mon, 12);
        for (size = 0; size < 4; ++size)
        {
            battleMon.moves[size] = GetMonData(mon, 13 + size);
            battleMon.pp[size] = GetMonData(mon, 17 + size);
        }
        battleMon.ppBonuses = GetMonData(mon, 21);
        battleMon.friendship = GetMonData(mon, 32);
        battleMon.experience = GetMonData(mon, 25);
        battleMon.hpIV = GetMonData(mon, 39);
        battleMon.attackIV = GetMonData(mon, 40);
        battleMon.defenseIV = GetMonData(mon, 41);
        battleMon.speedIV = GetMonData(mon, 42);
        battleMon.spAttackIV = GetMonData(mon, 43);
        battleMon.spDefenseIV = GetMonData(mon, 44);
        battleMon.personality = GetMonData(mon, 0);
        battleMon.status1 = GetMonData(mon, 55);
        battleMon.level = GetMonData(mon, 56);
        battleMon.hp = GetMonData(mon, 57);
        battleMon.maxHP = GetMonData(mon, 58);
        battleMon.attack = GetMonData(mon, 59);
        battleMon.defense = GetMonData(mon, 60);
        battleMon.speed = GetMonData(mon, 61);
        battleMon.spAttack = GetMonData(mon, 62);
        battleMon.spDefense = GetMonData(mon, 63);
        battleMon.isEgg = GetMonData(mon, 45);
        battleMon.abilityNum = GetMonData(mon, 46);
        battleMon.otId = GetMonData(mon, 1);
        GetMonData(mon, 2, nickname);
        StringCopy_Nickname(battleMon.nickname, nickname);
        GetMonData(mon, 7, battleMon.otName);
        src = (u8 *)&battleMon;
        for (size = 0; size < sizeof(battleMon); ++size)
            dst[size] = src[size];
        break;
    case REQUEST_SPECIES_BATTLE:
        data16 = GetMonData(mon, 11);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_HELDITEM_BATTLE:
        data16 = GetMonData(mon, 12);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_MOVES_PP_BATTLE:
        for (size = 0; size < 4; ++size)
        {
            moveData.moves[size] = GetMonData(mon, 13 + size);
            moveData.pp[size] = GetMonData(mon, 17 + size);
        }
        moveData.ppBonuses = GetMonData(mon, 21);
        src = (u8 *)(&moveData);
    ...
    case REQUEST_MOVE4_BATTLE:
        data16 = GetMonData(mon, 13 + gBattleBufferA[gActiveBattler][1] - REQUEST_MOVE1_BATTLE);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_PP_DATA_BATTLE:
        for (size = 0; size < 4; ++size)
            dst[size] = GetMonData(mon, 17 + size);
        dst[size] = GetMonData(mon, 21);
        ++size;
        break;
    ...
    case REQUEST_PPMOVE4_BATTLE:
        dst[0] = GetMonData(mon, 17 + gBattleBufferA[gActiveBattler][1] - REQUEST_PPMOVE1_BATTLE);
        size = 1;
        break;
    case REQUEST_OTID_BATTLE:
        data32 = GetMonData(mon, 1);
    ...
    case REQUEST_HP_EV_BATTLE:
        dst[0] = GetMonData(mon, 26);
        size = 1;
        break;
    case REQUEST_ATK_EV_BATTLE:
        dst[0] = GetMonData(mon, 27);
        size = 1;
        break;
    case REQUEST_DEF_EV_BATTLE:
        dst[0] = GetMonData(mon, 28);
        size = 1;
        break;
    case REQUEST_SPEED_EV_BATTLE:
        dst[0] = GetMonData(mon, 29);
        size = 1;
        break;
    case REQUEST_SPATK_EV_BATTLE:
        dst[0] = GetMonData(mon, 30);
        size = 1;
        break;
    case REQUEST_SPDEF_EV_BATTLE:
        dst[0] = GetMonData(mon, 31);
        size = 1;
        break;
    case REQUEST_FRIENDSHIP_BATTLE:
        dst[0] = GetMonData(mon, 32);
        size = 1;
        break;
    case REQUEST_POKERUS_BATTLE:
        dst[0] = GetMonData(mon, 34);
        size = 1;
        break;
    case REQUEST_MET_LOCATION_BATTLE:
        dst[0] = GetMonData(mon, 35);
        size = 1;
        break;
    case REQUEST_MET_LEVEL_BATTLE:
        dst[0] = GetMonData(mon, 36);
        size = 1;
        break;
    case REQUEST_MET_GAME_BATTLE:
        dst[0] = GetMonData(mon, 37);
        size = 1;
        break;
    case REQUEST_POKEBALL_BATTLE:
        dst[0] = GetMonData(mon, 38);
        size = 1;
        break;
    case REQUEST_ALL_IVS_BATTLE:
        dst[0] = GetMonData(mon, 39);
        dst[1] = GetMonData(mon, 40);
        dst[2] = GetMonData(mon, 41);
        dst[3] = GetMonData(mon, 42);
        dst[4] = GetMonData(mon, 43);
        dst[5] = GetMonData(mon, 44);
        size = 6;
        break;
    ...
    case REQUEST_ATK_IV_BATTLE:
        dst[0] = GetMonData(mon, 40);
        size = 1;
        break;
    case REQUEST_DEF_IV_BATTLE:
        dst[0] = GetMonData(mon, 41);
        size = 1;
        break;
    case REQUEST_SPEED_IV_BATTLE:
        dst[0] = GetMonData(mon, 42);
        size = 1;
        break;
    case REQUEST_SPATK_IV_BATTLE:
        dst[0] = GetMonData(mon, 43);
        size = 1;
        break;
    case REQUEST_SPDEF_IV_BATTLE:
        dst[0] = GetMonData(mon, 44);
        size = 1;
        break;
    case REQUEST_PERSONALITY_BATTLE:
        data32 = GetMonData(mon, 0);
        dst[0] = (data32 & 0x000000FF);
        dst[1] = (data32 & 0x0000FF00) >> 8;
        dst[2] = (data32 & 0x00FF0000) >> 16;
    ...
    case REQUEST_CHECKSUM_BATTLE:
        data16 = GetMonData(mon, 9);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_STATUS_BATTLE:
        data32 = GetMonData(mon, 55);
        dst[0] = (data32 & 0x000000FF);
        dst[1] = (data32 & 0x0000FF00) >> 8;
        dst[2] = (data32 & 0x00FF0000) >> 16;
        dst[3] = (data32 & 0xFF000000) >> 24;
    ...
    case REQUEST_LEVEL_BATTLE:
        dst[0] = GetMonData(mon, 56);
        size = 1;
        break;
    case REQUEST_HP_BATTLE:
        data16 = GetMonData(mon, 57);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_MAX_HP_BATTLE:
        data16 = GetMonData(mon, 58);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_ATK_BATTLE:
        data16 = GetMonData(mon, 59);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_DEF_BATTLE:
        data16 = GetMonData(mon, 60);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_SPEED_BATTLE:
        data16 = GetMonData(mon, 61);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_SPATK_BATTLE:
        data16 = GetMonData(mon, 62);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_SPDEF_BATTLE:
        data16 = GetMonData(mon, 63);
        dst[0] = data16;
        dst[1] = data16 >> 8;
        size = 2;
        break;
    case REQUEST_COOL_BATTLE:
        dst[0] = GetMonData(mon, 22);
        size = 1;
        break;
    case REQUEST_BEAUTY_BATTLE:
        dst[0] = GetMonData(mon, 23);
        size = 1;
        break;
    case REQUEST_CUTE_BATTLE:
        dst[0] = GetMonData(mon, 24);
        size = 1;
        break;
    case REQUEST_SMART_BATTLE:
        dst[0] = GetMonData(mon, 33);
        size = 1;
        break;
    case REQUEST_TOUGH_BATTLE:
        dst[0] = GetMonData(mon, 47);
        size = 1;
        break;
    case REQUEST_SHEEN_BATTLE:
        dst[0] = GetMonData(mon, 48);
        size = 1;
        break;
    case REQUEST_COOL_RIBBON_BATTLE:
        dst[0] = GetMonData(mon, 50);
        size = 1;
        break;
    case REQUEST_BEAUTY_RIBBON_BATTLE:
        dst[0] = GetMonData(mon, 51);
        size = 1;
        break;
    case REQUEST_CUTE_RIBBON_BATTLE:
        dst[0] = GetMonData(mon, 52);
        size = 1;
        break;
    case REQUEST_SMART_RIBBON_BATTLE:
        dst[0] = GetMonData(mon, 53);
        size = 1;
        break;
    case REQUEST_TOUGH_RIBBON_BATTLE:
        dst[0] = GetMonData(mon, 54);
        size = 1;
        break;
    }
    return size;
...
static void SetPokedudeMonData(u8 monId)
{
    ...
    case REQUEST_ALL_BATTLE:
    {
        ...
        SetMonData(mon, 11, &battlePokemon->species);
        SetMonData(mon, 12, &battlePokemon->item);
        for (i = 0; i < 4; ++i)
        {
            SetMonData(mon, 13 + i, &battlePokemon->moves[i]);
            SetMonData(mon, 17 + i, &battlePokemon->pp[i]);
        }
        SetMonData(mon, 21, &battlePokemon->ppBonuses);
        SetMonData(mon, 32, &battlePokemon->friendship);
        SetMonData(mon, 25, &battlePokemon->experience);
        iv = battlePokemon->hpIV;
        SetMonData(mon, 39, &iv);
        iv = battlePokemon->attackIV;
        SetMonData(mon, 40, &iv);
        iv = battlePokemon->defenseIV;
        SetMonData(mon, 41, &iv);
        iv = battlePokemon->speedIV;
        SetMonData(mon, 42, &iv);
        iv = battlePokemon->spAttackIV;
        SetMonData(mon, 43, &iv);
        iv = battlePokemon->spDefenseIV;
        SetMonData(mon, 44, &iv);
        SetMonData(mon, 0, &battlePokemon->personality);
        SetMonData(mon, 55, &battlePokemon->status1);
        SetMonData(mon, 56, &battlePokemon->level);
        SetMonData(mon, 57, &battlePokemon->hp);
        SetMonData(mon, 58, &battlePokemon->maxHP);
        SetMonData(mon, 59, &battlePokemon->attack);
        SetMonData(mon, 60, &battlePokemon->defense);
        SetMonData(mon, 61, &battlePokemon->speed);
        SetMonData(mon, 62, &battlePokemon->spAttack);
        SetMonData(mon, 63, &battlePokemon->spDefense);
    }
        break;
    case REQUEST_SPECIES_BATTLE:
        SetMonData(mon, 11, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_HELDITEM_BATTLE:
        SetMonData(mon, 12, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_MOVES_PP_BATTLE:
        for (i = 0; i < 4; ++i)
        {
            SetMonData(mon, 13 + i, &moveData->moves[i]);
            SetMonData(mon, 17 + i, &moveData->pp[i]);
        }
        SetMonData(mon, 21, &moveData->ppBonuses);
        break;
    case REQUEST_MOVE1_BATTLE:
    case REQUEST_MOVE2_BATTLE:
    case REQUEST_MOVE3_BATTLE:
    case REQUEST_MOVE4_BATTLE:
        SetMonData(mon, 13 + gBattleBufferA[gActiveBattler][1] - REQUEST_MOVE1_BATTLE, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_PP_DATA_BATTLE:
        SetMonData(mon, 17, &gBattleBufferA[gActiveBattler][3]);
        SetMonData(mon, 18, &gBattleBufferA[gActiveBattler][4]);
        SetMonData(mon, 19, &gBattleBufferA[gActiveBattler][5]);
        SetMonData(mon, 20, &gBattleBufferA[gActiveBattler][6]);
        SetMonData(mon, 21, &gBattleBufferA[gActiveBattler][7]);
        break;
    case REQUEST_PPMOVE1_BATTLE:
    case REQUEST_PPMOVE2_BATTLE:
    case REQUEST_PPMOVE3_BATTLE:
    case REQUEST_PPMOVE4_BATTLE:
        SetMonData(mon, 17 + gBattleBufferA[gActiveBattler][1] - REQUEST_PPMOVE1_BATTLE, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_OTID_BATTLE:
        SetMonData(mon, 1, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_EXP_BATTLE:
        SetMonData(mon, 25, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_HP_EV_BATTLE:
        SetMonData(mon, 26, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_ATK_EV_BATTLE:
        SetMonData(mon, 27, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_DEF_EV_BATTLE:
        SetMonData(mon, 28, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPEED_EV_BATTLE:
        SetMonData(mon, 29, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPATK_EV_BATTLE:
        SetMonData(mon, 30, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPDEF_EV_BATTLE:
        SetMonData(mon, 31, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_FRIENDSHIP_BATTLE:
        SetMonData(mon, 32, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_POKERUS_BATTLE:
        SetMonData(mon, 34, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_MET_LOCATION_BATTLE:
        SetMonData(mon, 35, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_MET_LEVEL_BATTLE:
        SetMonData(mon, 36, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_MET_GAME_BATTLE:
        SetMonData(mon, 37, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_POKEBALL_BATTLE:
        SetMonData(mon, 38, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_ALL_IVS_BATTLE:
        SetMonData(mon, 39, &gBattleBufferA[gActiveBattler][3]);
        SetMonData(mon, 40, &gBattleBufferA[gActiveBattler][4]);
        SetMonData(mon, 41, &gBattleBufferA[gActiveBattler][5]);
        SetMonData(mon, 42, &gBattleBufferA[gActiveBattler][6]);
        SetMonData(mon, 43, &gBattleBufferA[gActiveBattler][7]);
        SetMonData(mon, 44, &gBattleBufferA[gActiveBattler][8]);
        break;
    case REQUEST_HP_IV_BATTLE:
        SetMonData(mon, 39, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_ATK_IV_BATTLE:
        SetMonData(mon, 40, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_DEF_IV_BATTLE:
        SetMonData(mon, 41, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPEED_IV_BATTLE:
        SetMonData(mon, 42, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPATK_IV_BATTLE:
        SetMonData(mon, 43, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPDEF_IV_BATTLE:
        SetMonData(mon, 44, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_PERSONALITY_BATTLE:
        SetMonData(mon, 0, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_CHECKSUM_BATTLE:
        SetMonData(mon, 9, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_STATUS_BATTLE:
        SetMonData(mon, 55, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_LEVEL_BATTLE:
        SetMonData(mon, 56, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_HP_BATTLE:
        SetMonData(mon, 57, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_MAX_HP_BATTLE:
        SetMonData(mon, 58, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_ATK_BATTLE:
        SetMonData(mon, 59, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_DEF_BATTLE:
        SetMonData(mon, 60, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPEED_BATTLE:
        SetMonData(mon, 61, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPATK_BATTLE:
        SetMonData(mon, 62, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SPDEF_BATTLE:
        SetMonData(mon, 63, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_COOL_BATTLE:
        SetMonData(mon, 22, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_BEAUTY_BATTLE:
        SetMonData(mon, 23, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_CUTE_BATTLE:
        SetMonData(mon, 24, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SMART_BATTLE:
        SetMonData(mon, 33, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_TOUGH_BATTLE:
        SetMonData(mon, 47, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SHEEN_BATTLE:
        SetMonData(mon, 48, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_COOL_RIBBON_BATTLE:
        SetMonData(mon, 50, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_BEAUTY_RIBBON_BATTLE:
        SetMonData(mon, 51, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_CUTE_RIBBON_BATTLE:
        SetMonData(mon, 52, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_SMART_RIBBON_BATTLE:
        SetMonData(mon, 53, &gBattleBufferA[gActiveBattler][3]);
        break;
    case REQUEST_TOUGH_RIBBON_BATTLE:
        SetMonData(mon, 54, &gBattleBufferA[gActiveBattler][3]);
        break;
    }
    HandleLowHpMusicChange(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], gActiveBattler);
}
...
static void PokedudeHandleHealthBarUpdate(void)
{
    ...
    if (hpVal != 0x7FFF)
    {
        u32 maxHP = GetMonData(mon, 58);
        u32 curHP = GetMonData(mon, 57);
...
static void PokedudeHandleExpUpdate(void)
{
    ...
    else
    {
        ...
        expPointsToGive = ((&gBattleBufferA[gActiveBattler][2])[0] | ((&gBattleBufferA[gActiveBattler][2])[1] << 8));
...
static void PokedudeHandleStatusXor(void)
{
    ...
    if (GetBattlerSide(gActiveBattler) == 0)
        ...
        mon = &gEnemyParty[gBattlerPartyIndexes[gActiveBattler]];
    val = GetMonData(mon, 55) ^ gBattleBufferA[gActiveBattler][1];
    SetMonData(mon, 55, &val);
    PokedudeBufferExecCompleted();
}
...
static void PokedudeHandleIntroTrainerBallThrow(void)
{
    ...
    LoadCompressedPalette(gTrainerBackPicPaletteTable[4].data, (0x100 + ((paletteNum) * 16)), ((16) * sizeof(u16)));
...
static void StartSendOutAnim(u8 battlerId)
{
    ...
    gSprites[gBattleControllerData[battlerId]].data[0] = DoPokeballSendOutAnimation(0, 0xFF);
}
...
struct PokedudeBattlerState *gPokedudeBattlerStates[4];

static void PokedudeSimulateInputChooseAction(void)
{
    const struct PokedudeInputScript *script_p = sInputScripts_ChooseAction[gBattleStruct->simulatedInputState[2]];
...
static void PokedudeSimulateInputChooseMove(void)
{
    const struct PokedudeInputScript *script_p = sInputScripts_ChooseMove[gBattleStruct->simulatedInputState[2]];
    ...
    if (script_p[gPokedudeBattlerStates[gActiveBattler]->move_idx].delay[gActiveBattler] == gPokedudeBattlerStates[gActiveBattler]->timer)
    {
        ...
        BtlController_EmitTwoReturnValues(1,
                                          10,
                                          script_p[gPokedudeBattlerStates[gActiveBattler]->move_idx].cursorPos[gActiveBattler] | ((gActiveBattler ^ 1) << 8));
        PokedudeBufferExecCompleted();
...
static bool8 HandlePokedudeVoiceoverEtc(void)
{
    const struct PokedudeTextScriptHeader *header_p = sPokedudeTextScripts[gBattleStruct->simulatedInputState[2]];
    const u16 * bstringid_p = (const u16 *)&gBattleBufferA[gActiveBattler][2];

    if (gBattleBufferA[gActiveBattler][0] != header_p[gBattleStruct->simulatedInputState[3]].btlcmd)
        return 0;
    if (gActiveBattler != header_p[gBattleStruct->simulatedInputState[3]].side)
        return 0;
    if (gBattleBufferA[gActiveBattler][0] == CONTROLLER_PRINTSTRING && header_p[gBattleStruct->simulatedInputState[3]].stringid != *bstringid_p)
        return 0;
    if (header_p[gBattleStruct->simulatedInputState[3]].callback == ((void *)0))
    {
        gBattleStruct->simulatedInputState[3]++;
        return 0;
    }
    gBattlerControllerFuncs[gActiveBattler] = header_p[gBattleStruct->simulatedInputState[3]].callback;
    gPokedudeBattlerStates[gActiveBattler]->timer = 0;
    gPokedudeBattlerStates[gActiveBattler]->msg_idx = header_p[gBattleStruct->simulatedInputState[3]].stringid;
    gBattleStruct->simulatedInputState[3]++;
    return 1;
}
...
static void PokedudeAction_PrintVoiceoverMessage(void)
{
    ...
    case 0:
        if (!gPaletteFade.active)
        {
            BeginNormalPaletteFade(0xFFFFFF7F, 4, 0, 8, ((0) | ((0) << 5) | ((0) << 10)));
            ++gPokedudeBattlerStates[gActiveBattler]->timer;
        }
        break;
    case 1:
    ...
    case 2:
        ...
        break;
    case 3:
        if (!IsTextPrinterActive(24) && ({(gMain.newKeys) & (0x0001);}))
        {
            PlaySE(5);
            BeginNormalPaletteFade(0xFFFFFF7F, 4, 8, 0, ((0) | ((0) << 5) | ((0) << 10)));
            ++gPokedudeBattlerStates[gActiveBattler]->timer;
        }
        break;
    case 4:
        if (!gPaletteFade.active)
        {
            if (gPokedudeBattlerStates[gActiveBattler]->msg_idx == 13)
            {
...
static void PokedudeAction_PrintMessageWithHealthboxPals(void)
{
    ...
    case 1:
        if (!gPaletteFade.active)
        {
            ...
            ++mask;
            --mask;
            BeginNormalPaletteFade(mask, 4, 8, 0, ((0) | ((0) << 5) | ((0) << 10)));
            ++gPokedudeBattlerStates[gActiveBattler]->timer;
        }
        break;
    case 2:
    ...
    case 4:
        if (!IsTextPrinterActive(24) && ({(gMain.newKeys) & (0x0001);}))
        {
            ...
            ++mask;
            --mask;
            BeginNormalPaletteFade(mask, 4, 0, 8, ((0) | ((0) << 5) | ((0) << 10)));
            ++gPokedudeBattlerStates[gActiveBattler]->timer;
        }
        break;
    case 5:
    ...
    case 6:
        if (!gPaletteFade.active)
        {
            ...
            DoFreeHealthboxPalsForLevelUp(GetBattlerAtPosition(0));
            BtlCtrl_RemoveVoiceoverMessageFrame();
            ReturnFromPokedudeAction();
        }
        break;
