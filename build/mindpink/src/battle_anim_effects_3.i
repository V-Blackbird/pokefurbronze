...
static void SetPsychicBackground_Step(u8 taskId)
{
    ...
    {
        lastColor = gPlttBufferFaded[(0x000 + ((paletteIndex) * 16)) + 11];
        for (i = 10; i > 0; i--)
            gPlttBufferFaded[(0x000 + ((paletteIndex) * 16)) + i + 1] = gPlttBufferFaded[(0x000 + ((paletteIndex) * 16)) + i];

        gPlttBufferFaded[(0x000 + ((paletteIndex) * 16)) + 1] = lastColor;
...
static void FadeScreenToWhite_Step(u8 taskId)
{
    ...
    if (++gTasks[taskId].data[5] == 4)
    {
        ...
        lastColor = gPlttBufferUnfaded[(0x000 + ((paletteIndex) * 16)) + 11];
        for (i = 10; i > 0; i--)
            gPlttBufferUnfaded[(0x000 + ((paletteIndex) * 16)) + i + 1] = gPlttBufferUnfaded[(0x000 + ((paletteIndex) * 16)) + i];
        gPlttBufferUnfaded[(0x000 + ((paletteIndex) * 16)) + 1] = lastColor;
...
void AnimTask_MorningSunLightBeam(u8 taskId)
{
    ...
    case 0:
        ...
        LoadCompressedPalette(gBattleAnim_MorningSunPal, (0x000 + ((animBg.paletteId) * 16)), ((16) * sizeof(u16)));
        if (IsContest())
        {
...
void AnimTask_RolePlaySilhouette(u8 taskId)
{
    ...
    FillPalette(((31) | ((31) << 5) | ((31) << 10)), (0x100 + ((gSprites[spriteId].oam.paletteNum) * 16)), ((16) * sizeof(u16)));
    gSprites[spriteId].oam.priority = priority;
    ...
    case 1:
        {
            ...
            if (gBattleSpritesDataPtr->battlerData[gBattleAnimAttacker].transformSpecies != 0)
                BlendPalette((0x100 + ((gSprites[spriteId2].oam.paletteNum) * 16)), 16, 6, ((31) | ((31) << 5) | ((31) << 10)));
