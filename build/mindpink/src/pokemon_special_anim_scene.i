# 0 "src/pokemon_special_anim_scene.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokemon_special_anim_scene.c"
...
# 2 "src/pokemon_special_anim_scene.c" 2
# 1 "include/gflib.h" 1
...
# 3 "src/pokemon_special_anim_scene.c" 2
# 1 "include/data.h" 1
# 9 "include/data.h"
struct MonCoords
{
...
# 4 "src/pokemon_special_anim_scene.c" 2
...
# 5 "src/pokemon_special_anim_scene.c" 2
...
# 6 "src/pokemon_special_anim_scene.c" 2
# 1 "include/item.h" 1
...
# 7 "src/pokemon_special_anim_scene.c" 2
...
# 8 "src/pokemon_special_anim_scene.c" 2
# 1 "include/menu.h" 1
# 18 "include/menu.h"
struct MenuAction
{
...
# 9 "src/pokemon_special_anim_scene.c" 2
...
# 10 "src/pokemon_special_anim_scene.c" 2
# 1 "include/pokemon_special_anim_internal.h" 1
...
# 5 "include/pokemon_special_anim_internal.h" 2
...
struct PokemonSpecialAnim
{
               ...
               u8 nickname[10 + 1];
               u8 nameOfMoveForgotten[13];
               u8 nameOfMoveToTeach[13];
               bool8 cancelDisabled;
               u16 state;
               u16 species;
               u16 itemId;
               u16 animType;
               ...
               u32 personality;
               u32 field_00a4;
               struct PokemonSpecialAnimScene sceneResources;
};
...
# 11 "src/pokemon_special_anim_scene.c" 2
...
# 12 "src/pokemon_special_anim_scene.c" 2
...
extern const u8 gText_PokeSum_EggOrigin_Trade[];
# 13 "src/pokemon_special_anim_scene.c" 2
...
# 14 "src/pokemon_special_anim_scene.c" 2
# 1 "include/trig.h" 1
...
s16 Sin2(u16 angle);
s16 Cos2(u16 angle);
# 15 "src/pokemon_special_anim_scene.c" 2
# 1 "include/constants/songs.h" 1
# 16 "src/pokemon_special_anim_scene.c" 2
...
static const union AffineAnimCmd sAffineAnim_Zoom_1[] = {
    {.frame = {.xScale = 0x155, .yScale = 0x155, .rotation = 0, .duration = 0}},
    {.type = 0x7FFF}
};

static const union AffineAnimCmd sAffineAnim_Zoom_2[] = {
    {.frame = {.xScale = 0x1AA, .yScale = 0x1AA, .rotation = 0, .duration = 0}},
    {.type = 0x7FFF}
};
...
static const union AffineAnimCmd sAffineAnim_ItemZoom_0[] = {
    {.frame = {.xScale = 0x100, .yScale = 0x100, .rotation = 0, .duration = 0}},
    {.frame = {.xScale = -28, .yScale = -28, .rotation = 0, .duration = 8}},
    {.type = 0x7FFF}
};

static const union AffineAnimCmd sAffineAnim_ItemZoom_1[] = {
    {.frame = {.xScale = 0x155, .yScale = 0x155, .rotation = 0, .duration = 0}},
    {.frame = {.xScale = -37, .yScale = -37, .rotation = 0, .duration = 8}},
    {.type = 0x7FFF}
};

static const union AffineAnimCmd sAffineAnim_ItemZoom_2[] = {
    {.frame = {.xScale = 0x1AA, .yScale = 0x1AA, .rotation = 0, .duration = 0}},
    {.frame = {.xScale = -47, .yScale = -47, .rotation = 0, .duration = 8}},
    {.type = 0x7FFF}
};

static const union AffineAnimCmd sAffineAnim_ItemZoom_3[] = {
    {.frame = {.xScale = 0x200, .yScale = 0x200, .rotation = 0, .duration = 0}},
    {.frame = {.xScale = -56, .yScale = -56, .rotation = 0, .duration = 8}},
    {.type = 0x7FFF}
};
...
void InitPokemonSpecialAnimScene(struct PokemonSpecialAnimScene * buffer, u16 animType)
{
    ...
    SetGpuRegBits(0x0, 0x0000 | 0x0040 | 0x1000);
    SetGpuReg(0x50, 0);
}
...
void PSA_PrintMessage(u8 messageId)
{
    ...
    case 0:
        ...
        GetMonData(pokemon, 2, str);
        StringAppend(scene->textBuf, gText_Period);
        break;
    case 1:
        level = GetMonData(pokemon, 56);
        GetMonData(pokemon, 2, scene->textBuf);
        str = StringAppend(scene->textBuf, gText_LevelRoseTo);
        if (level < 100)
            level++;
        str = ConvertIntToDecimalStringN(str, level, STR_CONV_MODE_LEFT_ALIGN, level < 100 ? 2 : 3);
    ...
    AddTextPrinterParameterized5(0, FONT_NORMAL, scene->textBuf, strWidth, 0, textSpeed, ((void *)0), 0, 4);
}
...
void PSA_DarkenMonSprite(void)
{
    struct PokemonSpecialAnimScene * scene = PSA_GetSceneWork();
    scene->state = 0;
    BlendPalettes((0x10000 << IndexOfSpritePaletteTag(0)) | 4, 16, ((0) | ((0) << 5) | ((0) << 10)));
...
bool8 PSA_RunPoofAnim(void)
{
    ...
    case 0:
        ...
        {
            BeginNormalPaletteFade((0x10000 << IndexOfSpritePaletteTag(0)) | 4, -1, 16, 0, ((0) | ((0) << 5) | ((0) << 10)));
            scene->state++;
        }
        break;
    case 1:
...
bool8 PSA_UseTM_RunZoomOutAnim(void)
{
    ...
    case 4:
        ...
        if (scene->field_0004 > 0)
        {
            scene->field_0004 = 0;
            PlaySE(133);
            BeginNormalPaletteFade(0x00000001, 2, 0, 12, ((8) | ((13) << 5) | ((31) << 10)));
    ...
    case 5:
        ...
        if (scene->field_0004 > 70)
        {
            ...
            BeginNormalPaletteFade(0x00000001, 6, 12, 0, ((8) | ((13) << 5) | ((31) << 10)));
            scene->field_0004 = 0;
            scene->state++;
        }
        break;
...
void PSA_DrawLevelUpWindowPg1(u16 *statsBefore, u16 *statsAfter)
{
    ...
    DrawLevelUpWindowPg1(1, statsBefore, statsAfter, 0x1, 0x2, 0x3);
    PutWindowTilemap(1);
...
{
    DrawLevelUpWindowPg2(1, currStats, 0x1, 0x2, 0x3);
...
static void LoadBgGfxByAnimType(u16 animType)
{
    ...
    if (animType != 4)
        ...
        LoadPalette(sBg_TmHm_Pal, (0x000 + ((0) * 16)), sizeof(sBg_TmHm_Pal));
}
...
void PSA_CreateMonSpriteAtCloseness(u8 closeness)
{
    struct PokemonSpecialAnimScene * scene = PSA_GetSceneWork();
    struct Pokemon * pokemon = PSA_GetPokemon();
    u16 species = GetMonData(pokemon, 11);
    ...
    r9 = Alloc(0x2000);
    r4 = Alloc(0x100);
    if (r6 != ((void *)0) && r9 != ((void *)0) && r4 != ((void *)0))
    {
        ...
        if (spriteId != 64)
        {
            ...
            scene->monSprite = ((void *)0);
        scene->lastCloseness = closeness;
    }
    if (r6 != ((void *)0)) Free(r6);
    if (r9 != ((void *)0)) Free(r9);
    if (r4 != ((void *)0)) Free(r4);
}
# 700 "src/pokemon_special_anim_scene.c"
void PSA_SetUpZoomAnim(u8 closeness)
{
    struct PokemonSpecialAnimScene * scene = PSA_GetSceneWork();
    u8 taskId;
    if (closeness != scene->lastCloseness)
    {
        ...
        gTasks[taskId].data[1] = scene->lastCloseness;
...
static void Task_ZoomAnim(u8 taskId)
{
    ...
    case 0:
        ...
        if (data[8])
            SetSpriteWithCloseness((void *)GetWordTaskArg(taskId, 9), data[1]);
        data[1] += data[3];
        data[0]++;
        break;
    case 1:
        ...
        {
            PlaySE(109);
            MonSpriteZoom_UpdateYPos(sprite, data[1]);
            if (data[8])
                ItemSpriteZoom_UpdateYPos((void *)GetWordTaskArg(taskId, 9), data[1]);
            if (data[1] == data[2])
            {
                PSA_GetSceneWork()->lastCloseness = data[2];
                DestroyTask(taskId);
            }
            else
            {
...
# 1105 "src/pokemon_special_anim_scene.c"
static void MachineSetWobbleInit(void)
{
    struct PokemonSpecialAnimScene * scene = PSA_GetSceneWork();
...
# 1155 "src/pokemon_special_anim_scene.c"
static void StartZoomOutAnimForUseTM(u8 closeness)
{
    struct PokemonSpecialAnimScene * scene = PSA_GetSceneWork();
    u8 taskId;
...
# 1185 "src/pokemon_special_anim_scene.c"
static void CreateStarSprites(struct PokemonSpecialAnimScene * scene)
{
    int i;
    u8 spriteId;
...
# 1249 "src/pokemon_special_anim_scene.c"
static void PSAScene_SeedRandomInTask(struct PokemonSpecialAnimScene * scene)
{
    u8 taskId;
    LoadOutwardSpiralDotsGfx();
...
static void Task_UseItem_OutwardSpiralDots(u8 taskId)
{
    ...
    case 0:
        if (data[1] == 0)
        {
            ...
            x2 = x + ((u32)(gSineTable[data[5] + 0x40] * ampl) >> 8);
            y2 = y + ((u32)(gSineTable[data[5] ] * ampl) >> 8);
            data[5] += 0x4C;
            data[5] &= 0xFF;
            ...
            if (spriteId != 64)
            {
                ...
                gSprites[spriteId].data[1] = (PSAScene_RandomFromTask(taskId) & 1) + 6;
...
# 1379 "src/pokemon_special_anim_scene.c"
void CreateLevelUpVerticalSpritesTask(u16 x, u16 y, u16 tileTag, u16 paletteTag, u16 priority, u16 subpriority)
{
    ...
    gTasks[taskId].data[4] = x - 32;
    gTasks[taskId].data[5] = y + 32;
    gTasks[taskId].data[6] = tileTag;
    gTasks[taskId].data[7] = paletteTag;
    gTasks[taskId].data[8] = priority;
    gTasks[taskId].data[9] = subpriority;
    SetGpuReg(0x50, (0 << 6) | (((1 << 8) | (1 << 9) | (1 << 10) | (1 << 11)) | (1 << 12) | (1 << 13)));
    SetGpuReg(0x52, (((6) << 8) | (12)));
}
...
static void CreateLevelUpVerticalSprite(u8 taskId, s16 *data)
{
    ...
    if (spriteId != 64)
    {
        ...
        gSprites[spriteId].data[2] = ((1103515245 * (data[2]) + 24691) & 0x3F) + 0x20;
        gSprites[spriteId].data[7] = taskId;
        data[1]++;
    }
}
...
# 1484 "src/pokemon_special_anim_scene.c"
...
void DrawLevelUpWindowPg1(u16 windowId, u16 *beforeStats, u16 *afterStats, u8 bgColor, u8 fgColor, u8 shadowColor)
{
    ...
    for (i = 0; i < 6; i++)
    {
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, 0, i * 15, textColor, 0xFF, sLevelUpWindowStatNames[i]);
        StringCopy(textbuf, diffStats[i] >= 0 ? gText_LevelUp_Plus : gText_LevelUp_Minus);
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, 56, i * 15, textColor, 0xFF, textbuf);
        textbuf[0] = 0x00;
        x = abs(diffStats[i]) < 10 ? 12 : 6;
        ConvertIntToDecimalStringN(textbuf + 1, abs(diffStats[i]), STR_CONV_MODE_LEFT_ALIGN, 2);
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, x + 56, i * 15, textColor, 0xFF, textbuf);
    }
}
...
void DrawLevelUpWindowPg2(u16 windowId, u16 *currStats, u8 bgColor, u8 fgColor, u8 shadowColor)
{
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        x = 6 * (4 - ndigits);
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, 0, i * 15, textColor, 0xFF, sLevelUpWindowStatNames[i]);
        AddTextPrinterParameterized3(windowId, FONT_NORMAL, 56 + x, i * 15, textColor, 0xFF, textbuf);
    }
}
