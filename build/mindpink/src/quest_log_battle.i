# 0 "src/quest_log_battle.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/quest_log_battle.c"
...
void TrySetQuestLogBattleEvent(void)
{
    if (!(gBattleTypeFlags & ((1 << 1) | (1 << 9) | (1 << 16))) && (gBattleOutcome == 1 || gBattleOutcome == 7))
    {
        ...
        if (gBattleTypeFlags & (1 << 3))
        {
            ...
            if (gBattleTypeFlags & (1 << 0))
            {
                ...
                if (GetBattlerSide(gBattleStruct->field_182) == 0)
        ...
        else
        {
            if (gBattleOutcome == 1)
            {
                questLogWildBattleRecord->defeatedSpecies = GetMonData(gEnemyParty, 11);
                questLogWildBattleRecord->caughtSpecies = 0;
            }
            else
            {
                questLogWildBattleRecord->defeatedSpecies = 0;
                questLogWildBattleRecord->caughtSpecies = GetMonData(gEnemyParty, 11);
            }
            questLogWildBattleRecord->mapSec = GetCurrentRegionMapSectionId();
            SetQuestLogEvent(31, (const u16 *)questLogWildBattleRecord);
        }
