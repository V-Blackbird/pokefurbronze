...
static __attribute__((section("ewram_data"))) s16 sViewportObjectEvents[16][4] = {0};
__attribute__((section("ewram_data"))) struct ShopData gShopData = {0};
static __attribute__((section("ewram_data"))) u8 sShopMenuWindowId = 0;
__attribute__((section("ewram_data"))) u16 (*gShopTilemapBuffer1)[0x400] = {0};
__attribute__((section("ewram_data"))) u16 (*gShopTilemapBuffer2)[0x400] = {0};
__attribute__((section("ewram_data"))) u16 (*gShopTilemapBuffer3)[0x400] = {0};
__attribute__((section("ewram_data"))) u16 (*gShopTilemapBuffer4)[0x400] = {0};
__attribute__((section("ewram_data"))) struct ListMenuItem *sShopMenuListMenu = {0};
static __attribute__((section("ewram_data"))) u8 (*sShopMenuItemStrings)[13] = {0};
__attribute__((section("ewram_data"))) struct MartHistory gShopMenuHistory[2] = {0};
...
static u8 GetMartTypeFromItemList(u32 a0)
{
    ...
    {
        if (ItemId_GetPocket(gShopData.itemList[i]) == 4)
            return 1;
    }
    return 0;
}
...
static void BuyMenuInitBgs(void)
{
    ...
    InitBgsFromTemplates(0, sShopBuyMenuBgTemplates, (sizeof(sShopBuyMenuBgTemplates) / sizeof((sShopBuyMenuBgTemplates)[0])));
...
static void BuyMenuDecompressBgGraphics(void)
{
    ...
    pal = Alloc(2 * ((16) * sizeof(u16)));
    LZDecompressWram(gBuyMenuFrame_Pal, pal);
    LoadPalette(&pal[0 * 16], (0x000 + ((11) * 16)), ((16) * sizeof(u16)));
    LoadPalette(&pal[1 * 16], (0x000 + ((6) * 16)), ((16) * sizeof(u16)));
    Free(pal);
}
...
bool8 BuyMenuBuildListMenuTemplate(void)
{
    ...
    if (sShopMenuListMenu == ((void *)0)
     || (sShopMenuItemStrings = Alloc((gShopData.itemCount + 1) * sizeof(*sShopMenuItemStrings))) == ((void *)0))
    {
        BuyMenuFreeMemory();
        SetShopExitCallback();
        return 0;
...
static void BuyMenuPrintItemDescriptionAndShowItemIcon(s32 item, bool8 onInit, struct ListMenu *list)
{
    ...
    if (gShopData.martType != 1)
    {
        ...
        if (item != -2)
            ...
            CreateItemMenuIcon(376, gShopData.itemSlot);
...
static void BuyMenuPrintPriceInList(u8 windowId, u32 item, u8 y)
{
    ...
    if (item != -2)
    {
        ...
        BuyMenuPrint(windowId, FONT_SMALL, gStringVar4, 0x69, y, 0, 0, 0xFF, 1);
    }
}
...
static void LoadTmHmNameInMart(s32 item)
{
    if (item != -2)
    {
        ConvertIntToDecimalStringN(gStringVar1, item - 289, 2, 2);
        StringCopy(gStringVar4, gText_NumberClear01);
        StringAppend(gStringVar4, gStringVar1);
        BuyMenuPrint(6, FONT_SMALL, gStringVar4, 0, 0, 0, 0, 0xFF, 1);
        StringCopy(gStringVar4, gMoveNames[ItemIdToBattleMoveId(item)]);
        BuyMenuPrint(6, FONT_NORMAL, gStringVar4, 0, 0x10, 0, 0, 0, 1);
    }
    else
    {
        BuyMenuPrint(6, FONT_SMALL, gText_ThreeHyphens, 0, 0, 0, 0, 0xFF, 1);
...
static void BuyMenuFreeMemory(void)
{
    ...
    if (sShopMenuItemStrings != ((void *)0))
    ...
    for (j = 0; j < 10; j++)
    {
        for (i = 0; i < 5; i++)
        {
            ...
            if (metatile < 640)
                ...
                BuyMenuDrawMapMetatile(i, j, mapLayout->secondaryTileset->metatiles + ((metatile - 640) * 8), metatileLayerType);
        }
    }
}
...
static void BuyMenuCollectObjectEventData(void)
{
    ...
    for (y = 0; y < 16; y++)
        sViewportObjectEvents[y][OBJECT_EVENT_ID] = 16;
...
static void BuyMenuDrawObjectEvents(void)
{
    ...
    for (i = 0; i < 16; i++)
    {
        if (sViewportObjectEvents[i][OBJECT_EVENT_ID] == 16)
            continue;
...
static void BuyMenuPrintItemQuantityAndPrice(u8 taskId)
{
    ...
    PrintMoneyAmount(3, 0x36, 0xA, gShopData.itemPrice, 0xFF);
...
static void Task_BuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        ListMenuGetScrollAndRow(data[7], &gShopData.scrollOffset, &gShopData.selectedRow);
        switch (itemId)
        {
        case -1:
            break;
...
{
    s16 *data = gTasks[taskId].data;
    u16 quantityInBag = BagGetQuantityByItemId(data[5]);
    u16 maxQuantity;
    ...
    ScheduleBgCopyTilemapToVram(0);
    maxQuantity = GetMoney(&gSaveBlock1Ptr->money) / ItemId_GetPrice(data[5]);
    if (maxQuantity > 99)
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    if (AdjustQuantityAccordingToDPadInput(&data[1], gShopData.maxQuantity) == 1)
    {
        gShopData.itemPrice = ItemId_GetPrice(data[5]) * data[1];
        BuyMenuPrintItemQuantityAndPrice(taskId);
    }
    else
    {
...
static void BuyMenuTryMakePurchase(u8 taskId)
{
    ...
    if (AddBagItem(data[5], data[1]) == 1)
    {
        ...
        RecordItemPurchase(data[5], data[1], 1);
    }
    else
    {
...
static void Task_ExitBuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        DestroyListMenuTask(data[7], ((void *)0), ((void *)0));
        BuyMenuFreeMemory();
        SetMainCallback2(CB2_ReturnToField);
        DestroyTask(taskId);
    }
...
static void RecordQuestLogItemPurchase(void)
{
    ...
    if (v != 0)
        SetQuestLogEvent(v + 37 - 1, (const u16 *)&gShopMenuHistory[0]);

    v = gShopMenuHistory[1].unkA;
    if (v != 0)
        SetQuestLogEvent(v + 37 - 1, (const u16 *)&gShopMenuHistory[1]);
}
