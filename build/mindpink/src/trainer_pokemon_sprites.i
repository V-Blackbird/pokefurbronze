...
# 3 "src/trainer_pokemon_sprites.c" 2
...
void LoadPicPaletteByTagOrSlot(u16 species, u32 otId, u32 personality, u8 paletteSlot, u16 paletteTag, bool8 isTrainer)
{
    if (!isTrainer)
    {
        if (paletteTag == 0xFFFF)
        {
            ...
            LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(species, otId, personality), (0x100 + ((paletteSlot) * 16)), ((16) * sizeof(u16)));
        }
        else
        {
    ...
    else
    {
        ...
        {
            sCreatingSpriteTemplate.paletteTag = 0xFFFF;
            LoadCompressedPalette(gTrainerFrontPicPaletteTable[species].data, (0x100 + ((paletteSlot) * 16)), ((16) * sizeof(u16)));
        }
        else
        {
            sCreatingSpriteTemplate.paletteTag = paletteTag;
...
{
    if (!isTrainer)
        LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(species, otId, personality), (0x000 + ((paletteSlot) * 16)), ((16) * sizeof(u16)));
    else
        LoadCompressedPalette(gTrainerFrontPicPaletteTable[species].data, (0x000 + ((paletteSlot) * 16)), ((16) * sizeof(u16)));
}
...
{
    if (DecompressPic_HandleDeoxys(species, personality, isFrontPic, (u8 *)GetWindowAttribute(windowId, WINDOW_TILE_DATA), 0))
        return 0xFFFF;
...
{
    return CreateTrainerCardSprite(species, otId, personality, isFrontPic, 0, 0, paletteSlot, windowId, 0);
}
...
{
    return LoadPicSpriteInWindow(species, 0, 0, isFrontPic, paletteSlot, windowId, 1);
}
...
u16 PlayerGenderToFrontTrainerPicId(u8 gender, bool8 getClass)
{
    ...
    {
        if (gender != 0)
            return gFacilityClassToPicIndex[135];
        else
            return gFacilityClassToPicIndex[134];
    }
    return gender;
}
