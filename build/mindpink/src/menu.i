...
static __attribute__((section("ewram_data"))) u8 sTopBarWindowId = 0;
...
static void WindowFunc_DrawDialogFrameWithCustomTileAndPalette(u8 bg, u8 tilemapLeft, u8 tilemapTop, u8 width, u8 height, u8 paletteNum)
{
    ...
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 10)), tilemapLeft - 2, tilemapTop + 2, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 11)), tilemapLeft - 1, tilemapTop + 2, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 12)), tilemapLeft + width, tilemapTop + 2, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 13)), tilemapLeft + width + 1, tilemapTop + 2, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 5)), tilemapLeft - 2, tilemapTop + 3, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 6)), tilemapLeft - 1, tilemapTop + 3, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 8)), tilemapLeft + width, tilemapTop + 3, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 9)), tilemapLeft + width + 1, tilemapTop + 3, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum)), tilemapLeft - 2, tilemapTop + 4, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 1)), tilemapLeft - 1, tilemapTop + 4, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 2)), tilemapLeft, tilemapTop + 4, width, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 3)), tilemapLeft + width, tilemapTop + 4, 1, 1, sPaletteNum);
    FillBgTilemapBufferRect(bg, (0x800 + (sTileNum + 4)), tilemapLeft + width + 1, tilemapTop + 4, 1, 1, sPaletteNum);
}
...
u8 CreateTopBarWindowLoadPalette(u8 bg, u8 width, u8 yPos, u8 palette, u16 baseTile)
{
    ...
    LoadPalette(GetTextWindowPalette(2), palette, ((16) * sizeof(u16)));
...
void TopBarWindowPrintString(const u8 *string, u8 unused, bool8 copyToVram)
{
    ...
    if (sTopBarWindowId != 0xFF)
    {
        ...
        FillWindowPixelBuffer(sTopBarWindowId, ((15) | ((15) << 4)));
...
void DestroyTopBarWindow(void)
{
    if (sTopBarWindowId != 0xFF)
    {
        FillWindowPixelBuffer(sTopBarWindowId, ((0) | ((0) << 4)));
...
void MultichoiceList_PrintItems(u8 windowId, u8 fontId, u8 left, u8 top, u8 lineHeight, u8 itemCount, const struct MenuAction *strs, u8 letterSpacing, u8 lineSpacing)
{
    ...
    for (i = 0; i < itemCount; i++)
        AddTextPrinterParameterized5(windowId, fontId, strs[i].text, left, (lineHeight * i) + top, 0xFF, ((void *)0), letterSpacing, lineSpacing);
    CopyWindowToVram(windowId, COPYWIN_GFX);
}
...
void CreateYesNoMenu(const struct WindowTemplate *window, u8 fontId, u8 left, u8 top, u16 baseTileNum, u8 paletteNum, u8 initialCursorPos)
{
    ...
    AddTextPrinter(&textSubPrinter, 0xFF, ((void *)0));
...
static void MultichoiceGrid_RedrawCursor(u8 oldCursorPos, u8 newCursorPos)
{
    ...
    FillWindowPixelRect(sMenu.windowId, ((1) | ((1) << 4)), xPos, yPos, cursorWidth, cursorHeight);
...
static s8 Menu_ProcessGridInputRepeat_NoSoundLimit(void)
{
    ...
    else if (({(gMain.newAndRepeatedKeys) & (0x00F0);}) == 0x0040)
    {
        PlaySE(5);
        MultichoiceGrid_MoveCursor(0, -1);
        return -2;
    ...
    else if (({(gMain.newAndRepeatedKeys) & (0x00F0);}) == 0x0020 || GetLRKeysPressedAndHeld() == 1)
    {
        PlaySE(5);
        MultichoiceGrid_MoveCursor(-1, 0);
        return -2;
    }
    else if (({(gMain.newAndRepeatedKeys) & (0x00F0);}) == 0x0010 || GetLRKeysPressedAndHeld() == 2)
    {
        PlaySE(5);
        MultichoiceGrid_MoveCursor(1, 0);
        return -2;
