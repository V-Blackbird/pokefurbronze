# 0 "src/save_menu_util.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/save_menu_util.c"
...
void SaveStatToString(u8 gameStatId, u8 *dest0, u8 color)
{
    ...
    *dest++ = color + 1;
    ...
    case SAVE_STAT_BADGES:
        for (flagId = ((((0x4FF + 1) + 768 - 1) + 1) + 0x20), nBadges = 0; flagId < ((((0x4FF + 1) + 768 - 1) + 1) + 0x20) + 8; flagId++)
        {
            if (FlagGet(flagId))
                nBadges++;
        }
