...
{
    if (id >= (sizeof(flash->palettes) / sizeof((flash->palettes)[0])) || flash->palettes[id].available)
        return 0xFF;
...
{
    if (id >= (sizeof(flash->palettes) / sizeof((flash->palettes)[0])))
        return 0xFF;
...
void RouletteFlash_Run(struct RouletteFlashUtil *flash)
{
    ...
    if (flash->enabled)
    {
        for (i = 0; i < (sizeof(flash->palettes) / sizeof((flash->palettes)[0])); i++)
        {
            if ((flash->flags >> i) & 1)
            {
