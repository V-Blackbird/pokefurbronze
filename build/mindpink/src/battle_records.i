...
static __attribute__((section("ewram_data"))) u16 * sBg3TilemapBuffer_p = ((void *)0);
...
static void ResetGpu(void)
{
    ...
    u32 size = 0x18000;
    { void *_dest = dest; u32 _size = size; while (1) { { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(_dest); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | (((0x1000))/(16/8))); dmaRegs[2]; }; }; _dest += (0x1000); _size -= (0x1000); if (_size <= (0x1000)) { { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(_dest); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((_size)/(16/8))); dmaRegs[2]; }; }; break; } } };
    }

    {
    void *dest = (void *)0x7000000;
    u32 size = 0x400;
    { vu32 *_dest = (vu32 *)(dest); u32 _size = size; { vu32 tmp = (vu32)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(_dest); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0400 | 0x0100 | 0x0000) << 16 | ((_size)/(32/8))); dmaRegs[2]; }; }; };
    }

    {
    void *dest = (void *)0x5000000;
    u32 size = (0x200 + 0x200);
    { vu16 *_dest = (vu16 *)(dest); u32 _size = size; { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)(_dest); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | ((_size)/(16/8))); dmaRegs[2]; }; }; };
    }
...
static void EnableDisplay(void)
{
    SetGpuReg(0x0, 0x0000 | 0x0040 | 0x0100 | 0x0800);
}
...
{
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, record, 0x00000000 | 0x01000000 | ((sizeof(*record))/(16/8) & 0x1FFFFF)); };
    record->name[0] = 0xFF;
    record->trainerId = 0;
    record->wins = 0;
    record->losses = 0;
...
static s32 IndexOfOpponentLinkBattleRecord(struct LinkBattleRecords * records, const u8 * name, u16 trainerId)
{
    ...
    for (i = 0; i < 5; i++)
    {
        if (StringCompareN(records->entries[i].name, name, 7) == 0 && records->entries[i].trainerId == trainerId)
            return i;
    }
...
static void AddOpponentLinkBattleRecord(struct LinkBattleRecords * records, const u8 * name, u16 trainerId, s32 outcome, u32 language)
{
    ...
    {
        i = 5 - 1;
        record = &records->entries[5 - 1];
        ClearLinkBattleRecord(record);
        StringCopyN(record->name, namebuf, 7);
...
{
    if (gSaveBlock1Ptr->location.mapGroup != ((4 | (0 << 8)) >> 8) || gSaveBlock1Ptr->location.mapNum != ((4 | (0 << 8)) & 0xFF))
    {
...
{
    u32 nwins = GetGameStat(23);
    u32 nlosses = GetGameStat(24);
    u32 ndraws = GetGameStat(25);
    s32 i;
    s32 j;
    bool32 foundEnd;
    u8 * strvar;
    ...
    for (i = 0; i < (sizeof(sStringVars) / sizeof((sStringVars)[0])); i++)
    {
        strvar = sStringVars[i];
        foundEnd = 0;
        for (j = 0; j < 4; j++)
        {
            if (!foundEnd && *strvar == 0xFF)
                foundEnd = 1;
            if (foundEnd)
                *strvar = 0x00;
            strvar++;
...
static void PrintOpponentBattleRecord(struct LinkBattleRecord * record, u8 y)
{
    ...
    else
    {
        for (i = 0; i < 4; i++)
        {
            ...
            {
                x = 0;
                StringFillWithTerminator(gStringVar1, 7 + 1);
...
static void PrintBattleRecords(void)
{
    ...
    FillWindowPixelRect(0, ((0) | ((0) << 4)), 0, 0, 0xD8, 0x90);
...
static void LoadFrameGfxOnBg(u8 bg)
{
    ...
    LoadPalette(sPalette, (0x000 + ((0) * 16)), ((16) * sizeof(u16)));
}
