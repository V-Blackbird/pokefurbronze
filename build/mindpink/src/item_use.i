...
static __attribute__((section("ewram_data"))) void (*sItemUseOnFieldCB)(u8 taskId) = ((void *)0);
...
static void (*const sExitCallbackByItemType[])(void) = {
    ...
    [ITEM_TYPE_UNUSED - 1] = ((void *)0),
    [ITEM_TYPE_BAG_MENU - 1] = ((void *)0),
};

static void SetUpItemUseCallback(u8 taskId)
{
    u8 itemType;
    if (gSpecialVar_ItemId == 176)
...
void FieldUseFunc_Bike(u8 taskId)
{
    ...
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x30)) == 1
     ...
     || MetatileBehavior_IsIsolatedHorizontalRail(behavior) == 1)
        DisplayItemMessageInCurrentContext(taskId, gTasks[taskId].data[3], FONT_NORMAL, gText_CantDismountBike);
    else if (Overworld_IsBikingAllowed() == 1 && !IsBikingDisallowedByPlayer())
    {
...
static void ItemUseOnFieldCB_Bicycle(u8 taskId)
{
    if (!TestPlayerAvatarFlags((1 << PLAYER_AVATAR_STATE_MACH_BIKE) | (1 << PLAYER_AVATAR_STATE_ACRO_BIKE)))
        PlaySE(11);
    GetOnOffBike((1 << PLAYER_AVATAR_STATE_MACH_BIKE) | (1 << PLAYER_AVATAR_STATE_ACRO_BIKE));
    ClearPlayerHeldMovementAndUnfreezeObjectEvents();
    UnlockPlayerFieldControls();
    DestroyTask(taskId);
}
...
static bool8 CanFish(void)
{
    ...
    if (MetatileBehavior_IsWaterfall(behavior))
        return 0;
    if (TestPlayerAvatarFlags((1 << PLAYER_AVATAR_STATE_UNDERWATER)))
        return 0;
    ...
    else
    {
        if (MetatileBehavior_IsSurfable(behavior) && MapGridGetCollisionAt(x, y) == 0)
            return 1;
        if (MetatileBehavior_IsBridge(behavior) == 1)
            return 1;
    }
    return 0;
}
...
void FieldUseFunc_PokeFlute(u8 taskId)
{
    bool8 wokeSomeoneUp = 0;
    u8 i;

    for (i = 0; i < CalculatePlayerPartyCount(); i++)
    {
        if (!ExecuteTableBasedItemEffect(&gPlayerParty[i], 18, i, 0))
            wokeSomeoneUp = 1;
    }
...
static void InitTMCaseFromBag(void)
{
    InitTMCase(TMCASE_FIELD, CB2_BagMenuFromStartMenu, 0);
}
...
static void Task_InitTMCaseFromField(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        InitTMCase(TMCASE_FIELD, CB2_ReturnToField, 1);
        DestroyTask(taskId);
    }
}
...
static void Task_UseRepel(u8 taskId)
{
    ...
    {
        ItemUse_SetQuestLogEvent(4, ((void *)0), gSpecialVar_ItemId, 0xFFFF);
        VarSet(0x4020, ItemId_GetHoldEffectParam(gSpecialVar_ItemId));
        RemoveUsedItem();
        DisplayItemMessageInBag(taskId, FONT_NORMAL, gStringVar4, Task_ReturnToBagFromContextMenu);
    }
}
...
{
    ItemUse_SetQuestLogEvent(4, ((void *)0), gSpecialVar_ItemId, 0xFFFF);
    if (gSpecialVar_ItemId == 44)
    {
        FlagSet(((((0x4FF + 1) + 768 - 1) + 1) + 0x3));
        FlagClear(((((0x4FF + 1) + 768 - 1) + 1) + 0x4));
        CopyItemName(gSpecialVar_ItemId, gStringVar2);
        StringExpandPlaceholders(gStringVar4, gText_UsedVar2WildLured);
        gTasks[taskId].func = Task_UsedBlackWhiteFlute;
        gTasks[taskId].data[8] = 0;
    }
    else if (gSpecialVar_ItemId == 43)
    {
        FlagSet(((((0x4FF + 1) + 768 - 1) + 1) + 0x4));
        FlagClear(((((0x4FF + 1) + 768 - 1) + 1) + 0x3));
        CopyItemName(gSpecialVar_ItemId, gStringVar2);
...
void ItemUseOutOfBattle_EscapeRope(u8 taskId)
{
    if (CanUseEscapeRopeOnCurrMap() == 1)
    {
        ItemUse_SetQuestLogEvent(4, ((void *)0), gSpecialVar_ItemId, gMapHeader.regionMapSectionId);
...
void FieldUseFunc_VsSeeker(u8 taskId)
{
    if ((gMapHeader.mapType != 3
      && gMapHeader.mapType != 1
      && gMapHeader.mapType != 2)
     || (gSaveBlock1Ptr->location.mapGroup == ((0 | (1 << 8)) >> 8)
      && (gSaveBlock1Ptr->location.mapNum == ((0 | (1 << 8)) & 0xFF)
       || gSaveBlock1Ptr->location.mapNum == ((97 | (1 << 8)) & 0xFF)
       || gSaveBlock1Ptr->location.mapNum == ((109 | (1 << 8)) & 0xFF)
       || gSaveBlock1Ptr->location.mapNum == ((121 | (1 << 8)) & 0xFF))))
    {
        PrintNotTheTimeToUseThat(taskId, gTasks[taskId].data[3]);
    }
    else
...
void ItemUse_SetQuestLogEvent(u8 eventId, struct Pokemon *pokemon, u16 itemId, u16 param)
{
    ...
    if (pokemon != ((void *)0))
        questLog->species = GetMonData(pokemon, 65);
    else
