# 0 "src/special_field_anim.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/special_field_anim.c"
...
# 7 "src/special_field_anim.c" 2
# 1 "include/constants/metatile_labels.h" 1
# 8 "src/special_field_anim.c" 2

static __attribute__((section("ewram_data"))) u8 sEscalatorTaskId = 0;
...
static const u16 sEscalatorMetatiles_BottomNextRail[3] =
{
    0x308,
    0x30A,
    0x2D0
...
static const u16 sEscalatorMetatiles_BottomRail[3] =
{
    0x309,
    0x30B,
    0x2D1
...
static const u16 sEscalatorMetatiles_BottomNext[3] =
{
    0x310,
    0x312,
    0x2D8
...
static const u16 sEscalatorMetatiles_Bottom[3] =
{
    0x311,
    0x313,
    0x2D9
...
static const u16 sEscalatorMetatiles_TopNext[3] =
{
    0x2E3,
    0x316,
    0x314
...
static const u16 sEscalatorMetatiles_Top[3] =
{
    0x2E4,
    0x317,
    0x315
...
static const u16 sEscalatorMetatiles_TopNextRail[3] =
{
    0x2EB,
    0x31E,
    0x31C
};
# 75 "src/special_field_anim.c"
static void SetEscalatorMetatile(u8 taskId, const s16 *metatileIds, u16 metatileMasks)
{
    s16 x, y, transitionStage;
    s16 i, j;

    x = gTasks[taskId].data[4] - 1;
    y = gTasks[taskId].data[5] - 1;
    transitionStage = gTasks[taskId].data[1];
    ...
    else
    {
        for (i = 0; i < 3; i++)
        {
            for (j = 0; j < 3; j++)
            {
                ...
                if (metatileIds[(3 - 1) - transitionStage] == (s16)id)
                {
                    if (transitionStage != (3 - 1))
...
static void Task_DrawEscalator(u8 taskId)
{
    ...
    case 1:
        ...
        break;
    case 2:
        SetEscalatorMetatile(taskId, sEscalatorMetatiles_BottomNext, 0x0C00);
        break;
    case 3:
        SetEscalatorMetatile(taskId, sEscalatorMetatiles_Bottom, 0);
        break;
    case 4:
        SetEscalatorMetatile(taskId, sEscalatorMetatiles_TopNext, 0x0C00);
        break;
    case 5:
...
{
    if (gTasks[sEscalatorTaskId].data[3] == 0)
    {
        if (gTasks[sEscalatorTaskId].data[1] != (3 - 1))
            return 1;
        return 0;
    }
    else
...
static void Task_DrawTeleporterHousing(u8 taskId)
{
    ...
    if (data[0] == 0)
    {
        ...
        if ((data[1] & 1) == 0)
        {
            MapGridSetMetatileIdAt(data[2], data[3], 0x2B5 | 0x0C00);
            MapGridSetMetatileIdAt(data[2], data[3] + 2, 0x2B7 | 0x0C00);
        }
        else
        {
            MapGridSetMetatileIdAt(data[2], data[3], 0x2B6 | 0x0C00);
            MapGridSetMetatileIdAt(data[2], data[3] + 2, 0x2B8 | 0x0C00);
        }
    ...
    MapGridSetMetatileIdAt(data[2], data[3], 0x28A | 0x0C00);
    MapGridSetMetatileIdAt(data[2], data[3] + 2, 0x296 | 0x0C00);
    CurrentMapDrawMetatileAt(data[2], data[3]);
    CurrentMapDrawMetatileAt(data[2], data[3] + 2);
    DestroyTask(taskId);
}
...
static void Task_DrawTeleporterCable(u8 taskId)
{
    ...
    if (data[0] == 0)
    {
        if (data[1] != 0)
        {
            ...
            MapGridSetMetatileIdAt(data[2], data[3], 0x285 | 0x0C00);
            MapGridSetMetatileIdAt(data[2], data[3] + 1, 0x2B4 | 0x0C00);
            CurrentMapDrawMetatileAt(data[2], data[3]);
        ...
        MapGridSetMetatileIdAt(data[2], data[3], 0x2B9 | 0x0C00);
        MapGridSetMetatileIdAt(data[2], data[3] + 1, 0x2BA | 0x0C00);
        CurrentMapDrawMetatileAt(data[2], data[3]);
        CurrentMapDrawMetatileAt(data[2], data[3] + 1);
    }
