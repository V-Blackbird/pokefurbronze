# 0 "src/pokemon_storage_system_menu.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokemon_storage_system_menu.c"
...
# 2 "src/pokemon_storage_system_menu.c" 2
# 1 "include/gflib.h" 1
...
# 3 "src/pokemon_storage_system_menu.c" 2
...
void ClearTempFieldEventData(void);
# 4 "src/pokemon_storage_system_menu.c" 2
...
# 5 "src/pokemon_storage_system_menu.c" 2
...
# 6 "src/pokemon_storage_system_menu.c" 2
...
# 7 "src/pokemon_storage_system_menu.c" 2
# 1 "include/menu.h" 1
# 18 "include/menu.h"
struct MenuAction
{
...
# 8 "src/pokemon_storage_system_menu.c" 2
...
# 9 "src/pokemon_storage_system_menu.c" 2
...
# 10 "src/pokemon_storage_system_menu.c" 2
...
# 11 "src/pokemon_storage_system_menu.c" 2
...
# 12 "src/pokemon_storage_system_menu.c" 2
...
extern const u8 gText_PokeSum_EggOrigin_Trade[];
# 13 "src/pokemon_storage_system_menu.c" 2
# 1 "include/task.h" 1
# 14 "src/pokemon_storage_system_menu.c" 2
# 1 "include/constants/songs.h" 1
# 15 "src/pokemon_storage_system_menu.c" 2
# 1 "include/constants/field_weather.h" 1
# 16 "src/pokemon_storage_system_menu.c" 2
# 1 "include/constants/help_system.h" 1
# 17 "src/pokemon_storage_system_menu.c" 2

static __attribute__((section("ewram_data"))) u8 sPreviousBoxOption = 0;
static __attribute__((section("ewram_data"))) struct ChooseBoxMenu *sChooseBoxMenu = ((void *)0);
...
static void PrintStringToBufferCopyNow(const u8 *string, void *dst, u16 offset, u8 bgColor, u8 fgColor, u8 shadowColor, u8 *unused)
{
    ...
    CpuSet(tileData2, dst + offset, 0x00000000 | ((var)/(16/8) & 0x1FFFFF));
    RemoveWindow(windowId);
}
...
static void Task_PCMainMenu(u8 taskId)
{
    ...
    case STATE_LOAD:
        SetHelpContext(28);
        CreatePCMainMenu(task->data[1], &task->data[15]);
        LoadStdWindowFrameGfx();
        DrawDialogueFrame(0, 0);
        FillWindowPixelBuffer(0, ((1) | ((1) << 4)));
        AddTextPrinterParameterized2(0, FONT_NORMAL, sMainMenuTexts[task->data[1]].desc, 0xFF, ((void *)0), 0x2, 0x1, 0x3);
    ...
    case STATE_HANDLE_INPUT:
        ...
        case -2:
            ...
            if (({(gMain.newKeys) & (0x0080);}) && ++task->data[3] > OPTIONS_COUNT - 1)
                task->data[3] = 0;
            ...
            if (task->data[1] != task->data[3])
            {
                ...
                FillWindowPixelBuffer(0, ((1) | ((1) << 4)));
                AddTextPrinterParameterized2(0, FONT_NORMAL, sMainMenuTexts[task->data[1]].desc, 0, ((void *)0), 0x2, 0x1, 0x3);
            }
            break;
        case -1:
        case OPTION_EXIT:
            ...
            break;
        default:
            if (task->data[2] == OPTION_WITHDRAW && CountPartyMons() == 6)
            {

                FillWindowPixelBuffer(0, ((1) | ((1) << 4)));
                AddTextPrinterParameterized2(0, FONT_NORMAL, gText_PartyFull, 0, ((void *)0), 0x2, 0x1, 0x3);
            ...
            else if (task->data[2] == OPTION_DEPOSIT && CountPartyMons() == 1)
            {
                ...
                FillWindowPixelBuffer(0, ((1) | ((1) << 4)));
                AddTextPrinterParameterized2(0, FONT_NORMAL, gText_JustOnePkmn, 0, ((void *)0), 0x2, 0x1, 0x3);
                task->data[0] = STATE_ERROR_MSG;
            }
            else
            {
...
void ShowPokemonStorageSystemPC(void)
{
    ...
    gTasks[taskId].data[0] = STATE_LOAD;
...
static void CreatePCMainMenu(u8 whichMenu, s16 *windowIdPtr)
{
    ...
    PrintTextArray(windowId, FONT_NORMAL, GetMenuCursorDimensionByFont(FONT_NORMAL, 0), 2, 16, (sizeof(sMainMenuTexts) / sizeof((sMainMenuTexts)[0])), (void *)sMainMenuTexts);
    Menu_InitCursor(windowId, FONT_NORMAL, 0, 2, 16, (sizeof(sMainMenuTexts) / sizeof((sMainMenuTexts)[0])), whichMenu);
...
void ResetPokemonStorageSystem(void)
{
    ...
    for (boxId = 0; boxId < 14; boxId++)
        SetBoxWallpaper(boxId, boxId % (WALLPAPER_SAVANNA + 1));
}
...
static const union AffineAnimCmd sAffineAnim_ChooseBoxMenu[] = {
    {.frame = {.xScale = 224, .yScale = 224, .rotation = 0, .duration = 0}},
    {.type = 0x7FFF}
};
...
static void ChooseBoxMenu_CreateSprites(u8 curBox)
{
    ...
    for (i = 0; i < (sizeof(sChooseBoxMenu->menuCornerSprites) / sizeof((sChooseBoxMenu->menuCornerSprites)[0])); i++)
    {
        ...
        if (i & 2)
        {
            sChooseBoxMenu->menuCornerSprites[i]->x = 196;
            animNum = 2;
        }
        if (i & 1)
        {
            sChooseBoxMenu->menuCornerSprites[i]->y = 112;
            sChooseBoxMenu->menuCornerSprites[i]->oam.size = ((((0 << 2) | (2)) >> 2) & 0x03);
            animNum++;
        }
        StartSpriteAnim(sChooseBoxMenu->menuCornerSprites[i], animNum);
    }
    for (i = 0; i < (sizeof(sChooseBoxMenu->arrowSprites) / sizeof((sChooseBoxMenu->arrowSprites)[0])); i++)
    {
...
static void ChooseBoxMenu_DestroySprites(void)
{
    ...
    if (sChooseBoxMenu->menuSprite)
    {
        ...
        sChooseBoxMenu->menuSprite = ((void *)0);
    }
    for (i = 0; i < (sizeof(sChooseBoxMenu->menuCornerSprites) / sizeof((sChooseBoxMenu->menuCornerSprites)[0])); i++)
    {
        if (sChooseBoxMenu->menuCornerSprites[i])
        {
            ...
            sChooseBoxMenu->menuCornerSprites[i] = ((void *)0);
        }
    }
    for (i = 0; i < (sizeof(sChooseBoxMenu->arrowSprites) / sizeof((sChooseBoxMenu->arrowSprites)[0])); i++)
    {
...
static void ChooseBoxMenu_PrintBoxNameAndCount(void)
{
    ...
    while (boxName < sChooseBoxMenu->strbuf + 8)
        *boxName++ = 0x00;
    *boxName = 0xFF;
...
{
    void *dst = (void *)((void *)(0x6000000 + 0x10000) + GetSpriteTileStartByTag(sChooseBoxMenu->tileTag) * 32 + 256 * y + 32 * x);
    PrintStringToBufferCopyNow(str, dst, 0x100, 0x4, 0xF, 0xE, sChooseBoxMenu->buffer);
}
