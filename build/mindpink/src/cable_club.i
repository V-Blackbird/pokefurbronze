...
static void CreateLinkupTask(u8 minPlayers, u8 maxPlayers)
{
    ...
    if (FindTaskIdByFunc(Task_LinkupStart) == 0xFF)
    {
        ...
        gTasks[taskId].data[1] = minPlayers;
        gTasks[taskId].data[2] = maxPlayers;
    }
}
...
static void PrintNumPlayersInLink(u16 windowId, s32 numPlayers)
{
    ...
    AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar4, 0, 0, 0xFF, ((void *)0));
...
{
    if (({(gMain.newKeys) & (0x0002);}) && !IsLinkConnectionEstablished())
    {
        gLinkType = 0;
...
static void Task_LinkupAwaitConfirmation(u8 taskId)
{
    ...
    if (({(gMain.newKeys) & (0x0001);}) && linkPlayerCount >= data[1])
    {
...
static void Task_LinkupCheckStatusAfterConfirm(u8 taskId)
{
    ...
    {
        if (AnyConnectedPartnersPlayingRS() == 1)
            CloseLink();
        else
            SetCloseLinkCallback();
        HideFieldMessageBox();
...
static void Task_StartWiredCableClubBattle(u8 taskId)
{
    ...
    case 5:
        if (gLinkPlayers[0].trainerId & 1)
            PlayMapChosenOrBattleBGM(265);
        else
            PlayMapChosenOrBattleBGM(266);
        switch (gSpecialVar_0x8004)
        {
        case 1:
            gBattleTypeFlags = (1 << 3) | (1 << 1);
            break;
        case 2:
            gBattleTypeFlags = (1 << 3) | (1 << 1) | (1 << 0);
            break;
        case 5:
            ReducePlayerPartyToThree();
            gBattleTypeFlags = (1 << 3) | (1 << 1) | (1 << 0) | (1 << 6);
            break;
        }
