...
static void AnimTask_EruptionLaunchRocks_Step(u8 taskId)
{
    ...
    {
    case 0:
        BattleAnimHelper_SetSpriteSquashParams(task, task->data[15], 0x100, 0x100, 0xE0, 0x200, 32);
        task->data[0]++;
    case 1:
        if (++task->data[1] > 1)
        {
    ...
    case 2:
        ...
        {
            if (task->data[5] != 0)
                BattleAnimHelper_SetSpriteSquashParams(task, task->data[15], 0xE0, 0x200, 0x180, 0xF0, 6);
            else
                BattleAnimHelper_SetSpriteSquashParams(task, task->data[15], 0xE0, 0x200, 0x180, 0xC0, 6);
    ...
    case 4:
        ...
        {
            if (task->data[5] != 0)
                BattleAnimHelper_SetSpriteSquashParams(task, task->data[15], 0x180, 0xF0, 0x100, 0x100, 8);
            else
                BattleAnimHelper_SetSpriteSquashParams(task, task->data[15], 0x180, 0xC0, 0x100, 0x100, 8);
...
static void CreateEruptionLaunchRocks(u8 spriteId, u8 taskId, u8 activeSpritesIdx)
{
    ...
    for (i = 0, j = 0; i <= 6; i++)
    {
        ...
        if (spriteId != 64)
        {
            ...
            gSprites[spriteId].data[7] = activeSpritesIdx;
...
static void UpdateEruptionLaunchRockPos(struct Sprite *sprite)
{
    ...
    if (++sprite->data[0] > 2)
    {
        sprite->data[0] = 0;
        ++sprite->data[1];
        extraLaunchSpeed = (u16)sprite->data[1] * (u16)sprite->data[1];
    ...
    if (sprite->x < -8 || sprite->x > 240 + 8 || sprite->y < -8 || sprite->y > 120)
        sprite->invisible = 1;
}
# 994 "src/battle_anim_fire.c"
static void AnimEruptionFallingRock(struct Sprite *sprite)
{
    sprite->x = gBattleAnimArgs[0];
    sprite->y = gBattleAnimArgs[1];
...
static void AnimWillOWispOrb_Step(struct Sprite *sprite)
{
    ...
    if (!AnimTranslateLinear(sprite))
    {
        ...
        if ((initialData5 == 0 || initialData5 > 196) && newData5 > 0 && sprite->data[7] == 0)
            PlaySE12WithPanning(137, gAnimCustomPanning);
    }
    else
    {
        DestroyAnimSprite(sprite);
...
void AnimTask_BlendBackground(u8 taskId)
{
    ...
    BlendPalette((0x000 + ((animBg.paletteId) * 16)), 16, gBattleAnimArgs[0], gBattleAnimArgs[1]);
    DestroyAnimVisualTask(taskId);
}
# 1254 "src/battle_anim_fire.c"
