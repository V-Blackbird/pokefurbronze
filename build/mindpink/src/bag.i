...
static __attribute__((section("ewram_data"))) u8 sOpenWindows[11] = {};
...
void InitBagWindows(void)
{
    ...
    LoadUserWindowGfx(0, 0x64, (0x000 + ((14) * 16)));
    LoadMenuMessageWindowGfx(0, 0x6D, (0x000 + ((13) * 16)));
    LoadStdWindowGfx(0, 0x81, (0x000 + ((12) * 16)));
    LoadPalette(sBagWindowPalF, (0x000 + ((15) * 16)), sizeof(sBagWindowPalF));
    for (i = 0; i < 3; i++)
    {
...
{
    u32 x;
    DrawStdFrameWithCustomTileAndPalette(2, 0, 0x081, 12);
    x = 0x40 - GetStringWidth(FONT_SMALL, gText_DepositItem, 0);
    AddTextPrinterParameterized(2, FONT_SMALL, gText_DepositItem, x / 2, 1, 0, ((void *)0));
}
...
u8 ShowBagWindow(u8 whichWindow, u8 nItems)
{
    if (sOpenWindows[whichWindow] == 0xFF)
    {
        ...
        if (whichWindow != 6)
        {
            DrawStdFrameWithCustomTileAndPalette(sOpenWindows[whichWindow], 0, 0x064, 14);
        }
        else
        {
            DrawStdFrameWithCustomTileAndPalette(sOpenWindows[whichWindow], 0, 0x081, 12);
        }
        ScheduleBgCopyTilemapToVram(0);
    }
...
{
    ClearStdWindowAndFrameToTransparent(sOpenWindows[whichWindow], 0);
...
void CloseBagWindow(u8 whichWindow)
{
    if (sOpenWindows[whichWindow] != 0xFF)
    {
        ClearDialogWindowAndFrameToTransparent(sOpenWindows[whichWindow], 0);
        ClearWindowTilemap(sOpenWindows[whichWindow]);
        RemoveWindow(sOpenWindows[whichWindow]);
        PutWindowTilemap(1);
        ScheduleBgCopyTilemapToVram(0);
