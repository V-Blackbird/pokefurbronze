...
static __attribute__((section("ewram_data"))) struct ObjectEvent * sPlayerObjectPtr = ((void *)0);
static __attribute__((section("ewram_data"))) u8 sTeleportSavedFacingDirection = 0;
...
static bool8 TryInterruptObjectEventSpecialAnim(struct ObjectEvent *playerObjEvent, u8 direction)
{
        ...
        && !ObjectEventClearHeldMovementIfFinished(playerObjEvent))
    {
        ...
        if (heldMovementActionId > 0x20 && heldMovementActionId < 0x25)
        {
            if (direction != 0 && playerObjEvent->movementDirection != direction)
            {
...
static void npc_clear_strange_bits(struct ObjectEvent *objEvent)
{
    ...
    gPlayerAvatar.flags &= ~(1 << PLAYER_AVATAR_STATE_DASH);
}
...
static bool8 TryUpdatePlayerSpinDirection(void)
{
    if ((gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_FORCED)) && MetatileBehavior_IsSpinTile(gPlayerAvatar.lastSpinTile))
    {
...
static bool8 TryDoMetatileBehaviorForcedMovement(void)
{
    int i;
    u8 behavior;
    if (!(gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_CONTROLLABLE)))
    {
        behavior = gObjectEvents[gPlayerAvatar.objectEventId].currentMetatileBehavior;
        for (i = 0; sForcedMovementFuncs[i].check != ((void *)0); i++)
        {
...
static bool8 ForcedMovement_None(void)
{
    if (gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_FORCED))
    {
        ...
        gPlayerAvatar.flags &= ~(1 << PLAYER_AVATAR_STATE_FORCED);
    }
    return 0;
}
...
static u8 DoForcedMovement(u8 direction, MovementAction movementAction)
{
    ...
    playerAvatar->flags |= (1 << PLAYER_AVATAR_STATE_FORCED);
    if (collision)
    {
        ForcedMovement_None();
...
static bool8 ForcedMovement_WalkSouth(void)
{
    return DoForcedMovement(1, PlayerWalkNormal);
}

static bool8 ForcedMovement_WalkNorth(void)
{
    return DoForcedMovement(2, PlayerWalkNormal);
}

static bool8 ForcedMovement_WalkWest(void)
{
    return DoForcedMovement(3, PlayerWalkNormal);
}

static bool8 ForcedMovement_WalkEast(void)
{
    return DoForcedMovement(4, PlayerWalkNormal);
}

static bool8 ForcedMovement_SpinRight(void)
{
    PlaySpinSound();
    return DoForcedMovement(4, PlayerGoSpin);
}

static bool8 ForcedMovement_SpinLeft(void)
{
    PlaySpinSound();
    return DoForcedMovement(3, PlayerGoSpin);
}

static bool8 ForcedMovement_SpinUp(void)
{
    PlaySpinSound();
    return DoForcedMovement(2, PlayerGoSpin);
}

static bool8 ForcedMovement_SpinDown(void)
{
    PlaySpinSound();
    return DoForcedMovement(1, PlayerGoSpin);
}
...
static bool8 ForcedMovement_SlideSouth(void)
{
    return ForcedMovement_Slide(1, PlayerWalkFast);
}

static bool8 ForcedMovement_SlideNorth(void)
{
    return ForcedMovement_Slide(2, PlayerWalkFast);
}

static bool8 ForcedMovement_SlideWest(void)
{
    return ForcedMovement_Slide(3, PlayerWalkFast);
}

static bool8 ForcedMovement_SlideEast(void)
{
    return ForcedMovement_Slide(4, PlayerWalkFast);
}
...
static void PlayerNotOnBikeMoving(u8 direction, u16 heldKeys)
{
    ...
    if ((heldKeys & 0x0002) && FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x2F))
...
static bool8 ShouldJumpLedge(s16 x, s16 y, u8 direction)
{
    if (GetLedgeJumpDirection(x, y, direction) != 0)
        return 1;
    else
        return 0;
}
...
static bool8 TryPushBoulder(s16 x, s16 y, u8 direction)
{
    ...
    if (gObjectEvents[objectEventId].graphicsId != 97)
        return 0;

    x = gObjectEvents[objectEventId].currentCoords.x;
    y = gObjectEvents[objectEventId].currentCoords.y;
    MoveCoords(direction_, &x, &y);
    if (MapGridGetMetatileBehaviorAt(x, y) == 0x66 || (GetCollisionAtCoords(&gObjectEvents[objectEventId], x, y, direction_) == COLLISION_NONE && !MetatileBehavior_IsNonAnimDoor(MapGridGetMetatileBehaviorAt(x, y))))
    {
...
static void CheckAcroBikeCollision(s16 x, s16 y, u8 metatileBehavior, u8 *collision)
{
    ...
    for (i = 0; i < (sizeof(sAcroBikeTrickMetatiles) / sizeof((sAcroBikeTrickMetatiles)[0])); i++)
    {
...
static void DoPlayerAvatarTransition(void)
{
    ...
    if (flags != 0)
    {
        for (i = 0; i < (sizeof(sPlayerAvatarTransitionFuncs) / sizeof((sPlayerAvatarTransitionFuncs)[0])); i++, flags >>= 1)
        {
            if (flags & 1)
...
{
    QuestLogTryRecordPlayerAvatarGfxTransition(0);
    QuestLogCallUpdatePlayerSprite(0);
}

static void PlayerAvatarTransition_Bike(struct ObjectEvent * playerObjEvent)
{
    QuestLogTryRecordPlayerAvatarGfxTransition(1);
    QuestLogCallUpdatePlayerSprite(1);
    BikeClearState(0, 0);
}

static void PlayerAvatarTransition_Surfing(struct ObjectEvent * playerObjEvent)
{
    QuestLogTryRecordPlayerAvatarGfxTransition(3);
    QuestLogCallUpdatePlayerSprite(3);
}
...
static bool8 PlayerAnimIsMultiFrameStationary(void)
{
    ...
    if (movementActionId <= 0x7
        || (movementActionId >= 0x18 && movementActionId <= 0x1C)
        || (movementActionId >= 0x21 && movementActionId <= 0x30)
        || (movementActionId >= 0x70 && movementActionId <= 0x7B)
        || (movementActionId >= 0x88 && movementActionId <= 0x8B))
        return 1;
    else
        return 0;
}
...
static const u8 sPlayerAvatarGfxIds[][2] = {
    [PLAYER_AVATAR_GFX_NORMAL] = {0, 7},
    [PLAYER_AVATAR_GFX_BIKE] = {1, 8},
    [PLAYER_AVATAR_GFX_RIDE] = {2, 9},
    [PLAYER_AVATAR_GFX_FIELD_MOVE] = {3, 10},
    [PLAYER_AVATAR_GFX_FISH] = {4, 11},
    [PLAYER_AVATAR_GFX_VSSEEKER] = {5, 12},
};
...
bool8 IsPlayerFacingSurfableFishableWater(void)
{
    ...
    if (GetCollisionAtCoords(playerObjEvent, x, y, playerObjEvent->facingDirection) == COLLISION_ELEVATION_MISMATCH
        ...
        && MetatileAtCoordsIsWaterTile(x, y) == 1)
        return 1;
    else
        return 0;
}
...
{
    gPlayerAvatar.flags &= ((1 << PLAYER_AVATAR_STATE_DASH) | (1 << PLAYER_AVATAR_STATE_FORCED) | (1 << PLAYER_AVATAR_STATE_CONTROLLABLE));
    gPlayerAvatar.flags |= flags;
}

static const u8 sPlayerAvatarGfxToStateFlag[][3][2] = {
    [0] = {
        {0, (1 << PLAYER_AVATAR_STATE_NORMAL)},
        {1, (1 << PLAYER_AVATAR_STATE_MACH_BIKE)},
        {2, (1 << PLAYER_AVATAR_STATE_SURFING)},
    },
    [1] = {
        {7, (1 << PLAYER_AVATAR_STATE_NORMAL)},
        {8, (1 << PLAYER_AVATAR_STATE_MACH_BIKE)},
        {9, (1 << PLAYER_AVATAR_STATE_SURFING)},
    }
};
...
u8 GetPlayerAvatarStateTransitionByGraphicsId(u8 graphicsId, u8 gender)
{
    ...
    for (i = 0; i < (sizeof(*sPlayerAvatarGfxToStateFlag) / sizeof((*sPlayerAvatarGfxToStateFlag)[0])); i++)
    {
...
void InitPlayerAvatar(s16 x, s16 y, u8 direction, u8 gender)
{
    ...
    playerObjEventTemplate.objUnion.normal.movementType = 0xB;
    ...
    SetPlayerAvatarStateMask((1 << PLAYER_AVATAR_STATE_CONTROLLABLE) | (1 << PLAYER_AVATAR_STATE_NORMAL));
...
{
    QuestLogCallUpdatePlayerSprite(2);
}
...
static bool8 PlayerAvatar_DoSecretBaseMatJump(struct Task *task, struct ObjectEvent *objectEvent)
{
    ...
    if (ObjectEventClearHeldMovementIfFinished(objectEvent))
    {
        ...
        {
            gPlayerAvatar.preventStep = 0;
            gPlayerAvatar.transitionFlags |= (1 << PLAYER_AVATAR_STATE_CONTROLLABLE);
...
void SeafoamIslandsB4F_CurrentDumpsPlayerOnLand(void)
{
    ...
    {
        QuestLogRecordPlayerAvatarGfxTransitionWithDuration(sQuestLogSurfDismountActionIds[2], 16);
        CreateStopSurfingTask(2);
    }
}
...
# 1685 "src/field_player_avatar.c"
void StartFishing(u8 rod)
{
    ...
    Task_Fishing(taskId);
    if (QuestLogTryRecordPlayerAvatarGfxTransition(2) == 1)
...
static bool8 Fishing5(struct Task *task)
{
    ...
    if (task->data[1] >= 20)
    {
        ...
        {
            AddTextPrinterParameterized(0, FONT_NORMAL, dot, task->data[2] * 12, 1, 0, ((void *)0));
            task->data[2]++;
        }
    }
    return 0;
...
static bool8 Fishing10(struct Task *task)
{
    ...
    AddTextPrinterParameterized2(0, FONT_NORMAL, gText_PokemonOnHook, 1, 0, 0x2, 0x1, 0x3);
    task->data[0]++;
    task->data[1] = 0;
    return 0;
}
...
static bool8 Fishing12(struct Task *task)
{
    ...
    FillWindowPixelBuffer(0, ((1) | ((1) << 4)));
    AddTextPrinterParameterized2(0, FONT_NORMAL, gText_NotEvenANibble, 1, ((void *)0), 0x2, 0x1, 0x3);
    task->data[0] = 13;
    return 1;
}
...
static bool8 Fishing13(struct Task *task)
{
    AlignFishingAnimationFrames(&gSprites[gPlayerAvatar.spriteId]);
    StartSpriteAnim(&gSprites[gPlayerAvatar.spriteId], GetFishingNoCatchDirectionAnimNum(GetPlayerFacingDirection()));
    AddTextPrinterParameterized2(0, FONT_NORMAL, gText_ItGotAway, 1, ((void *)0), 0x2, 0x1, 0x3);
    task->data[0]++;
    return 1;
}
...
# 2004 "src/field_player_avatar.c"
...
{
    if (sTeleportSavedFacingDirection == 0)
        return 1;
    else
...
static void Task_TeleportWarpOutPlayerAnim(u8 taskId)
{
    ...
    case 0:
        ...
        data[1] = 0;
        data[2] = 1;
        data[3] = (u16)(sprite->y + sprite->y2) * 16;
        sprite->y2 = 0;
        CameraObjectReset2();
        ...
        data[0]++;
    case 1:
        TeleportAnim_RotatePlayer(object, &data[1]);
        data[3] -= data[2];
        data[2] += 3;
        sprite->y = data[3] >> 4;
...
static void Task_TeleportWarpInPlayerAnim(u8 taskId)
{
    ...
    {
    case 0:
        data[5] = GetTeleportSavedFacingDirection();
        ObjectEventForceSetHeldMovement(object, GetFaceDirectionMovementAction(sTeleportFacingDirectionSequence[data[5]]));
        data[1] = 0;
        data[2] = 116;
        data[4] = sprite->y;
        data[6] = sprite->oam.priority;
        data[7] = sprite->subpriority;
        data[3] = -((u16)sprite->y2 + 32) * 16;
        sprite->y2 = 0;
        CameraObjectReset2();
        object->fixedPriority = 1;
    ...
    case 2:
        ...
        break;
    case 3:
        if (data[5] == TeleportAnim_RotatePlayer(object, &data[1]))
        {
