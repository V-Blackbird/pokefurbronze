# 0 "src/help_system_util.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/help_system_util.c"
...
# 7 "src/help_system_util.c" 2
# 1 "include/list_menu.h" 1
# 8 "src/help_system_util.c" 2
...
extern const u8 gText_PokeSum_EggOrigin_Trade[];
# 9 "src/help_system_util.c" 2
# 1 "include/constants/songs.h" 1
# 10 "src/help_system_util.c" 2
...
static __attribute__((section("ewram_data"))) u8 sMapTilesBackup[0x4000] = {0};
__attribute__((section("ewram_data"))) u8 gDisableHelpSystemVolumeReduce = 0;
__attribute__((section("ewram_data"))) bool8 gHelpSystemToggleWithRButtonDisabled = 0;
static __attribute__((section("ewram_data"))) u8 sDelayTimer = 0;
static __attribute__((section("ewram_data"))) u8 sInHelpSystem = 0;
static __attribute__((section("ewram_data"))) struct HelpSystemVideoState sVideoState = {0};
__attribute__((section("ewram_data"))) struct HelpSystemListMenu gHelpSystemListMenu = {0};
__attribute__((section("ewram_data"))) struct ListMenuItem gHelpSystemListMenuItems[52] = {0};
...
u8 RunHelpSystemCallback(void)
{
    ...
    case 0:
        sInHelpSystem = 0;
        if (gSaveBlock2Ptr->optionsButtonMode != 0)
            return 0;
        if (({(gMain.newKeys) & (0x0100);}) && gHelpSystemToggleWithRButtonDisabled == 1)
            return 0;
    ...
    case 1:
        ...
        SaveMapGPURegs();
        SaveMapTextColors();
        (*(vu16 *)0x5000000) = sPals[15];
        SetGpuReg(0x0, 0);
        sVideoState.state = 2;
        break;
    case 2:
        RequestDma3Fill(0, (void *)(void *)(0x6000000 + (0x4000 * (3))), 0x4000, 0);
        RequestDma3Copy(sPals, (void *)0x5000000, sizeof(sPals), 0);
    ...
    case 3:
        ...
        if (HelpSystem_UpdateHasntSeenIntro() == 1)
    ...
    case 4:
        ...
        SetGpuReg(0x10, 0);
        SetGpuReg(0x12, 0);
        SetGpuReg(0x8, (0) | ((3) << 2) | 0x0000 | ((31) << 8));
        SetGpuReg(0x0, 0x0100);
        sVideoState.state = 5;
        break;
    case 5:
    ...
    case 6:
        ...
        for (i = 0; i < 0x200; i += 2)
        {
            *((vu16 *)(0x5000000 + 0x000 + i)) = sPals[15];
            *((vu16 *)(0x5000000 + 0x200 + i)) = sPals[15];
        }
        sVideoState.state = 7;
        break;
    case 7:
...
void SaveCallbacks(void)
{
    ...
    dma = (void *)(0x4000000 + 0xb0);
    dma[5] &= ~(0x3000 | 0x0800 | 0x0200);
    dma[5] &= ~0x8000;
    dma[5];
}
...
void SaveMapGPURegs(void)
{
    sVideoState.savedDispCnt = GetGpuReg(0x0);
    sVideoState.savedBg0Cnt = GetGpuReg(0x8);
    sVideoState.savedBg0Hofs = GetGpuReg(0x10);
    sVideoState.savedBg0Vofs = GetGpuReg(0x12);
    sVideoState.savedBldCnt = GetGpuReg(0x50);
}

void SaveMapTiles(void)
{
    RequestDma3Copy((void *)(void *)(0x6000000 + (0x4000 * (3))), sMapTilesBackup, 0x4000, 0);
}
...
void RestoreGPURegs(void)
{
    SetGpuReg(0x50, sVideoState.savedBldCnt);
    SetGpuReg(0x10, sVideoState.savedBg0Hofs);
    SetGpuReg(0x12, sVideoState.savedBg0Vofs);
    SetGpuReg(0x8, sVideoState.savedBg0Cnt);
    SetGpuReg(0x0, sVideoState.savedDispCnt);
}

void RestoreMapTiles(void)
{
    RequestDma3Copy(sMapTilesBackup, (void *)(void *)(0x6000000 + (0x4000 * (3))), 0x4000, 0);
}
...
void CommitTilemap(void)
{
    RequestDma3Copy(gDecompressionBuffer, (void *)(void *)(0x6000000 + (0x4000 * (3))), 0x4000, 0);
}
    ...
    while (1)
    {
        ...
        case 0xFD:
            ...
            if (curChar == 0x1)
            {
                for (i = 0; i < 10; i++)
                {
                    if (gSaveBlock2Ptr->playerName[i] == 0xFF)
                    {
                        break;
                    }
            ...
            else if (curChar == 0x2)
            {
                for (i = 0; ; i++)
                {
                    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x34)) == 1)
                    {
                        if (gString_Bill[i] == 0xFF)
                        {
                            break;
                        }
...
{
    if (fontId == FONT_SMALL)
        DecompressGlyph_Small(glyph, 0);
    else if (fontId == FONT_FEMALE)
        DecompressGlyph_Female(glyph, 0);
    else
        DecompressGlyph_Normal(glyph, 0);
...
void HelpSystem_PrintTextInTopLeftCorner(const u8 * str)
{
    GenerateFontHalfRowLookupTable(0x1, 0xF, 0x2);
...
{
    { vu16 tmp = (vu16)(0xEEEE); CpuSet((void *)&tmp, gDecompressionBuffer + 0x0000, 0x00000000 | 0x01000000 | ((0x3400)/(16/8) & 0x1FFFFF)); };
    GenerateFontHalfRowLookupTable(0x1, 0xE, 0x2);
...
void HelpSystem_PrintTopicMouseoverDescription(const u8 * str)
{
    { vu16 tmp = (vu16)(0x1111); CpuSet((void *)&tmp, gDecompressionBuffer + 0x23C0, 0x00000000 | 0x01000000 | ((0x1040)/(16/8) & 0x1FFFFF)); };
    GenerateFontHalfRowLookupTable(0x2, 0x1, 0x3);
    HelpSystemRenderText(2, gDecompressionBuffer + 0x23C0, str, 2, 6, 26, 5);
}

void HelpSystem_FillPanel3(void)
{
    { vu16 tmp = (vu16)(0xFFFF); CpuSet((void *)&tmp, gDecompressionBuffer + 0x3D00, 0x00000000 | 0x01000000 | ((0x1C0)/(16/8) & 0x1FFFFF)); };
}

void HelpSystem_FillPanel2(void)
{
    { vu16 tmp = (vu16)(0xFFFF); CpuSet((void *)&tmp, gDecompressionBuffer + 0x3400, 0x00000000 | 0x01000000 | ((0x400)/(16/8) & 0x1FFFFF)); };
}

void HelpSystem_FillPanel1(void)
{
    { vu16 tmp = (vu16)(0xFFFF); CpuSet((void *)&tmp, gDecompressionBuffer + 0x0000, 0x00000000 | 0x01000000 | ((0x3400)/(16/8) & 0x1FFFFF)); };
}
...
s32 HelpSystem_GetMenuInput(void)
{
    ...
    else if (({(gMain.newKeys) & (0x0200 | 0x0100);}))
    {
        return -6;
    }
