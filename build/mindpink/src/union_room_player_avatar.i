...
# 23 "src/union_room_player_avatar.c"
...
static const u8 sUnionRoomObjGfxIds[2][(1 << 3) + 2] = {
    [0] = {
        41,
        54,
        39,
...
static bool32 SetUnionRoomPlayerEnterExitMovement(u32 leaderId, const u8 * movement)
{
    ...
    {
        (0) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 387, "0", 1);;
        return 0;
    }
    return 1;
}
...
u8 InitUnionRoomPlayerObjects(struct UnionRoomObject * players)
{
    ...
    (sUnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 442, "UnionObjWork != NULL", 1);
    for (i = 0; i < 8; i++)
    {
        players[i].state = 0;
        players[i].gfxId = 0;
...
static bool32 AnimateUnionRoomPlayerDespawn(s8 * state, u32 leaderId, struct UnionRoomObject * object)
{
    ...
    {
    case 0:
        if (SetUnionRoomPlayerEnterExitMovement(leaderId, sMovement_UnionPlayerExit) == 1)
        {
...
static bool32 AnimateUnionRoomPlayerSpawn(s8 * state, u32 leaderId, struct UnionRoomObject * object)
{
    ...
    case 0:
        ...
        if (IsUnionRoomPlayerAt(leaderId, 0, x, y) == 1)
            break;
...
static bool32 SpawnGroupLeader(u32 leaderId, u32 gender, u32 id)
{
    ...
    (sUnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 561, "UnionObjWork != NULL", 1);
    object->schedAnim = 1;
...
{
    struct UnionRoomObject * object = &sUnionObjWork[leaderId];
    (sUnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 577, "UnionObjWork != NULL", 1);
    object->schedAnim = 2;
...
static void AnimateUnionRoomPlayer(u32 leaderId, struct UnionRoomObject * object)
{
    ...
    case 2:
        ...
        else if (AnimateUnionRoomPlayerSpawn(&object->animState, leaderId, object) == 1)
        {
            object->state = 1;
        }
        break;
...
{
    s32 i;
    (sUnionObjWork != ((void *)0)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 643, "UnionObjWork != NULL", 1);
    for (i = 0; i < 8; i++)
...
static u8 CreateTask_AnimateUnionRoomPlayers(void)
{
    ...
    {
        (0) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 655, "0", 1);
        return 16;
    }
    else
    {
...
void CreateUnionRoomPlayerSprites(u8 * spriteIds, s32 leaderId)
{
    ...
    {
        s32 id = (5 * leaderId + memberId);
        spriteIds[id] = CreateVirtualObject(25,
                                                 id - (64 - 8),
                                                 ...
                                                 3, 1);
        SetVirtualObjectInvisibility(id - (64 - 8), 1);
    }
}
...
void MakeGroupAssemblyAreasPassable(void)
{
    ...
    for (leaderId = 0; leaderId < 8; leaderId++)
    {
        for (memberId = 0; memberId < 5; memberId++)
        {
...
{
    return IsVirtualObjectInvisible((5 * leaderId + memberId) - (64 - 8));
}
...
static void SpawnGroupMember(u32 leaderId, u32 memberId, u8 graphicsId, struct RfuGameData * gameData)
{
    s32 x, y;
    s32 id = (5 * leaderId + memberId);
    if (IsUnionRoomPlayerInvisible(leaderId, memberId) == 1)
    {
        SetVirtualObjectInvisibility(id - (64 - 8), 0);
        SetVirtualObjectSpriteAnim(id - (64 - 8), 1);
    }
    SetVirtualObjectGraphics(id - (64 - 8), graphicsId);
...
{
    s32 x, y;
    SetVirtualObjectSpriteAnim((5 * leaderId + memberId) - (64 - 8), 2);
    GetUnionRoomPlayerCoords(leaderId, memberId, &x, &y);
    MapGridSetMetatileImpassabilityAt(x, y, 0);
}
...
static void AssembleGroup(u32 leaderId, struct RfuGameData * gameData)
{
    ...
    if (IsVirtualObjectInvisible((5 * leaderId + 0) - (64 - 8)) == 1)
    {
        if (IsUnionRoomPlayerAt(leaderId, 0, x, y) == 1 || IsUnionRoomPlayerAt(leaderId, 0, x2, y2) == 1)
            return;
    ...
    for (i = 1; i < 5; i++)
    {
        ...
        else if (IsUnionRoomPlayerAt(leaderId, i, x, y) == 0 && IsUnionRoomPlayerAt(leaderId, i, x2, y2) == 0)
...
static void SpawnGroupLeaderAndMembers(u32 leaderId, struct RfuGameData * gameData)
{
    ...
    case 19 | (1 << 6):
        ...
        break;
    default:
        (0) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "rfu_union_tool.c"), 979, "0", 1);
    }
}
...
bool32 TryInteractWithUnionRoomMember(struct RfuPlayerList *list, s16 *memberIdPtr, s16 *leaderIdPtr, u8 *spriteIds)
{
    ...
    for (leaderId = 0, leaders = list->players; leaderId < 8; leaderId++)
    {
        for (memberId = 0; memberId < 5; memberId++)
        {
            s32 objId = (5 * leaderId + memberId);
            ...
            if (IsVirtualObjectInvisible(objId - (64 - 8)) != 0)
                continue;
            if (IsVirtualObjectAnimating(objId - (64 - 8)) != 0)
                continue;
            if (leaders[leaderId].groupScheduledAnim != 1)
                continue;
...
{
    TurnVirtualObject(5 * leaderId - (64 - 8) + memberId, direction);
}
