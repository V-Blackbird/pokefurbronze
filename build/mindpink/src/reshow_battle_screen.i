...
static void ReshowBattleScreen_TurnOnDisplay(void)
{
    ...
    SetGpuReg(0x42, 0);
    SetGpuReg(0x46, 0);
    SetGpuRegBits(0x0, 0x0040 | 0x1000 | 0x2000 | 0x8000);
}
...
static bool8 LoadBattlerSpriteGfx(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        else if (gBattleTypeFlags & (1 << 20) && battler == 0)
            DecompressTrainerBackPalette(gSaveBlock2Ptr->playerGender, battler);
        else if (!gBattleSpritesDataPtr->battlerData[battler].behindSubstitute)
    ...
    if (battler < gBattlersCount)
    {
        ...
        if (GetBattlerSide(battler) != 0)
        {
            if (GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 57) == 0)
                return;
            SetMultiuseSpriteTemplateToPokemon(GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 11), GetBattlerPosition(battler));
            ...
            gSprites[gBattlerSpriteIds[battler]].data[2] = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]], 11);
        ...
        else if (GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 57) == 0)
        {
            return;
        }
        else
        {
            SetMultiuseSpriteTemplateToPokemon(GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 11), GetBattlerPosition(battler));
            gBattlerSpriteIds[battler] = CreateSprite(&gMultiuseSpriteTemplate, GetBattlerSpriteCoord(battler, BATTLER_COORD_X_2), posY, GetBattlerSpriteSubpriority(battler));
            gSprites[gBattlerSpriteIds[battler]].oam.paletteNum = battler;
            gSprites[gBattlerSpriteIds[battler]].callback = SpriteCallbackDummy;
            gSprites[gBattlerSpriteIds[battler]].data[0] = battler;
            gSprites[gBattlerSpriteIds[battler]].data[2] = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 11);
            StartSpriteAnim(&gSprites[gBattlerSpriteIds[battler]], gBattleMonForms[battler]);
        }
...
static void CreateHealthboxSprite(u8 battler)
{
    if (battler < gBattlersCount)
    {
        ...
        else if (!(gBattleTypeFlags & (1 << 7)) && GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], 57) == 0)
        {
            SetHealthboxSpriteInvisible(healthboxSpriteId);
        }
    }
