...
static const struct WindowTemplate sWindowTemplate[] = {
    ...
    [MAIN_MENU_WINDOW_COUNT] = { 0xFF, 0, 0, 0, 0, 0, 0, }
};
...
static bool32 MainMenuGpuInit(u8 a0)
{
    ...
    { vu16 tmp = (vu16)(0); { vu32 *dmaRegs = (vu32 *)(0x4000000 + 0xd4); dmaRegs[0] = (vu32)(&tmp); dmaRegs[1] = (vu32)((void *)(0x5000000 + 2)); dmaRegs[2] = (vu32)((0x8000 | 0x0000 | 0x0000 | 0x0100 | 0x0000) << 16 | (((0x200 + 0x200) - 2)/(16/8))); dmaRegs[2]; }; };
    ScanlineEffect_Stop();
    ResetTasks();
    ResetSpriteData();
    ...
    InitBgsFromTemplates(0, sBgTemplate, (sizeof(sBgTemplate) / sizeof((sBgTemplate)[0])));
    ChangeBgX(0, 0, 0);
    ChangeBgY(0, 0, 0);
    ChangeBgX(1, 0, 0);
    ChangeBgY(1, 0, 0);
    ...
    LoadPalette(sTextbox_Pal, (0x000 + ((15) * 16)), sizeof(sTextbox_Pal));
    SetGpuReg(0x40, 0);
    SetGpuReg(0x44, 0);
    SetGpuReg(0x48, 0);
    SetGpuReg(0x4a, 0);
...
static void Task_SetWin0BldRegsAndCheckSaveFile(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        SetGpuReg(0x40, 0);
        SetGpuReg(0x44, 0);
        SetGpuReg(0x48, 0x0001);
        SetGpuReg(0x4a, 0x0021);
        SetGpuReg(0x50, (1 << 0) | (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4) | (1 << 5) | (3 << 6));
        ...
        case 1:
            ...
            {
                gTasks[taskId].data[0] = MAIN_MENU_MYSTERYGIFT;
            }
            else
            {
                gTasks[taskId].data[0] = MAIN_MENU_CONTINUE;
            }
            gTasks[taskId].func = Task_SetWin0BldRegsNoSaveFileCheck;
            break;
        case 2:
            SetStdFrame0OnBg(0);
            gTasks[taskId].data[0] = MAIN_MENU_NEWGAME;
...
static void Task_SaveErrorStatus_RunPrinterThenWaitButton(u8 taskId)
{
    ...
    {
        RunTextPrinters();
        if (!IsTextPrinterActive(MAIN_MENU_WINDOW_ERROR) && ({(gMain.newKeys) & (0x0001);}))
        {
            ClearWindowTilemap(MAIN_MENU_WINDOW_ERROR);
            MainMenu_EraseWindow(&sWindowTemplate[MAIN_MENU_WINDOW_ERROR]);
            LoadUserFrameToBg(0);
            if (gTasks[taskId].data[0] == MAIN_MENU_NEWGAME)
                gTasks[taskId].func = Task_SetWin0BldRegsNoSaveFileCheck;
            else
...
static void Task_PrintMainMenuText(u8 taskId)
{
    ...
    if (gSaveBlock2Ptr->playerGender == 0)
        ...
        pal = ((31) | ((3) << 5) | ((21) << 10));
    LoadPalette(&pal, (0x000 + ((15) * 16)) + 1, ((1) * sizeof(u16)));
    switch (gTasks[taskId].data[0])
    {
    case MAIN_MENU_NEWGAME:
    default:
        FillWindowPixelBuffer(MAIN_MENU_WINDOW_NEWGAME_ONLY, ((10) | ((10) << 4)));
    ...
    case MAIN_MENU_CONTINUE:
        FillWindowPixelBuffer(MAIN_MENU_WINDOW_CONTINUE, ((10) | ((10) << 4)));
        FillWindowPixelBuffer(MAIN_MENU_WINDOW_NEWGAME, ((10) | ((10) << 4)));
    ...
    case MAIN_MENU_MYSTERYGIFT:
        FillWindowPixelBuffer(MAIN_MENU_WINDOW_CONTINUE, ((10) | ((10) << 4)));
        FillWindowPixelBuffer(MAIN_MENU_WINDOW_NEWGAME, ((10) | ((10) << 4)));
        FillWindowPixelBuffer(MAIN_MENU_WINDOW_MYSTERYGIFT, ((10) | ((10) << 4)));
        AddTextPrinterParameterized3(MAIN_MENU_WINDOW_CONTINUE, FONT_NORMAL, 2, 2, sTextColor1, -1, gText_Continue);
        AddTextPrinterParameterized3(MAIN_MENU_WINDOW_NEWGAME, FONT_NORMAL, 2, 2, sTextColor1, -1, gText_NewGame);
...
static void MoveWindowByMenuTypeAndCursorPos(u8 menuType, u8 cursorPos)
{
    ...
    SetGpuReg(0x44, (win0vTop + (2 << 8)) | (win0vBot - 2));
}
...
static bool8 HandleMenuInput(u8 taskId)
{
    ...
    else if (({(gMain.newKeys) & (0x0040);}) && gTasks[taskId].data[1] > 0)
    {
        gTasks[taskId].data[1]--;
        return 1;
    }
    else if (({(gMain.newKeys) & (0x0080);}) && gTasks[taskId].data[1] < sMenuCursorYMax[gTasks[taskId].data[0]])
    {
        gTasks[taskId].data[1]++;
        return 1;
    }
...
static void PrintMessageOnWindow4(const u8 *str)
{
    FillWindowPixelBuffer(MAIN_MENU_WINDOW_ERROR, ((10) | ((10) << 4)));
    ...
    SetGpuReg(0x40, (((19) << 8) | (221)));
    SetGpuReg(0x44, (((115) << 8) | (157)));
}
...
static void PrintBadgeCount(void)
{
    ...
    u32 flagId;
    u8 nbadges = 0;
    for (flagId = ((((0x4FF + 1) + 768 - 1) + 1) + 0x20); flagId < ((((0x4FF + 1) + 768 - 1) + 1) + 0x20) + 8; flagId++)
    {
        if (FlagGet(flagId))
            nbadges++;
    }
...
static void LoadUserFrameToBg(u8 bgId)
{
    ...
    LoadPalette(GetUserWindowGraphics(gSaveBlock2Ptr->optionsWindowFrameType)->palette, (0x000 + ((2) * 16)), ((16) * sizeof(u16)));
    MainMenu_EraseWindow(&sWindowTemplate[MAIN_MENU_WINDOW_ERROR]);
}

static void SetStdFrame0OnBg(u8 bgId)
{
    LoadStdWindowGfx(MAIN_MENU_WINDOW_NEWGAME_ONLY, 0x1B1, (0x000 + ((2) * 16)));
    MainMenu_EraseWindow(&sWindowTemplate[MAIN_MENU_WINDOW_ERROR]);
}
