...
u8 GiveMailToMon(struct Pokemon *mon, u16 itemId)
{
    ...
    for (id = 0; id < 6; id++)
    {
        if (gSaveBlock1Ptr->mail[id].itemId == 0)
        {
            ...
            for (i = 0; i < 7 && gSaveBlock2Ptr->playerName[i] != 0xFF; i++)
                gSaveBlock1Ptr->mail[id].playerName[i] = gSaveBlock2Ptr->playerName[i];
            for (; i <= 5; i++)
                gSaveBlock1Ptr->mail[id].playerName[i] = 0x00;
...
u8 TakeMailFromMon2(struct Pokemon *mon)
{
    ...
    for (i = 6; i < (6 + 10); i++)
    {
        ...
        {
            memcpy(&gSaveBlock1Ptr->mail[i], &gSaveBlock1Ptr->mail[GetMonData(mon, 64)], sizeof(struct Mail));
