...
static __attribute__((section("ewram_data"))) const u8 *sStringPointers[8] = {0};
...
void DynamicPlaceholderTextUtil_Reset(void)
{
    ...
    u8 *fillval = ((void *)0);
    const u8 **ptr2 = ptr + ((sizeof(sStringPointers) / sizeof((sStringPointers)[0])) - 1);
...
{
    if (idx < (sizeof(sStringPointers) / sizeof((sStringPointers)[0])))
...
u8 GetColorFromTextColorTable(u16 graphicId)
{
    ...
    if (test >= (sizeof(sTextColorTable) / sizeof((sTextColorTable)[0])))
        return 3;
    else
