...
# 9 "src/mailbox_pc.c" 2
# 1 "include/constants/songs.h" 1
# 10 "src/mailbox_pc.c" 2

static __attribute__((section("ewram_data"))) u8 sWindowIds[3] = {};
...
bool8 MailboxPC_InitBuffers(u8 num)
{
    ...
    if (sListMenuItems == ((void *)0))
        return 0;
    for (i = 0; i < (sizeof(sWindowIds) / sizeof((sWindowIds)[0])); i++)
        sWindowIds[i] = 0xFF;
    return 1;
}
...
{
    ClearStdWindowAndFrameToTransparent(sWindowIds[winIdx], 0);
...
static void ItemPrintFunc(u8 windowId, u32 itemId, u8 y)
{
    ...
    if (itemId != -2)
    {
        StringCopy(strbuf, gSaveBlock1Ptr->mail[itemId + 6].playerName);
        if (StringLength(strbuf) <= 5)
            ConvertInternationalString(strbuf, 1);
