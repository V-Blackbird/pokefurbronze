...
__attribute__((section("ewram_data"))) struct UnusedControllerStruct gUnusedControllerStruct = {};
...
static void InitSinglePlayerBtlControllers(void)
{
    if (!(gBattleTypeFlags & (1 << 0)))
    {
        ...
        else
        {
            ...
            else if (gBattleTypeFlags & ((1 << 9) | (1 << 4) | (1 << 20)))
                ...
                if (i < 2)
                {
                    if (((((gBattlerPositions[i])) & 1)) == 0)
                    {
                        if (GetMonData(&gPlayerParty[j], 57) != 0
                         ...
                         && GetMonData(&gPlayerParty[j], 65) != 479
                         && !GetMonData(&gPlayerParty[j], 45))
                        {
                    ...
                    else
                    {
                        if (GetMonData(&gEnemyParty[j], 57) != 0
                         ...
                         && GetMonData(&gEnemyParty[j], 65) != 479
                         && !GetMonData(&gEnemyParty[j], 45))
                        {
                            gBattlerPartyIndexes[i] = j;
                            break;
                        }
...
static void Task_HandleSendLinkBuffersData(u8 taskId)
{
    ...
    case 2:
        ...
        if ((gBattleTypeFlags & (1 << 6) && playerCount > 3) || (!(gBattleTypeFlags & (1 << 6)) && playerCount > 1))
        {
            if (IsLinkMaster())
            {
...
void TryReceiveLinkBattleData(void)
{
    ...
    if (gReceivedRemoteLinkPlayers != 0 && (gBattleTypeFlags & (1 << 5)) && (gLinkPlayers[0].linkType == 0x2211))
    {
        DestroyTask_RfuIdle();
...
void BtlController_EmitMoveAnimation(u8 bufferId, u16 move, u8 turnOfMove, u16 movePower, s32 dmg, u8 friendship, struct DisableStruct *disableStructPtr)
{
    ...
    if (((!AbilityBattleEffects(14, 0, 13, 0, 0) && !AbilityBattleEffects(14, 0, 77, 0, 0))))
    {
...
static void BtlController_EmitUnknownYesNoBox(u8 bufferId, u32 arg1)
{
...
void BtlController_EmitChosenMonReturnValue(u8 bufferId, u8 partyId, u8 *battlePartyOrder)
{
    ...
    sBattleBuffersTransferData[1] = partyId;
    for (i = 0; i < (int)(sizeof(gBattlePartyCurrentOrder) / sizeof((gBattlePartyCurrentOrder)[0])); i++)
