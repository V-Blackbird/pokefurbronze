...
__attribute__((section("ewram_data"))) u16 gBackupMapData[(0x2800)] = {};
__attribute__((section("ewram_data"))) struct MapHeader gMapHeader = {};
__attribute__((section("ewram_data"))) struct Camera gCamera = {};
static __attribute__((section("ewram_data"))) struct ConnectionFlags gMapConnectionFlags = {};
__attribute__((section("ewram_data"))) u8 gGlobalFieldTintMode = 0;
...
static void InitMapLayoutData(struct MapHeader * mapHeader)
{
    ...
    VMap.Xsize = mapLayout->width + (7 * 2 + 1);
    VMap.Ysize = mapLayout->height + (7 * 2);
    (VMap.Xsize * VMap.Ysize <= (0x2800)) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "fieldmap.c"), 158, "VMap.Xsize * VMap.Ysize <= VIRTUAL_MAP_SIZE", 1);;
...
static void InitBackupMapLayoutData(const u16 *map, u16 width, u16 height)
{
    ...
    for (y = 0; y < height; y++)
    {
        CpuSet(map, dest, 0x00000000 | ((width * sizeof(u16))/(16/8) & 0x1FFFFF));
        dest += width + (7 * 2 + 1);
        map += width;
    }
}
...
u8 MapGridGetElevationAt(s32 x, s32 y)
{
    u16 block = ((x >= 0 && x < VMap.Xsize && y >= 0 && y < VMap.Ysize) ? VMap.map[x + VMap.Xsize * y] : ({ u16 block; s32 xprime; s32 yprime; const struct MapLayout *mapLayout = gMapHeader.mapLayout; xprime = x - 7; xprime += 8 * mapLayout->borderWidth; xprime %= mapLayout->borderWidth; yprime = y - 7; yprime += 8 * mapLayout->borderHeight; yprime %= mapLayout->borderHeight; block = mapLayout->border[xprime + yprime * mapLayout->borderWidth] | 0x0C00; }));
...
u32 MapGridGetMetatileIdAt(s32 x, s32 y)
{
    ...
    if (block == 0x03FF)
        return ({ u16 block; s32 xprime; s32 yprime; const struct MapLayout *mapLayout = gMapHeader.mapLayout; xprime = x - 7; xprime += 8 * mapLayout->borderWidth; xprime %= mapLayout->borderWidth; yprime = y - 7; yprime += 8 * mapLayout->borderHeight; yprime %= mapLayout->borderHeight; block = mapLayout->border[xprime + yprime * mapLayout->borderWidth] | 0x0C00; }) & 0x03FF;
...
void MapGridSetMetatileIdAt(s32 x, s32 y, u16 metatile)
{
    ...
    {
        i = x + y * VMap.Xsize;
        VMap.map[i] = (VMap.map[i] & 0xF000) | (metatile & ~0xF000);
    }
}
...
static u32 GetAttributeByMetatileIdAndMapLayout(const struct MapLayout *mapLayout, u16 metatile, u8 attributeType)
{
    ...
    else if (metatile < 1024)
    {
        ...
        return ExtractMetatileAttribute(attributes[metatile - 640], attributeType);
    }
    else
    {
        return 0xFF;
...
{
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, gSaveBlock2Ptr->mapView, 0x00000000 | 0x01000000 | ((sizeof(gSaveBlock2Ptr->mapView))/(16/8) & 0x1FFFFF)); };
}
...
{
    if (((x >= 0 && x < VMap.Xsize && y >= 0 && y < VMap.Ysize) ? VMap.map[x + VMap.Xsize * y] : ({ u16 block; s32 xprime; s32 yprime; const struct MapLayout *mapLayout = gMapHeader.mapLayout; xprime = x - 7; xprime += 8 * mapLayout->borderWidth; xprime %= mapLayout->borderWidth; yprime = y - 7; yprime += 8 * mapLayout->borderHeight; yprime %= mapLayout->borderHeight; block = mapLayout->border[xprime + yprime * mapLayout->borderWidth] | 0x0C00; })) == 0x03FF)
        return -1;
...
const struct MapConnection *GetIncomingConnection(u8 direction, s32 x, s32 y)
{
    ...
    for (i = 0; i < count; i++, connection++)
    {
        if (connection->direction == direction && IsPosInIncomingConnectingMap(direction, x, y, connection) == 1)
            return connection;
    }
    return ((void *)0);
    ...
    else
    {
        ...
        for (i = 0; i < count; i++, connection++)
        {
            ...
            if ((direction == 5 || direction == 6)
                || (direction == 2 && y > 7 - 1)
                || (direction == 1 && y < gMapHeader.mapLayout->height + 7)
                || (direction == 3 && x > 7 - 1)
                || (direction == 4 && x < gMapHeader.mapLayout->width + 7))
            {
                continue;
            }

            if (IsPosInConnectingMap(connection, x - 7, y - 7) == 1)
                return connection;
        }
    }
    return ((void *)0);
...
static void ApplyGlobalTintToPaletteEntries(u16 offset, u16 size)
{
    ...
    CpuSet(&gPlttBufferUnfaded[offset], &gPlttBufferFaded[offset], 0x00000000 | ((((size) * sizeof(u16)))/(16/8) & 0x1FFFFF));
}
...
void ApplyGlobalTintToPaletteSlot(u8 slot, u8 count)
{
    ...
    case 0:
        return;
    case 1:
        TintPalette_GrayScale(&gPlttBufferUnfaded[(0x000 + ((slot) * 16))], count * 16);
        break;
    case 2:
        TintPalette_SepiaTone(&gPlttBufferUnfaded[(0x000 + ((slot) * 16))], count * 16);
        break;
    case 3:
        QuestLog_BackUpPalette((0x000 + ((slot) * 16)), count * 16);
        TintPalette_GrayScale(&gPlttBufferUnfaded[(0x000 + ((slot) * 16))], count * 16);
        break;
    default:
        return;
    }
    CpuFastSet(&gPlttBufferUnfaded[(0x000 + ((slot) * 16))], &gPlttBufferFaded[(0x000 + ((slot) * 16))], ((count * ((16) * sizeof(u16)))/(32/8) & 0x1FFFFF));
}
...
static void LoadTilesetPalette(struct Tileset const *tileset, u16 destOffset, u16 size)
{
    ...
    if (tileset)
    {
        ...
        {
            LoadPalette(&black, destOffset, ((1) * sizeof(u16)));
            LoadPalette(tileset->palettes[0] + 1, destOffset + 1, size - ((1) * sizeof(u16)));
            ApplyGlobalTintToPaletteEntries(destOffset + 1, (size - 2) >> 1);
        }
        else if (tileset->isSecondary == 1)
        {
            LoadPalette(tileset->palettes[7], destOffset, size);
...
void CopyPrimaryTilesetToVram(const struct MapLayout *mapLayout)
{
    CopyTilesetToVram(mapLayout->primaryTileset, 640, 0);
}

void CopySecondaryTilesetToVram(const struct MapLayout *mapLayout)
{
    CopyTilesetToVram(mapLayout->secondaryTileset, 1024 - 640, 640);
}

void CopySecondaryTilesetToVramUsingHeap(const struct MapLayout *mapLayout)
{
    CopyTilesetToVramUsingHeap(mapLayout->secondaryTileset, 1024 - 640, 640);
}

static void LoadPrimaryTilesetPalette(const struct MapLayout *mapLayout)
{
    LoadTilesetPalette(mapLayout->primaryTileset, (0x000 + ((0) * 16)), 7 * ((16) * sizeof(u16)));
}

void LoadSecondaryTilesetPalette(const struct MapLayout *mapLayout)
{
    LoadTilesetPalette(mapLayout->secondaryTileset, (0x000 + ((7) * 16)), (13 - 7) * ((16) * sizeof(u16)));
}
...
void CopyMapTilesetsToVram(struct MapLayout const *mapLayout)
{
    if (mapLayout)
    {
        CopyTilesetToVramUsingHeap(mapLayout->primaryTileset, 640, 0);
        CopyTilesetToVramUsingHeap(mapLayout->secondaryTileset, 1024 - 640, 640);
    }
}
