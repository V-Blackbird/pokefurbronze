...
void CopyItemName(u16 itemId, u8 * dest)
{
    if (itemId == 176)
    {
        StringCopy(dest, GetBerryInfo((((176 - 134) + 1)))->name);
...
bool8 HasAtLeastOneBerry(void)
{
    ...
    exists = CheckBagHasItem(366, 1);
    if (!exists)
    {
        gSpecialVar_Result = 0;
        return 0;
    }
    for (itemId = 134; itemId <= 176; itemId++)
    {
...
bool8 AddBagItem(u16 itemId, u16 count)
{
    ...
    if (pocket == 4 - 1 && !CheckBagHasItem(365, 1))
    {
    ...
    if (pocket == 5 - 1 && !CheckBagHasItem(366, 1))
    {
        idx = BagPocketGetFirstEmptySlot(2 - 1);
        if (idx == -1)
            return 0;
        gBagPockets[2 - 1].itemSlots[idx].itemId = 366;
        SetBagItemQuantity(&gBagPockets[2 - 1].itemSlots[idx].quantity, 1);
        FlagSet(((((0x4FF + 1) + 768 - 1) + 1) + 0x47));
    }
...
void RegisteredItemHandleBikeSwap(void)
{
    ...
    {
    case 260:
        gSaveBlock1Ptr->registeredItem = 273;
        break;
    case 273:
        gSaveBlock1Ptr->registeredItem = 260;
        break;
    }
}
...
void SortPocketAndPlaceHMsFirst(struct BagPocket * pocket)
{
    ...
    for (i = 0; i < pocket->capacity; i++)
    {
        if (pocket->itemSlots[i].itemId == 0 && GetBagItemQuantity(&pocket->itemSlots[i].quantity) == 0)
            return;
        if (pocket->itemSlots[i].itemId >= 340 && GetBagItemQuantity(&pocket->itemSlots[i].quantity) != 0)
        {
...
void TrySetObtainedItemQuestLogEvent(u16 itemId)
{
    ...
    {
        if (itemId != 362 || (gSaveBlock1Ptr->location.mapGroup == ((2 | (5 << 8)) >> 8) && gSaveBlock1Ptr->location.mapNum == ((2 | (5 << 8)) & 0xFF)))
        {
            ptr = Alloc(sizeof(*ptr));
            ptr->itemId = itemId;
            ptr->mapSectionId = gMapHeader.regionMapSectionId;
            SetQuestLogEvent(40, (void *)ptr);
            Free(ptr);
        }
    }
}
