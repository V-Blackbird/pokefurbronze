...
static __attribute__((section("ewram_data"))) struct MonMarkingsMenu *sMenu = ((void *)0);
...
static void BufferMenuWindowTiles(void)
{
    ...
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, sMenu->windowSpriteTiles, 0x00000000 | 0x01000000 | ((sizeof(sMenu->windowSpriteTiles))/(16/8) & 0x1FFFFF)); };
}
...
void FreeMonMarkingsMenu(void)
{
    ...
    for (i = 0; i < (sizeof(sMenu->windowSprites) / sizeof((sMenu->windowSprites)[0])); i++)
    {
        if (sMenu->windowSprites[i] == ((void *)0))
            return;
        DestroySprite(sMenu->windowSprites[i]);
    }
    for (i = 0; i < 4; i++)
    {
        if (sMenu->markingSprites[i] == ((void *)0))
            return;
        DestroySprite(sMenu->markingSprites[i]);
    }
    if (sMenu->cursorSprite != ((void *)0))
        DestroySprite(sMenu->cursorSprite);
    if (sMenu->textSprite != ((void *)0))
...
static void CreateMonMarkingsMenuSprites(s16 x, s16 y, u16 baseTileTag, u16 basePaletteTag)
{
    ...
    if (spriteId != 64)
    {
        ...
        sMenu->textSprite->oam.shape = (((2 << 2) | (0)) & 0x03);
        sMenu->textSprite->oam.size = ((((2 << 2) | (0)) >> 2) & 0x03);
        StartSpriteAnim(sMenu->textSprite, ((4 * 2) + 1));
        sMenu->textSprite->x = x + 32;
        sMenu->textSprite->y = y + 80;
        CalcCenterToCornerVec(sMenu->textSprite, (((2 << 2) | (1)) & 0x03), ((((2 << 2) | (1)) >> 2) & 0x03), 0);
    }
    else
    {
        sMenu->textSprite = ((void *)0);
    }
    ...
    if (spriteId != 64)
    {
        ...
        StartSpriteAnim(sMenu->cursorSprite, (4 * 2));
    }
    else
    {
        sMenu->cursorSprite = ((void *)0);
    }
}
...
struct Sprite *CreateMonMarkingAllCombosSprite(u16 tileTag, u16 paletteTag, const u16 *palette)
{
    ...
    return CreateMarkingComboSprite(tileTag, paletteTag, palette, 1 << 4);
}
...
void UpdateMonMarkingTiles(u8 markings, void *dest)
{
    RequestDma3Copy(&sMonMarkings_Gfx[64 * markings], dest, 0x80, 1);
}
