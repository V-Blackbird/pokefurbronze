...
static __attribute__((section("ewram_data"))) bool8 sScheduleOpenDottedHole = 0;
...
bool8 SetUpFieldMove_Cut(void)
{
    s16 x, y;
    u8 i, j;
    sScheduleOpenDottedHole = 0;
    if (CutMoveRuinValleyCheck() == 1)
    {
        sScheduleOpenDottedHole = 1;
        gFieldCallback2 = FieldCallback_PrepareFadeInFromMenu;
        gPostMenuFieldCallback = FieldCallback_CutGrass;
        return 1;
    }

    if (CheckObjectGraphicsInFrontOfPlayer(95) == 1)
    {
        gFieldCallback2 = FieldCallback_PrepareFadeInFromMenu;
    ...
    else
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            for (j = 0; j < 3; j++)
            {
                ...
                {
                    if (MetatileAtCoordsIsGrassTile(x, y) == 1)
                    {
                        gFieldCallback2 = FieldCallback_PrepareFadeInFromMenu;
                        gPostMenuFieldCallback = FieldCallback_CutGrass;
                        return 1;
...
bool8 FldEff_UseCutOnGrass(void)
{
    ...
    gTasks[taskId].data[8] = (u32)FieldMoveCallback_CutGrass >> 16; gTasks[taskId].data[9] = (u32)FieldMoveCallback_CutGrass;;
...
{
    u8 taskId = CreateFieldEffectShowMon();
    gTasks[taskId].data[8] = (u32)FieldMoveCallback_CutTree >> 16; gTasks[taskId].data[9] = (u32)FieldMoveCallback_CutTree;;
    IncrementGameStat(18);
    return 0;
}
...
bool8 FldEff_CutGrass(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        gSprites[sCutGrassSpriteArrayPtr[i]].data[2] = i * (0x100 / 8);
    }
    return 0;
}
