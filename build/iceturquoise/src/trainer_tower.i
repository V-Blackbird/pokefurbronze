# 0 "src/trainer_tower.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/trainer_tower.c"
...
# 10 "src/trainer_tower.c" 2
# 1 "include/battle_main.h" 1
# 11 "src/trainer_tower.c" 2
...
# 15 "src/trainer_tower.c" 2
# 1 "include/constants/items.h" 1
# 16 "src/trainer_tower.c" 2
# 1 "include/constants/songs.h" 1
# 17 "src/trainer_tower.c" 2
# 1 "include/constants/layouts.h" 1
# 18 "src/trainer_tower.c" 2

# 1 "include/constants/event_objects.h" 1
# 20 "src/trainer_tower.c" 2
# 1 "include/constants/trainer_tower.h" 1
# 21 "src/trainer_tower.c" 2
...
static __attribute__((section("ewram_data"))) struct TrainerTowerState * sTrainerTowerState = ((void *)0);
static __attribute__((section("ewram_data"))) struct TrainerTowerOpponent * sTrainerTowerOpponent = ((void *)0);
static __attribute__((section("ewram_data"))) u8 sUnused = 0;
...
static const u8 sTextColors[3] = {0x0, 0x2, 0x3};

static void (*const sTrainerTowerFunctions[])(void) = {
    [0] = InitTrainerTowerFloor,
    [1] = BufferTowerOpponentSpeech,
    [2] = DoTrainerTowerBattle,
    [3] = TrainerTowerGetChallengeType,
    [4] = TrainerTowerAddFloorCleared,
    [5] = GetFloorAlreadyCleared,
    [6] = StartTrainerTowerChallenge,
    [7] = GetOwnerState,
    [8] = GiveChallengePrize,
    [9] = CheckFinalTime,
    [10] = TrainerTowerResumeTimer,
    [11] = TrainerTowerSetPlayerLost,
    [12] = GetTrainerTowerChallengeStatus,
    [13] = GetCurrentTime,
    [14] = ShowResultsBoard,
    [15] = CloseResultsBoard,
    [16] = TrainerTowerGetDoublesEligiblity,
    [17] = TrainerTowerGetNumFloors,
    [18] = ShouldWarpToCounter,
    [19] = PlayTrainerTowerEncounterMusic,
    [20] = HasSpokenToOwner
};


static const u16 sFloorLayouts[8][4 - 1] = {
    {280, 348, 356},
    {281, 349, 357},
    {282, 350, 358},
    {283, 351, 359},
...
static const u8 sSingleBattleChallengeMonIdxs[8][2] = {
    {0, 2},
    {1, 3},
    {2, 4},
    {3, 5},
...
static const u8 sDoubleBattleChallengeMonIdxs[8][2] = {
    {0, 1},
    {1, 3},
    {2, 0},
    {3, 4},
...
static const u8 sKnockoutChallengeMonIdxs[8][3] = {
    {0, 2, 4},
    {1, 3, 5},
    {2, 3, 1},
    {3, 4, 0},
...
extern const struct TrainerTowerFloor *const gTrainerTowerFloors[][8];
...
void InitTrainerTowerBattleStruct(void)
{
    ...
    trainerId = VarGet((0x4000 + 0x1));
    StringCopyN(sTrainerTowerOpponent->name, sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].name, 11);

    for (i = 0; i < 6; i++)
    {
        sTrainerTowerOpponent->speechWin[i] = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].speechWin[i];
        sTrainerTowerOpponent->speechLose[i] = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].speechLose[i];

        if (sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].challengeType == 1)
        {
            sTrainerTowerOpponent->speechWin2[i] = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId + 1].speechWin[i];
            sTrainerTowerOpponent->speechLose2[i] = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId + 1].speechLose[i];
        }
    }

    sTrainerTowerOpponent->battleType = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].challengeType;
    sTrainerTowerOpponent->facilityClass = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].facilityClass;
    sTrainerTowerOpponent->textColor = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].textColor;
    SetVBlankCounter1Ptr(&gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer);
    FreeTrainerTowerDataStruct();
}

void FreeTrainerTowerBattleStruct(void)
{
    { Free(sTrainerTowerOpponent); sTrainerTowerOpponent = ((void *)0); };
}

void GetTrainerTowerOpponentWinText(u8 *dest, u8 opponentIdx)
{
    VarSet((0x4000 + 0x3), opponentIdx);
...
static void SetUpTrainerTowerDataStruct(void)
{
    ...
    sTrainerTowerState->floorIdx = gMapHeader.mapLayoutId - 280;
    if (ReadTrainerTowerAndValidate() == 1)
...
static void FreeTrainerTowerDataStruct(void)
{
    { Free(sTrainerTowerState); sTrainerTowerState = ((void *)0); };
}

static void InitTrainerTowerFloor(void)
{
    if (gMapHeader.mapLayoutId - 279 > sTrainerTowerState->data.numFloors)
    {
        gSpecialVar_Result = 3;
        SetCurrentMapLayout(288);
    }
    else
    {
        gSpecialVar_Result = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].challengeType;
...
static void SetTrainerTowerNPCGraphics(void)
{
    ...
    switch (sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].challengeType)
    {
    case 0:
        facilityClass = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[0].facilityClass;
        for (i = 0; i < (sizeof(sSingleBattleTrainerInfo) / sizeof((sSingleBattleTrainerInfo)[0])); i++)
        {
            if (sSingleBattleTrainerInfo[i].facilityClass == facilityClass)
                break;
        }

        if (i != (sizeof(sSingleBattleTrainerInfo) / sizeof((sSingleBattleTrainerInfo)[0])))
            trainerGfx1 = sSingleBattleTrainerInfo[i].objGfx;
        else
            trainerGfx1 = 18;

        VarSet(0x4011, trainerGfx1);
        break;
    case 1:
        facilityClass = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[0].facilityClass;
        for (i = 0; i < (sizeof(sDoubleBattleTrainerInfo) / sizeof((sDoubleBattleTrainerInfo)[0])); i++)
        {
            if (sDoubleBattleTrainerInfo[i].facilityClass == facilityClass)
                break;
        }
        if (i != (sizeof(sDoubleBattleTrainerInfo) / sizeof((sDoubleBattleTrainerInfo)[0])))
        {
        ...
        VarSet(0x4010, trainerGfx1);
        VarSet(0x4013, trainerGfx2);
        break;
    case 2:
        for (j = 0; j < 3; j++)
        {
            facilityClass = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[j].facilityClass;
            for (i = 0; i < (sizeof(sSingleBattleTrainerInfo) / sizeof((sSingleBattleTrainerInfo)[0])); i++)
            {
                if (sSingleBattleTrainerInfo[i].facilityClass == facilityClass)
                    break;
...
static void BufferTowerOpponentSpeech(void)
{
    ...
    u8 facilityClass;
    u8 challengeType = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].challengeType;

    if (challengeType != 1)
        facilityClass = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].facilityClass;
    else
        facilityClass = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[0].facilityClass;
    ...
    case 2:
        ...
        TT_ConvertEasyChatMessageToString(sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].speechBefore, gStringVar4);
        break;
    case 3:
        TrainerTowerGetOpponentTextColor(challengeType, facilityClass);
        TT_ConvertEasyChatMessageToString(sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].speechWin, gStringVar4);
        break;
    case 4:
        TrainerTowerGetOpponentTextColor(challengeType, facilityClass);
        TT_ConvertEasyChatMessageToString(sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].speechLose, gStringVar4);
        break;
    case 5:
        TT_ConvertEasyChatMessageToString(sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerId].speechAfter, gStringVar4);
        break;
    }
}
...
static void TrainerTowerAddFloorCleared(void)
{
    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].floorsCleared++;
}
...
static void GetFloorAlreadyCleared(void)
{
    ...
    if (mapLayoutId - 280 == gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].floorsCleared
     && mapLayoutId - 279 <= sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].floorIdx)
...
static void StartTrainerTowerChallenge(void)
{
    ...
    if (gSaveBlock1Ptr->towerChallengeId >= 4)
        gSaveBlock1Ptr->towerChallengeId = 0;
    ValidateOrResetCurTrainerTowerRecord();
    if (!ReadTrainerTowerAndValidate())
        gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].validated = 1;
    else
        gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].validated = 0;
    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].floorsCleared = 0;
    SetVBlankCounter1Ptr(&gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer);
    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer = 0;
    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].spokeToOwner = 0;
    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].checkedFinalTime = 0;
}
...
static void GetOwnerState(void)
{
    ...
    if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].spokeToOwner)
        gSpecialVar_Result++;
    if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].receivedPrize && gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].checkedFinalTime)
        gSpecialVar_Result++;

    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].spokeToOwner = 1;
}
...
static void GiveChallengePrize(void)
{
    ...
    if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].receivedPrize)
    {
    ...
    else if (AddBagItem(itemId, 1) == 1)
    {
        ...
        gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].receivedPrize = 1;
        gSpecialVar_Result = 0;
    }
    else
    {
...
static void CheckFinalTime(void)
{
    if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].checkedFinalTime)
    {
        gSpecialVar_Result = 2;
    }
    else if (GetTrainerTowerRecordTime(&gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].bestTime) > gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer)
    {
        SetTrainerTowerRecordTime(&gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].bestTime, gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer);
        gSpecialVar_Result = 0;
    }
    else
    {
    ...
    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].checkedFinalTime = 1;
}
...
static void TrainerTowerResumeTimer(void)
{
    if (!gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].spokeToOwner)
    {
        if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer >= 215999)
            gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer = 215999;
        else
            SetVBlankCounter1Ptr(&gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer);
    }
}
...
static void TrainerTowerSetPlayerLost(void)
{
    gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].hasLost = 1;
}

static void GetTrainerTowerChallengeStatus(void)
{
    if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].hasLost)
    {
        gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].hasLost = 0;
        gSpecialVar_Result = 0;
    }
    else if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].unkA_4)
    {
        gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].unkA_4 = 0;
        gSpecialVar_Result = 1;
    }
    else
    {
...
static void GetCurrentTime(void)
{
    ...
    ({ s32 minutes, seconds, centiseconds, frames; frames = (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].timer); minutes = frames / (60 * 60); frames %= (60 * 60); seconds = frames / 60; frames %= 60; centiseconds = frames * 168 / 100; ConvertIntToDecimalStringN(gStringVar1, minutes, STR_CONV_MODE_RIGHT_ALIGN, 2); ConvertIntToDecimalStringN(gStringVar2, seconds, STR_CONV_MODE_RIGHT_ALIGN, 2); ConvertIntToDecimalStringN(gStringVar3, centiseconds, STR_CONV_MODE_LEADING_ZEROS, 2); });
}
...
static void ShowResultsBoard(void)
{
    ...
    AddTextPrinterParameterized(windowId, FONT_NORMAL, gText_TimeBoard, 74, 0, 0xFF, ((void *)0));

    for (i = 0; i < 4; i++)
    {
        ({ s32 minutes, seconds, centiseconds, frames; frames = (GetTrainerTowerRecordTime(&gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].bestTime)); minutes = frames / (60 * 60); frames %= (60 * 60); seconds = frames / 60; frames %= 60; centiseconds = frames * 168 / 100; ConvertIntToDecimalStringN(gStringVar1, minutes, STR_CONV_MODE_RIGHT_ALIGN, 2); ConvertIntToDecimalStringN(gStringVar2, seconds, STR_CONV_MODE_RIGHT_ALIGN, 2); ConvertIntToDecimalStringN(gStringVar3, centiseconds, STR_CONV_MODE_LEADING_ZEROS, 2); });

        StringExpandPlaceholders(gStringVar4, gText_XMinYZSec);
        AddTextPrinterParameterized(windowId, FONT_NORMAL, gTrainerTowerChallengeTypeTexts[i - 1], 24, 36 + 20 * i, 0xFF, ((void *)0));
        AddTextPrinterParameterized(windowId, FONT_NORMAL, gStringVar4, 96, 46 + 20 * i, 0xFF, ((void *)0));
    }

    PutWindowTilemap(windowId);
    CopyWindowToVram(windowId, COPYWIN_FULL);
    VarSet((0x4000 + 0x1), windowId);
}

static void CloseResultsBoard(void)
{
    u8 windowId = VarGet((0x4000 + 0x1));
...
static void ShouldWarpToCounter(void)
{
    if (gMapHeader.mapLayoutId == 279 && VarGet(0x4082) == 0)
        gSpecialVar_Result = 0;
    else
        gSpecialVar_Result = 0;
}
...
{
    s32 i;
    u16 idx = VarGet((0x4000 + 0x1));
    u8 facilityClass = sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[idx].facilityClass;

    for (i = 0; i < (sizeof(sTrainerEncounterMusicLUT) / sizeof((sTrainerEncounterMusicLUT)[0])); i++)
    {
        if (sTrainerEncounterMusicLUT[i].facilityClass == gFacilityClassToTrainerClass[facilityClass])
            break;
    }

    if (i != (sizeof(sTrainerEncounterMusicLUT) / sizeof((sTrainerEncounterMusicLUT)[0])))
    {
...
static void HasSpokenToOwner(void)
{
    gSpecialVar_Result = gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].spokeToOwner;
}

static void BuildEnemyParty(void)
{
    u16 trainerIdx = VarGet((0x4000 + 0x1));
    s32 level = GetPartyMaxLevel();
    u8 floorIdx = gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].floorsCleared;
    s32 i;
    u8 monIdx;
    ...
    default:
        for (i = 0; i < 2; i++)
        {
            ...
            sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerIdx].mons[monIdx].level = level;
            CreateBattleTowerMon(&gEnemyParty[i], &sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerIdx].mons[monIdx]);
        }
        break;
    case 1:
        monIdx = sDoubleBattleChallengeMonIdxs[floorIdx][0];
        sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[0].mons[monIdx].level = level;
        CreateBattleTowerMon(&gEnemyParty[0], &sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[0].mons[monIdx]);

        monIdx = sDoubleBattleChallengeMonIdxs[floorIdx][1];
        sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[1].mons[monIdx].level = level;
        CreateBattleTowerMon(&gEnemyParty[1], &sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[1].mons[monIdx]);
        break;
    case 2:
        monIdx = sKnockoutChallengeMonIdxs[floorIdx][trainerIdx];
        sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerIdx].mons[monIdx].level = level;
        CreateBattleTowerMon(&gEnemyParty[0], &sTrainerTowerState->data.floors[sTrainerTowerState->floorIdx].trainers[trainerIdx].mons[monIdx]);
        break;
    }
}
...
static s32 GetPartyMaxLevel(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (GetMonData(&gPlayerParty[i], 11, ((void *)0)) != 0 && GetMonData(&gPlayerParty[i], 65, ((void *)0)) != 479)
        {
            s32 currLevel = GetMonData(&gPlayerParty[i], 56, ((void *)0));
...
static void ValidateOrResetCurTrainerTowerRecord(void)
{
    if (gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].unk9 != sTrainerTowerState->data.id)
    {
        gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].unk9 = sTrainerTowerState->data.id;
        SetTrainerTowerRecordTime(&gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].bestTime, 215999);
        gSaveBlock1Ptr->trainerTower[gSaveBlock1Ptr->towerChallengeId].receivedPrize = 0;
    }
}
...
void PrintTrainerTowerRecords(void)
{
    ...
    SetUpTrainerTowerDataStruct();
    FillWindowPixelRect(0, ((0) | ((0) << 4)), 0, 0, 216, 144);
    ValidateOrResetCurTrainerTowerRecord();
    AddTextPrinterParameterized3(0, FONT_NORMAL, 0x4a, 0, sTextColors, 0, gText_TimeBoard);

    for (i = 0; i < 4; i++)
    {
        ({ s32 minutes, seconds, centiseconds, frames; frames = (GetTrainerTowerRecordTime(&gSaveBlock1Ptr->trainerTower[i].bestTime)); minutes = frames / (60 * 60); frames %= (60 * 60); seconds = frames / 60; frames %= 60; centiseconds = frames * 168 / 100; ConvertIntToDecimalStringN(gStringVar1, minutes, STR_CONV_MODE_RIGHT_ALIGN, 2); ConvertIntToDecimalStringN(gStringVar2, seconds, STR_CONV_MODE_RIGHT_ALIGN, 2); ConvertIntToDecimalStringN(gStringVar3, centiseconds, STR_CONV_MODE_LEADING_ZEROS, 2); });
        StringExpandPlaceholders(gStringVar4, gText_XMinYZSec);
...
void ResetTrainerTowerResults(void)
{
    ...
    for (i = 0; i < 4; i++)
    {
        SetTrainerTowerRecordTime(&gSaveBlock1Ptr->trainerTower[i].bestTime, 215999);
    }
}
