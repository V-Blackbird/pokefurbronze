...
bool8 EnterHallOfFame(void)
{
    ...
    HealPlayerParty();
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x2C)) == 1)
    {
    ...
    gaveAtLeastOneRibbon = 0;
    for (i = 0, r7 = &ribbonState; i < 6; i++)
    {
        if (GetMonData(&gPlayerParty[i], 5) && !GetMonData(&gPlayerParty[i], 6))
        {
            if (!GetMonData(&gPlayerParty[i], 67))
            {
                *r7 = 1;
                SetMonData(&gPlayerParty[i], 67, &ribbonState);
                gaveAtLeastOneRibbon = 1;
            }
        }
    }
    if (gaveAtLeastOneRibbon == 1)
    {
