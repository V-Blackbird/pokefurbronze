...
# 13 "src/itemfinder.c" 2
# 1 "include/constants/songs.h" 1
# 14 "src/itemfinder.c" 2
...
static const union AffineAnimCmd sAffineAnim_Left[] = {
    {.frame = {.xScale = 0, .yScale = 0, .rotation = 0x00, .duration = 1}},
    {.type = 0x7FFF}
};

static const union AffineAnimCmd sAffineAnim_Down[] = {
    {.frame = {.xScale = 0, .yScale = 0, .rotation = 0x40, .duration = 1}},
    {.type = 0x7FFF}
};

static const union AffineAnimCmd sAffineAnim_Right[] = {
    {.frame = {.xScale = 0, .yScale = 0, .rotation = 0x80, .duration = 1}},
    {.type = 0x7FFF}
};

static const union AffineAnimCmd sAffineAnim_Up[] = {
    {.frame = {.xScale = 0, .yScale = 0, .rotation = 0xc0, .duration = 1}},
    {.type = 0x7FFF}
};
...
# 131 "src/itemfinder.c"
void ItemUseOnFieldCB_Itemfinder(u8 taskId)
{
    u8 i;
    for (i = 0; i < 16; i++)
        gTasks[taskId].data[i] = 0;
    if (HiddenItemIsWithinRangeOfPlayer(gMapHeader.events, taskId) == 1)
    {
...
static void Task_ItemfinderResponseSoundsAndAnims(u8 taskId)
{
    ...
    if (data[3] % 25 == 0)
    {
        ...
        {
            PlaySE(65);
            CreateArrowSprite(data[5], direction);
            data[5]++;
            data[4]--;
        }
    }
...
static bool8 HiddenItemIsWithinRangeOfPlayer(const struct MapEvents * events, u8 taskId)
{
    ...
    for (i = 0; i < events->bgEventCount; i++)
    {
        if (events->bgEvents[i].kind == 7 && !FlagGet(GetHiddenItemAttr(events->bgEvents[i].bgUnion.hiddenItem, HIDDEN_ITEM_FLAG)))
        {
            ...
            if (GetHiddenItemAttr(events->bgEvents[i].bgUnion.hiddenItem, HIDDEN_ITEM_UNDERFOOT) == 1)
            {
                if (dx == 0 && dy == 0)
                {
...
static bool8 HiddenItemAtPos(const struct MapEvents * events, s16 x, s16 y)
{
    ...
    for (i = 0; i < bgEventCount; i++)
    {
        ...
        )
        {
            ...
            if (GetHiddenItemAttr(bgEvents[i].bgUnion.hiddenItem, HIDDEN_ITEM_UNDERFOOT) != 1 && !FlagGet(eventFlag))
                return 1;
            else
                return 0;
        }
    ...
    for (curX = x - 7; curX <= x + 7; curX++)
    {
        for (curY = y - 5; curY <= y + 5; curY++)
        {
                ...
                || curY >= height)
            {
                ...
                if (conn != ((void *)0) && HiddenItemInConnectedMapAtPos(conn, curX, curY) == 1)
...
static void RegisterHiddenItemRelativeCoordsIfCloser(u8 taskId, s16 dx, s16 dy)
{
    ...
    else
    {
        ...
        {
            if (dx2 + dy2 == dx3 + dy3 && (dy2 > dy3 || (dy2 == dy3 && data[1] < dy)))
            {
                data[0] = dx;
                data[1] = dy;
            }
...
static void CreateArrowSprite(u8 animNum, u8 direction)
{
    ...
    case 0:
        ...
        {
        case 3:
            gSprites[spriteId].data[1] = -100;
            gSprites[spriteId].data[2] = 0;
            StartSpriteAffineAnim(&gSprites[spriteId], 0);
            break;
        case 2:
            gSprites[spriteId].data[1] = 0;
            gSprites[spriteId].data[2] = -100;
