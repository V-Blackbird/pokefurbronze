...
void InitEnigmaBerry(void)
{
    ...
    gSaveBlock1Ptr->enigmaBerry.berry = ({ const struct Berry2 * berries = (const struct Berry2 *)gBerries; berries[176 - 134]; });
    for (i = 0; i < 18; i++)
...
void ClearEnigmaBerries(void)
{
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, &gSaveBlock1Ptr->enigmaBerry, 0x00000000 | 0x01000000 | ((sizeof(gSaveBlock1Ptr->enigmaBerry))/(16/8) & 0x1FFFFF)); };
    InitEnigmaBerry();
}
...
static u32 GetEnigmaBerryChecksum(struct EnigmaBerry * enigmaBerry)
{
    ...
    for (i = 0; i < ((size_t)&((struct EnigmaBerry *)0)->checksum); i++)
        result += src[i];
...
{
    if (berryIdx == (((176 - 134) + 1)) && IsEnigmaBerryValid())
        return (struct Berry *)&gSaveBlock1Ptr->enigmaBerry.berry;

    if (berryIdx == 0 || berryIdx > (((176 - 134) + 1)))
        berryIdx = 1;
...
{
    if (itemId - 134 < 0 || itemId - 134 > 176 - 134)
        return 1;
...
{
    if (berryType - 1 < 0 || berryType - 1 > 176 - 134)
        return 134;

    return berryType + 134 - 1;
}
