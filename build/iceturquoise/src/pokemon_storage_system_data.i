# 0 "src/pokemon_storage_system_data.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokemon_storage_system_data.c"
...
# 2 "src/pokemon_storage_system_data.c" 2
# 1 "include/gflib.h" 1
...
# 3 "src/pokemon_storage_system_data.c" 2
# 1 "include/data.h" 1
# 9 "include/data.h"
struct MonCoords
{
...
# 4 "src/pokemon_storage_system_data.c" 2
# 1 "include/item.h" 1
...
# 5 "src/pokemon_storage_system_data.c" 2
...
# 6 "src/pokemon_storage_system_data.c" 2
# 1 "include/menu.h" 1
# 18 "include/menu.h"
struct MenuAction
{
...
# 7 "src/pokemon_storage_system_data.c" 2
...
# 8 "src/pokemon_storage_system_data.c" 2
...
# 9 "src/pokemon_storage_system_data.c" 2
...
# 10 "src/pokemon_storage_system_data.c" 2
...
extern const u8 gText_PokeSum_EggOrigin_Trade[];
# 11 "src/pokemon_storage_system_data.c" 2
# 1 "include/constants/items.h" 1
# 12 "src/pokemon_storage_system_data.c" 2
# 1 "include/constants/moves.h" 1
# 13 "src/pokemon_storage_system_data.c" 2
# 1 "include/constants/songs.h" 1
# 14 "src/pokemon_storage_system_data.c" 2

static __attribute__((section("ewram_data"))) struct Pokemon sMonBeingCarried = {};
static __attribute__((section("ewram_data"))) s8 sCursorArea = 0;
static __attribute__((section("ewram_data"))) s8 sCursorPosition = 0;
static __attribute__((section("ewram_data"))) bool8 sIsMonBeingMoved = 0;
static __attribute__((section("ewram_data"))) u8 sMovingMonOrigBoxId = 0;
static __attribute__((section("ewram_data"))) u8 sMovingMonOrigBoxPos = 0;
static __attribute__((section("ewram_data"))) bool8 sInMultiMoveMode = 0;
static __attribute__((section("ewram_data"))) u8 sSavedCursorPosition = 0;
...
static u16 GetSpeciesAtCursorPosition(void)
{
    ...
    {
    case CURSOR_AREA_IN_PARTY:
        return GetMonData(&gPlayerParty[sCursorPosition], 11);
    case CURSOR_AREA_IN_BOX:
        return GetCurrentBoxMonData(sCursorPosition, 11);
    default:
        return 0;
    }
}
...
bool8 UpdateCursorPos(void)
{
    ...
    else if (--gStorage->cursorMoveSteps != 0)
    {
        ...
        if (gStorage->cursorSprite->x > 240 + 16)
        {
            tmp = gStorage->cursorSprite->x - (240 + 16);
        ...
        if (gStorage->cursorSprite->x < 64)
        {
            ...
            gStorage->cursorSprite->x = 240 + 16 - tmp;
        }


        if (gStorage->cursorSprite->y > 160 + 16)
        {
            tmp = gStorage->cursorSprite->y - (160 + 16);
        ...
        if (gStorage->cursorSprite->y < -16)
        {
            ...
            gStorage->cursorSprite->y = 160 + 16 - tmp;
        }


        if (gStorage->cursorFlipTimer && --gStorage->cursorFlipTimer == 0)
            gStorage->cursorSprite->vFlip = (gStorage->cursorSprite->vFlip == 0);
    }
    else
    {
...
static void DoCursorNewPosUpdate(void)
{
    ...
    case CURSOR_AREA_IN_BOX:
        if (gStorage->inBoxMovingMode == MOVE_MODE_NORMAL)
        {
            ...
            gStorage->cursorShadowSprite->invisible = 0;
            SetMovingMonPriority(2);
        }
        break;
    }
...
static void MoveMon(void)
{
    ...
    {
    case CURSOR_AREA_IN_PARTY:
        SetMovedMonData(14, sCursorPosition);
...
void InitCanReleaseMonVars(void)
{
    ...
    gStorage->isSurfMon = 0;
    gStorage->isDiveMon = 0;
    gStorage->restrictedMoveList[0] = 57;
    gStorage->restrictedMoveList[1] = 291;
    gStorage->restrictedMoveList[2] = 355;
    knownMoveFlags = GetMonData(&gStorage->tempMon, 81, (u8 *)gStorage->restrictedMoveList);
    gStorage->isSurfMon = knownMoveFlags & 1;
    gStorage->isDiveMon = (knownMoveFlags >> 1) & 1;
    if (gStorage->isSurfMon || gStorage->isDiveMon)
        gStorage->releaseMonStatusResolved = 0;
    else
    {
        gStorage->releaseMonStatusResolved = 1;
...
s8 RunCanReleaseMon(void)
{
    ...
    case 0:
        for (i = 0; i < 6; i++)
        {
            if (gStorage->releaseBoxId != 14 || gStorage->releaseBoxPos != i)
            {
                knownMoveFlags = GetMonData(&gPlayerParty[i], 81, (u8 *)gStorage->restrictedMoveList);
                if (knownMoveFlags & 1)
                    gStorage->isSurfMon = 0;
                if (knownMoveFlags & 2)
                    gStorage->isDiveMon = 0;
    ...
    case 1:
        ...
        for (i = 0; i < 5; i++)
        {
            knownMoveFlags = GetAndCopyBoxMonDataAt(gStorage->releaseCheckBoxId, gStorage->releaseCheckBoxPos, 81, (u8 *)gStorage->restrictedMoveList);
            if (knownMoveFlags != 0
            ...
            if (++gStorage->releaseCheckBoxPos >= (5 * 6))
            {
                gStorage->releaseCheckBoxPos = 0;
                if (++gStorage->releaseCheckBoxId >= 14)
                {
                    gStorage->releaseMonStatusResolved = 1;
...
void InitSummaryScreenData(void)
{
    ...
    else
    {
        ...
        gStorage->summaryCursorPos = sCursorPosition;
        gStorage->summaryLastIndex = (5 * 6) - 1;
...
void SetMonMarkings(u8 markings)
{
    ...
    if (sIsMonBeingMoved)
        SetMonData(&gStorage->movingMon, 8, &markings);
    else
    {
        if (sCursorArea == CURSOR_AREA_IN_PARTY)
            SetMonData(gPlayerParty + sCursorPosition, 8, &markings);
        if (sCursorArea == CURSOR_AREA_IN_BOX)
            SetCurrentBoxMonData(sCursorPosition, 8, &markings);
    }
}
...
bool8 CanShiftMon(void)
{
    if (sIsMonBeingMoved)
    {
        ...
        {
            if (gStorage->displayMonIsEgg || GetMonData(&gStorage->movingMon, 57) == 0)
                return 0;
        }
        return 1;
    }
...
static void TrySetDisplayMonData(void)
{
    gStorage->setMosaic = (sIsMonBeingMoved == 0);
    if (!sIsMonBeingMoved)
    {
        switch (sCursorArea)
        {
        ...
        case CURSOR_AREA_BUTTONS:
        case CURSOR_AREA_BOX_TITLE:
            SetDisplayMonData(((void *)0), MODE_MOVE);
            break;
        case CURSOR_AREA_IN_BOX:
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    if (mode == MODE_PARTY)
    {
        ...
        gStorage->displayMonSpecies = GetMonData(mon, 65);
        if (gStorage->displayMonSpecies != 0)
        {
            sanityIsBagEgg = GetMonData(mon, 4);
            if (sanityIsBagEgg)
                gStorage->displayMonIsEgg = 1;
            else
                gStorage->displayMonIsEgg = GetMonData(mon, 45);

            GetMonData(mon, 2, gStorage->displayMonNickname);
            StringGet_Nickname(gStorage->displayMonNickname);
            gStorage->displayMonLevel = GetMonData(mon, 56);
            gStorage->displayMonMarkings = GetMonData(mon, 8);
            gStorage->displayMonPersonality = GetMonData(mon, 0);
            gStorage->displayMonPalette = GetMonFrontSpritePal(mon);
            gender = GetMonGender(mon);
            gStorage->displayMonItemId = GetMonData(mon, 12);
        }
    }
    else if (mode == MODE_BOX)
    {
        ...
        gStorage->displayMonSpecies = GetBoxMonData(pokemon, 65);
        if (gStorage->displayMonSpecies != 0)
        {
            ...
            if (sanityIsBagEgg)
                ...
                gStorage->displayMonIsEgg = GetBoxMonData(boxMon, 45);

            GetBoxMonData(boxMon, 2, gStorage->displayMonNickname);
            StringGet_Nickname(gStorage->displayMonNickname);
            gStorage->displayMonLevel = GetLevelFromBoxMonExp(boxMon);
            gStorage->displayMonMarkings = GetBoxMonData(boxMon, 8);
            gStorage->displayMonPersonality = GetBoxMonData(boxMon, 0);
            ...
            gStorage->displayMonItemId = GetBoxMonData(boxMon, 12);
        }
    }
    else
    {
        gStorage->displayMonSpecies = 0;
        gStorage->displayMonItemId = 0;
    }

    if (gStorage->displayMonSpecies == 0)
    {
        StringFill(gStorage->displayMonNickname, 0x00, 5);
        StringFill(gStorage->displayMonTexts[0], 0x00, 8);
        StringFill(gStorage->displayMonTexts[1], 0x00, 8);
        StringFill(gStorage->displayMonTexts[2], 0x00, 8);
        StringFill(gStorage->displayMonTexts[3], 0x00, 8);
    }
    else if (gStorage->displayMonIsEgg)
    {
        if (sanityIsBagEgg)
            StringCopyPadded(gStorage->displayMonTexts[0], gStorage->displayMonNickname, 0x00, 5);
        else
            StringCopyPadded(gStorage->displayMonTexts[0], gText_EggNickname, 0x00, 8);
    ...
    {
        if (gStorage->displayMonSpecies == 29 || gStorage->displayMonSpecies == 32)
            gender = 0xFF;
        ...
        txtPtr = gStorage->displayMonTexts[1];
        *(txtPtr)++ = 0xBA;
        StringCopyPadded(txtPtr, gSpeciesNames[gStorage->displayMonSpecies], 0x00, 5);


        txtPtr = gStorage->displayMonTexts[2];
        *(txtPtr)++ = 0xFC;
        *(txtPtr)++ = 0x04;
        switch (gender)
        {
        ...
        *(txtPtr++) = 0x05;
        ...
        if (gStorage->displayMonItemId != 0)
            StringCopyPadded(gStorage->displayMonTexts[3], ItemId_GetName(gStorage->displayMonItemId), 0x00, 8);
        else
            StringFill(gStorage->displayMonTexts[3], 0x00, 8);
    }
}
...
static u8 HandleInput_InBox_Normal(void)
{
    ...
    do
    {
        ...
        if ((({(gMain.newKeys) & (0x0001);})) && SetSelectionMenuTexts())
        {
            if (!sInMultiMoveMode)
                return INPUT_IN_MENU;

            if (gStorage->boxOption != OPTION_MOVE_MONS || sIsMonBeingMoved == 1)
            {
...
static u8 HandleInput_InBox_GrabbingMultiple(void)
{
    ...
    else
    {
        ...
        {
            sIsMonBeingMoved = (gStorage->displayMonSpecies != 0);
...
static u8 HandleInput_InParty(void)
{
    ...
    do
    {
        ...
        else if (({(gMain.newAndRepeatedKeys) & (0x0020);}) && sCursorPosition != 0)
        {
            input = INPUT_MOVE_CURSOR;
...
static bool8 SetMenuTextsForItem(void)
{
    if (gStorage->displayMonSpecies == 479)
        return 0;
    ...
    else
    {
        ...
        {
            if (ItemIsMail(gStorage->displayMonItemId) == 1)
                return 0;
...
static void CreateCursorSprites(void)
{
    ...
    if (spriteId != 64)
    {
        ...
        gStorage->cursorShadowSprite = ((void *)0);
}
...
void SetMenuText(u8 textId)
{
    if (gStorage->menuItemsCount < (sizeof(gStorage->menuItems) / sizeof((gStorage->menuItems)[0])))
    {
        u8 len;
...
void AddMenu(void)
{
    ...
    DrawStdFrameWithCustomTileAndPalette(gStorage->menuWindowId, 0, 11, 14);
...
void RemoveMenu(void)
{
    ClearStdWindowAndFrameToTransparent(gStorage->menuWindowId, 1);
