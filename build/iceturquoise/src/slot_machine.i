...
# 29 "src/slot_machine.c"
enum {
    PALSLOT_LINE_NORMAL = 4,
    PALSLOT_LINE_BET,
    PALSLOT_LINE_MATCH,
};
...
static __attribute__((section("ewram_data"))) struct SlotMachineState * sSlotMachineState = ((void *)0);
static __attribute__((section("ewram_data"))) struct SlotMachineGfxManager * sSlotMachineGfxManager = ((void *)0);
...
static const u8 sThirdReelBiasCheckIndices[5][3] = {
    {0x00, 0x03, 0x06},
    {0x01, 0x04, 0x07},
    {0x02, 0x05, 0x08},
    {0x00, 0x04, 0x08},
...
static const u8 sRowAttributes[5][4] = {
    {0x00, 0x04, 0x08, 0x03},
    {0x00, 0x03, 0x06, 0x02},
    {0x01, 0x04, 0x07, 0x01},
    {0x02, 0x05, 0x08, 0x02},
...
static const u8 sReelIconAnimByReelAndPos[3][21] = {
    {
        ICON_7,
        ICON_PSYDUCK,
        ICON_CHERRIES,
...
static const union AffineAnimCmd sAffineAnimCmd_ReelIcons_Unused[] = {
    {.frame = {.xScale = 0x100, .yScale = 0x100, .rotation = 0, .duration = 1}},
    {.type = 0x7FFF}
};
...
static const struct LineStateTileIdxList sLineStateTileIdxs[5] = {
    { sLineTiles_TLBR, (sizeof(sLineTiles_TLBR) / sizeof((sLineTiles_TLBR)[0])) },
    { sLineTiles_TopRow, (sizeof(sLineTiles_TopRow) / sizeof((sLineTiles_TopRow)[0])) },
    { sLineTiles_MiddleRow, (sizeof(sLineTiles_MiddleRow) / sizeof((sLineTiles_MiddleRow)[0])) },
    { sLineTiles_BottomRow, (sizeof(sLineTiles_BottomRow) / sizeof((sLineTiles_BottomRow)[0])) },
    { sLineTiles_BLTR, (sizeof(sLineTiles_BLTR) / sizeof((sLineTiles_BLTR)[0])) }
};
...
static const u16 sReelButtonMapTileIdxs[3][4] = {
...
void PlaySlotMachine(u16 machineIdx, MainCallback savedCallback)
{
    ...
    if (sSlotMachineState == ((void *)0))
        SetMainCallback2(savedCallback);
    else
    {
        if (machineIdx >= (sizeof(sReelBiasChances) / sizeof((sReelBiasChances)[0])))
            machineIdx = 0;
...
static void InitSlotMachineState(struct SlotMachineState * ptr)
{
    ...
    for (i = 0; i < 3; i++)
    {
        sSlotMachineState->reelIsSpinning[i] = 0;
...
static void MainTask_SlotsGameLoop(u8 taskId)
{
    ...
    case 0:
        ...
        else if (({(gMain.newKeys) & (0x0001);}) && sSlotMachineState->bet != 0)
        {
            data[0] = 2;
        }
            ...
            if (sSlotMachineState->reelSubpixel[i] != 0 || sSlotMachineState->reelPositions[i] != sSlotMachineState->destReelPos[i])
            {
                ...
                if (sSlotMachineState->reelSubpixel[i] > 2)
                {
                    ...
                    if (sSlotMachineState->reelPositions[i] < 0)
                        sSlotMachineState->reelPositions[i] = 21 - 1;
                }
...
static void StartReels(void)
{
    ...
    for (i = 0; i < 3; i++)
        sSlotMachineState->reelIsSpinning[i] = 1;
}
...
static bool32 TestReelIconAttribute(s32 attr, s32 icon)
{
    ...
    case PAYOUT_CHERRIES2:
    case PAYOUT_CHERRIES3:
        return icon == ICON_CHERRIES ? 1 : 0;
    case PAYOUT_MAGSHELL:
        return icon == ICON_MAGNEMITE || icon == ICON_SHELLDER ? 1 : 0;
    case PAYOUT_PIKAPSY:
        return icon == ICON_PIKACHU || icon == ICON_PSYDUCK ? 1 : 0;
    case PAYOUT_ROCKET:
        return icon == ICON_ROCKET ? 1 : 0;
    case PAYOUT_7:
        return icon == ICON_7 ? 1 : 0;
    default:
        return 0;
    }
}
...
static void CalcSlotBias(void)
{
    ...
    if (sSlotMachineState->machineBias < PAYOUT_ROCKET)
    {
        ...
        if (sSlotMachineState->biasCooldown != 0)
        {
            if (i == 0 && (Random() & 0x3FFF) < (int)(0.7 * 0x3FFF))
                sSlotMachineState->biasCooldown = (Random() & 1) ? 5 : 60;
...
static u16 CalcPayout(void)
{
    ...
    for (i = 0; i < 5; i++)
        sSlotMachineState->winFlags[i] = 0;
    ...
    for (i = 0; i < 5; i++)
    {
        if (sSlotMachineState->bet >= sRowAttributes[i][ROWATTR_MINBET])
        {
            ...
            if (curMatch != 0)
            {
                sSlotMachineState->winFlags[i] = 1;
...
static bool32 LoadSpriteGraphicsAndAllocateManager(void)
{
    ...
    for (i = 0; i < (sizeof(sSpriteSheets) / sizeof((sSpriteSheets)[0])); i++)
        LoadCompressedSpriteSheet(&sSpriteSheets[i]);
    LoadSpritePalettes(sSpritePalettes);
    sSlotMachineGfxManager = Alloc(sizeof(*sSlotMachineGfxManager));
    if (sSlotMachineGfxManager == ((void *)0))
        return 0;
...
static void InitGfxManager(struct SlotMachineGfxManager * manager)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        for (j = 0; j < 5; j++)
            manager->reelIconSprites[i][j] = ((void *)0);
    }
}
...
static void CreateReelIconSprites(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        for (j = 0; j < 5; j++)
        {
            ...
            sSlotMachineGfxManager->reelIconAffineParamPtr = (vu16 *)(0x7000000 + 0 * sizeof(struct OamData) + ((size_t)&((struct OamData *)0)->affineParam));
        }
    }
}
...
{
    s32 vcount = (*(vu16 *)(0x4000000 + 0x6)) - 0x2B;
    if (vcount < 0x54u)
    {
        *sSlotMachineGfxManager->reelIconAffineParamPtr = sReelIconAffineParams[vcount];
        (*(vu16 *)(0x4000000 + 0x54)) = sReelIconBldY[vcount];
    }
    else
    {
...
static void CreateClefairySprites(void)
{
    ...
    spriteId = CreateSprite(&sSpriteTemplate_Clefairy, 240 - 16, 136, 1);
...
{
    s32 i;
    for (i = 0; i < (int)(sizeof(sSlotMachineGfxManager->clefairySprites) / sizeof((sSlotMachineGfxManager->clefairySprites)[0])); i++)
...
static bool32 CreateSlotMachine(void)
{
    ...
    if (ptr == ((void *)0))
        return 0;
    for (i = 0; i < (int)(sizeof(ptr->tasks) / sizeof((ptr->tasks)[0])); i++)
        ptr->tasks[i].active = 0;
    ptr->yesNoMenuActive = 0;
...
static bool8 SlotsTask_GraphicsInit(u8 * state, struct SlotMachineSetupTaskData * ptr)
{
    ...
    case 1:
        ...
        RequestDma3Fill(0, (void *)0x7000000, 0x400, 1);
        RequestDma3Fill(0, (void *)0x6000000, 0x20, 1);
        RequestDma3Fill(0, (void *)(0x6000000 + 0xC000), 0x20, 1);
        SetGpuReg(0x0, 0);
        ResetBgPositions();
        ...
        LoadPalette(sBgPal_MatchLines, (0x000 + ((PALSLOT_LINE_BET) * 16)), sizeof(sBgPal_MatchLines));
        LoadPalette(sCombosWindow_Pal, (0x000 + ((7) * 16)), sizeof(sCombosWindow_Pal));
        SetBackdropColor(((30) | ((30) << 5) | ((31) << 10)), pal);
        LoadUserWindowGfx2(0, 0x00A, (0x000 + ((13) * 16)));
        LoadStdWindowGfxOnBg(0, 0x001, (0x000 + ((15) * 16)));
        ...
        LoadPalette(GetTextWindowPalette(2), (0x000 + ((14) * 16)), ((16) * sizeof(u16)));
        FillWindowPixelBuffer(1, 0xFF);
        PutWindowTilemap(1);

        x = 240 - 4 - GetStringWidth(FONT_SMALL, gString_SlotMachineControls, 0);
        textColor[0] = 0xF;
        textColor[1] = 0x1;
        textColor[2] = 0x2;
        ...
        SetGpuRegBits(0x0, 0x0000 | 0x20 | 0x0040 | 0x1000);
        SetGpuReg(0x50, (1 << 3) | (1 << 4) | (1 << 5) | (3 << 6));
...
static bool8 SlotsTask_FadeOut(u8 * state, struct SlotMachineSetupTaskData * ptr)
{
    ...
    {
    case 0:
        BeginNormalPaletteFade((0x0000FFFF | 0xFFFF0000), -1, 0, 16, 0);
        (*state)++;
        break;
    case 1:
        if (!gPaletteFade.active)
...
static bool8 SlotsTask_ShowHelp(u8 * state, struct SlotMachineSetupTaskData * ptr)
{
    ...
    case 0:
        ...
        SetGpuReg(0x40, (((0) << 8) | (0)));
        SetGpuReg(0x42, (((0) << 8) | (160)));
        ShowBg(1);
        PlaySE(6);
        ptr->bg1X = 0;
        (*state)++;
...
static void Slot_PrintOnWindow0(const u8 * str)
{
    ...
    AddTextPrinterParameterized5(0, FONT_NORMAL, str, 1, 2, 0xFF, ((void *)0), 1, 2);
}
...
static void Task_FlashWinningLine(u8 taskId)
{
    ...
    {
    case 0:
        LoadPalette(sBgPal_MatchLines, (0x000 + ((PALSLOT_LINE_MATCH) * 16)), sizeof(sBgPal_MatchLines));
        for (i = 0; i < 5; i++)
        {
    ...
    case 1:
        if (data[1] == 0)
        {
            ...
            LoadPalette(&sBgPal_PayoutLight[y], (0x000 + ((1) * 16)), sizeof(sBgPal_PayoutLight[0]));
            data[2] += 32;
            data[2] &= 0x7F;
            data[1] = 8;
        }
        ...
        if (data[3] == 0)
        {
            ...
            BlendPalettes(1 << PALSLOT_LINE_MATCH, data[5], ((0) | ((0) << 5) | ((0) << 10)));
        }
        else
        {
            data[4]++;
            if (data[4] > 1)
            {
                ...
                data[5]++;
                data[5] &= 1;
                BlendPalettes(1 << PALSLOT_LINE_MATCH, data[5] * 8, ((0) | ((0) << 5) | ((0) << 10)));
            }
        }

        for (i = 0; i < (sizeof(sWinningLineFlashPalIdxs) / sizeof((sWinningLineFlashPalIdxs)[0])); i++)
            gPlttBufferFaded[sWinningLineFlashPalIdxs[i] + (0x000 + ((PALSLOT_LINE_MATCH) * 16))] = gPlttBufferUnfaded[sWinningLineFlashPalIdxs[i] + (0x000 + ((PALSLOT_LINE_MATCH) * 16))];
        break;
    case 2:
        ...
        LoadPalette(&sBg_Pal[1], (0x000 + ((1) * 16)), sizeof(sBg_Pal[1]));
...
static void Slot_CreateYesNoMenu(u8 cursorPos)
{
    ...
    GetSlotMachineSetupTaskDataPtr()->yesNoMenuActive = 1;
}
