# 0 "src/mystery_gift_menu.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/mystery_gift_menu.c"
...
# 9 "src/mystery_gift_menu.c" 2
# 1 "include/title_screen.h" 1



void CB2_InitTitleScreen(void);
# 10 "src/mystery_gift_menu.c" 2
...
# 14 "src/mystery_gift_menu.c" 2
# 1 "include/link.h" 1
# 15 "src/mystery_gift_menu.c" 2
...
void ClearTempFieldEventData(void);
# 16 "src/mystery_gift_menu.c" 2
# 1 "include/mystery_gift_server.h" 1
# 17 "src/mystery_gift_menu.c" 2
...
# 22 "src/mystery_gift_menu.c" 2
# 1 "include/constants/cable_club.h" 1
# 23 "src/mystery_gift_menu.c" 2
# 1 "include/constants/songs.h" 1
# 24 "src/mystery_gift_menu.c" 2
# 1 "include/constants/union_room.h" 1
# 25 "src/mystery_gift_menu.c" 2
...
__attribute__((aligned(4))) static const u8 sTextColors_TopMenu[3] = { 0, 1, 2 };
__attribute__((aligned(4))) static const u8 sTextColors_TopMenu_Copy[3] = { 0, 1, 2 };
__attribute__((aligned(4))) static const u8 sMG_Ereader_TextColor_2[3] = { 1, 2, 3 };
...
bool32 HandleMysteryGiftOrEReaderSetup(s32 isEReader)
{
    ...
    case 0:
        ...
        LoadUserWindowGfx2(0, 10, (0x000 + ((14) * 16)));
        LoadStdWindowGfxOnBg(0, 1, (0x000 + ((15) * 16)));
        ...
        break;
    case 1:
        LoadPalette(sTextboxBorder_Pal, (0x000 + ((0) * 16)), sizeof(sTextboxBorder_Pal));
        LoadPalette(GetTextWindowPalette(2), (0x000 + ((13) * 16)), ((16) * sizeof(u16)));
        ...
        PrintMysteryGiftOrEReaderTopMenu(isEReader, 0);
        gMain.state++;
        break;
    case 2:
...
void PrintMysteryGiftOrEReaderTopMenu(bool8 isEReader, bool32 useCancel)
{
    ...
    if (!isEReader)
    {
        options = useCancel == 1 ? gText_PickOKExit : gText_PickOKCancel;
...
bool32 PrintMysteryGiftMenuMessage(u8 *textState, const u8 *str)
{
    ...
    case 0:
        ...
        goto inc;
    case 1:
        DrawDownArrow(1, 208, 20, 1, 0, &sDownArrowCounterAndYCoordIdx[0], &sDownArrowCounterAndYCoordIdx[1]);
        ...
        break;
    case 2:
        DrawDownArrow(1, 208, 20, 1, 1, &sDownArrowCounterAndYCoordIdx[0], &sDownArrowCounterAndYCoordIdx[1]);
        *textState = 0;
        ClearTextWindow();
        return 1;
    case 0xFF:
...
static u32 MysteryGift_HandleThreeOptionMenu(u8 * unused0, u16 * unused1, u8 whichMenu)
{
    ...
    response = DoMysteryGiftListMenu(&windowTemplate, &listMenuTemplate, 1, 0x00A, (0x000 + ((14) * 16)));
    if (response != -1)
    {
...
static s32 HandleMysteryGiftListMenu(u8 * textState, u16 * windowId, bool32 cannotToss, bool32 cannotSend)
{
    ...
    case 1:
        ...
        {
            if (!cannotToss)
                input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_2Options, &sListMenu_ReceiveToss, 1, 0x00A, (0x000 + ((14) * 16)));
            else
                input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_1Option, &sListMenu_Receive, 1, 0x00A, (0x000 + ((14) * 16)));
        }
        else
        {
            if (!cannotToss)
                input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_3Options, &sListMenu_ReceiveSendToss, 1, 0x00A, (0x000 + ((14) * 16)));
            else
                input = DoMysteryGiftListMenu(&sWindowTemplate_GiftSelect_2Options, &sListMenu_ReceiveSend, 1, 0x00A, (0x000 + ((14) * 16)));
        }
        if (input != -1)
        {
            *textState = 0;
...
{
    if (!isWonderNews)
        return DoMysteryGiftYesNo(textState, windowId, 1, gText_IfThrowAwayCardEventWontHappen);
    else
        return DoMysteryGiftYesNo(textState, windowId, 1, gText_OkayToDiscardNews);
}
...
static void Task_MysteryGift(u8 taskId)
{
    ...
    case MG_STATE_CLIENT_YES_NO:
        input = DoMysteryGiftYesNo(&data->textState, &data->var, 0, MysteryGiftClient_GetMsg());
        switch (input)
        {
        case 0:
    ...
    case MG_STATE_CLIENT_ASK_TOSS:
        ...
        input = DoMysteryGiftYesNo(&data->textState, &data->var, 0, gText_ThrowAwayWonderCard);
        switch (input)
        {
        case 0:
            if (IsSavedWonderCardGiftNotReceived() == 1)
            {
    ...
    case MG_STATE_CLIENT_ASK_TOSS_UNRECEIVED:
        ...
        input = DoMysteryGiftYesNo(&data->textState, &data->var, 0, gText_HaventReceivedCardsGift);
        switch (input)
        {
        case 0:
            MysteryGiftClient_SetParam(0);
    ...
    case MG_STATE_HANDLE_GIFT_SELECT:
    {
        ...
        if (!data->isWonderNews)
        {
            if (IsSendingSavedWonderCardAllowed())
                result = HandleMysteryGiftListMenu(&data->textState, &data->var, data->isWonderNews, 0);
            else
                result = HandleMysteryGiftListMenu(&data->textState, &data->var, data->isWonderNews, 1);
        }
        else
        {
    ...
    case MG_STATE_ASK_TOSS:
        ...
        {
        case 0:
            if (!data->isWonderNews && IsSavedWonderCardGiftNotReceived() == 1)
    ...
    case MG_STATE_ASK_TOSS_UNRECEIVED:
        ...
        switch ((u32)DoMysteryGiftYesNo(&data->textState, &data->var, 1, gText_HaventReceivedGiftOkayToDiscard))
        {
        case 0:
            data->state = MG_STATE_TOSS;
            break;
    ...
    case MG_STATE_SERVER_RESULT_MSG:
        ...
        {
            if (data->sourceIsFriend == 1 && data->msgId == SVR_MSG_NEWS_SENT)
            {
