...
static void AnimUnusedIceCrystalThrow(struct Sprite *sprite)
{
    ...
    for (; (targetX >= -32 && targetX <= 240 + 32) && (targetY >= -32 && targetY <= 160 + 32);
            targetX += sprite->data[1], targetY += sprite->data[2])
        ;
    sprite->data[1] = -sprite->data[1];
    sprite->data[2] = -sprite->data[2];
    for (; (attackerX >= -32 && attackerX <= 240 + 32) && (attackerY >= -32 && attackerY <= 160 + 32);
...
static void AnimSwirlingSnowball_Step2(struct Sprite *sprite)
{
    s16 tempVar = GetBattlerSide(gBattleAnimAttacker) != 0 ? 20 : -20;
...
void AnimTask_HazeScrollingFog(u8 taskId)
{
    ...
    LoadPalette(&gDefaultWeatherSpritePalette, (0x000 + ((animBg.paletteId) * 16)), ((16) * sizeof(u16)));
    if (IsContest())
