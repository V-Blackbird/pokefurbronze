...
static __attribute__((section("ewram_data"))) const struct YesNoFuncTable *sYesNo = ((void *)0);
static __attribute__((section("ewram_data"))) TaskFunc sMessageNextTask = ((void *)0);
static __attribute__((section("ewram_data"))) u8 sMessageWindowId = {0};
...
void DisplayMessageAndContinueTask(u8 taskId, u8 windowId, u16 tileNum, u8 paletteNum, u8 fontId, u8 textSpeed, const u8 *string, void *taskFunc)
{
    ...
    DrawDialogFrameWithCustomTileAndPalette(windowId, 1, tileNum, paletteNum);

    if (string != gStringVar4)
        StringExpandPlaceholders(gStringVar4, string);

    gTextFlags.canABSpeedUpPrint = 1;
    AddTextPrinterParameterized2(windowId, fontId, gStringVar4, textSpeed, ((void *)0), 0x2, 0x1, 0x3);
...
bool8 IsHoldingItemAllowed(u16 itemId)
{
    ...
    if (itemId == 176
     && ((gSaveBlock1Ptr->location.mapGroup == ((1 | (0 << 8)) >> 8)
       && gSaveBlock1Ptr->location.mapNum == ((1 | (0 << 8)) & 0xFF))
       || InUnionRoom() == 1))
        return 0;
    else
        return 1;
...
bool8 IsWritingMailAllowed(u16 itemId)
{
    if ((IsUpdateLinkStateCBActive() == 1 || InUnionRoom() == 1) && ItemIsMail(itemId) == 1)
        return 0;
    else
        return 1;
}
...
void ResetVramOamAndBgCntRegs(void)
{
    ...
    { vu16 tmp = (vu16)(0); CpuSet((void *)&tmp, (void *) 0x5000000, 0x00000000 | 0x01000000 | (((0x200 + 0x200))/(16/8) & 0x1FFFFF)); };
}
...
bool8 AdjustQuantityAccordingToDPadInput(s16 *quantity_p, u16 qmax)
{
    ...
    else if (({(gMain.newAndRepeatedKeys) & (0x00F0);}) == 0x0010)
    {
        (*quantity_p) += 10;
        if ((*quantity_p) > qmax)
            (*quantity_p) = qmax;
    ...
    else if (({(gMain.newAndRepeatedKeys) & (0x00F0);}) == 0x0020)
    {
        (*quantity_p) -= 10;
        if ((*quantity_p) <= 0)
            (*quantity_p) = 1;
