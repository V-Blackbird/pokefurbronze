...
# 26 "src/battle_ai_script_commands.c"
enum
{
    AIState_SettingUp,
    AIState_Processing,
...
# 42 "src/battle_ai_script_commands.c"
static __attribute__((section("ewram_data"))) const u8 *sAIScriptPtr = ((void *)0);
...
void BattleAI_HandleItemUseBeforeAISetup(void)
{
    ...
    if ((gBattleTypeFlags & (1 << 3))
        ...
        && !(gBattleTypeFlags & ((1 << 19) | (1 << 11) | (1 << 8) | (1 << 7) | (1 << 1)))
        )
    {
        for (i = 0; i < 4; i++)
        {
...
void BattleAI_SetupAIData(void)
{
    ...
    {
        (gBattleResources->ai)->aiFlags = (1 << 30);
        return;
    }
    else if (gBattleTypeFlags & (1 << 10))
    {
        (gBattleResources->ai)->aiFlags = (1 << 29);
        return;
    }
    else if (!(gBattleTypeFlags & ((1 << 19) | (1 << 11) | (1 << 8))) && (gTrainerBattleOpponent_A != 0x400))
    {
        if (gBattleTypeFlags & (1 << 17))
        {
            (gBattleResources->ai)->aiFlags = (1 << 0);
            return;
        }
        else if (gBattleTypeFlags & (1 << 18))
        {
            (gBattleResources->ai)->aiFlags = ((1 << 0) | (1 << 2) | (1 << 1));
            return;
        }
    }
    else
...
u8 BattleAI_ChooseMoveOrAction(void)
{
    ...
    if ((gBattleResources->ai)->aiAction & 0x0002)
        return 4;
    if ((gBattleResources->ai)->aiAction & 0x0004)
        return 5;
...
static void BattleAI_DoAIProcessing(void)
{
    while ((gBattleResources->ai)->aiState != AIState_FinishedProcessing)
    {
        ...
        case AIState_SettingUp:
            sAIScriptPtr = gBattleAI_ScriptsTable[(gBattleResources->ai)->aiLogicId];

            if (gBattleMons[gBattlerAttacker].pp[(gBattleResources->ai)->movesetIndex] == 0)
            {
                (gBattleResources->ai)->moveConsidered = 0;
            }
            else
            {
                (gBattleResources->ai)->moveConsidered = gBattleMons[gBattlerAttacker].moves[(gBattleResources->ai)->movesetIndex];
            }
        ...
        case AIState_Processing:
            ...
            else
            {
                ...
                (gBattleResources->ai)->aiAction |= 0x0001;
            }
            if ((gBattleResources->ai)->aiAction & 0x0001)
            {
                (gBattleResources->ai)->movesetIndex++;

                if ((gBattleResources->ai)->movesetIndex < 4 && ((gBattleResources->ai)->aiAction & 0x0008) == 0)
                    (gBattleResources->ai)->aiState = AIState_SettingUp;
                else
                    (gBattleResources->ai)->aiState++;
                (gBattleResources->ai)->aiAction &= (0x0002 | 0x0004 | 0x0008 |
...
static void RecordLastUsedMoveByTarget(void)
{
    ...
    for (i = 0; i < 8; i++)
    {
        if ((gBattleResources->battleHistory)->usedMoves[gBattlerTarget >> 1][i] == 0)
        {
            (gBattleResources->battleHistory)->usedMoves[gBattlerTarget >> 1][i] = gLastMoves[gBattlerTarget];
            return;
        }
    }
}
...
static void ClearBattlerMoveHistory(u8 battlerId)
{
    ...
    for (i = 0; i < 8; i++)
        (gBattleResources->battleHistory)->usedMoves[battlerId / 2][i] = 0;
}

void RecordAbilityBattle(u8 battlerId, u8 abilityId)
{
    if (GetBattlerSide(battlerId) == 0)
        (gBattleResources->battleHistory)->abilities[((GetBattlerPosition(battlerId) & 1))] = abilityId;
}

void RecordItemEffectBattle(u8 battlerId, u8 itemEffect)
{
    if (GetBattlerSide(battlerId) == 0)
        (gBattleResources->battleHistory)->itemEffects[((GetBattlerPosition(battlerId) & 1))] = itemEffect;
}

static void Cmd_if_random_less_than(void)
{
    if (Random() % 256 < sAIScriptPtr[1])
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));
    else
        sAIScriptPtr += 6;
}
...
static void Cmd_score(void)
{
    (gBattleResources->ai)->score[(gBattleResources->ai)->movesetIndex] += sAIScriptPtr[1];
...
static void Cmd_if_hp_less_than(void)
{
    ...
    if ((u32)(100 * gBattleMons[battlerId].hp / gBattleMons[battlerId].maxHP) < sAIScriptPtr[2])
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 3)[0] | ((sAIScriptPtr + 3)[1] << 8) | ((sAIScriptPtr + 3)[2] << 16) | ((sAIScriptPtr + 3)[3] << 24));
    else
        sAIScriptPtr += 7;
}
...
static void Cmd_if_status(void)
{
    ...
    status = ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));

    if (gBattleMons[battlerId].status1 & status)
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 6)[0] | ((sAIScriptPtr + 6)[1] << 8) | ((sAIScriptPtr + 6)[2] << 16) | ((sAIScriptPtr + 6)[3] << 24));
    else
        sAIScriptPtr += 10;
}
...
static void Cmd_if_side_affecting(void)
{
    ...
    side = ((GetBattlerPosition(battlerId) & 1));
    status = ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));
...
static void Cmd_if_less_than(void)
{
    if ((gBattleResources->ai)->funcResult < sAIScriptPtr[1])
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));
    else
        sAIScriptPtr += 6;
}
...
static void Cmd_if_less_than_ptr(void)
{
    const u8 *value = (u8 *) ((sAIScriptPtr + 1)[0] | ((sAIScriptPtr + 1)[1] << 8) | ((sAIScriptPtr + 1)[2] << 16) | ((sAIScriptPtr + 1)[3] << 24));

    if ((gBattleResources->ai)->funcResult < *value)
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 5)[0] | ((sAIScriptPtr + 5)[1] << 8) | ((sAIScriptPtr + 5)[2] << 16) | ((sAIScriptPtr + 5)[3] << 24));
    else
        sAIScriptPtr += 9;
}
...
static void Cmd_if_move(void)
{
    u16 move = ((sAIScriptPtr + 1)[0] | ((sAIScriptPtr + 1)[1] << 8));
...
static void Cmd_if_in_bytes(void)
{
    const u8 *ptr = (u8 *) ((sAIScriptPtr + 1)[0] | ((sAIScriptPtr + 1)[1] << 8) | ((sAIScriptPtr + 1)[2] << 16) | ((sAIScriptPtr + 1)[3] << 24));
...
static void Cmd_if_in_hwords(void)
{
    const u16 *ptr = (const u16 *)(u8 *) ((sAIScriptPtr + 1)[0] | ((sAIScriptPtr + 1)[1] << 8) | ((sAIScriptPtr + 1)[2] << 16) | ((sAIScriptPtr + 1)[3] << 24));
...
static void Cmd_if_user_has_attacking_move(void)
{
    ...
    if (i == 4)
        ...
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 1)[0] | ((sAIScriptPtr + 1)[1] << 8) | ((sAIScriptPtr + 1)[2] << 16) | ((sAIScriptPtr + 1)[3] << 24));
}
...
static void Cmd_get_type(void)
{
    ...
    {
    case 1:
        (gBattleResources->ai)->funcResult = gBattleMons[gBattlerAttacker].type1;
        break;
    case 0:
        (gBattleResources->ai)->funcResult = gBattleMons[gBattlerTarget].type1;
        break;
    case 3:
        (gBattleResources->ai)->funcResult = gBattleMons[gBattlerAttacker].type2;
        break;
    case 2:
        (gBattleResources->ai)->funcResult = gBattleMons[gBattlerTarget].type2;
        break;
    case 4:
...
static void Cmd_get_how_powerful_move_is(void)
{
    ...
    {
        if (gBattleMoves[(gBattleResources->ai)->moveConsidered].effect == sDiscouragedPowerfulMoveEffects[i])
            break;
    }
        ...
        && sDiscouragedPowerfulMoveEffects[i] == 0xFFFF)
    {
        ...
        for (checkedMove = 0; checkedMove < 4; checkedMove++)
        {
            ...
            if (gBattleMons[gBattlerAttacker].moves[checkedMove] != 0
                && sDiscouragedPowerfulMoveEffects[i] == 0xFFFF
                && gBattleMoves[gBattleMons[gBattlerAttacker].moves[checkedMove]].power > 1)
            {
                ...
                moveDmgs[checkedMove] = gBattleMoveDamage * (gBattleResources->ai)->simulatedRNG[checkedMove] / 100;
...
{
    if (sAIScriptPtr[1] == 1)
        (gBattleResources->ai)->funcResult = gLastMoves[gBattlerAttacker];
    else
...
static void Cmd_if_equal_(void)
{
    if (sAIScriptPtr[1] == (gBattleResources->ai)->funcResult)
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));
    else
        sAIScriptPtr += 6;
}
...
static void Cmd_if_would_go_first(void)
{
    if (GetWhoStrikesFirst(gBattlerAttacker, gBattlerTarget, 1) == sAIScriptPtr[1])
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));
    else
        sAIScriptPtr += 6;
}
...
static void Cmd_count_alive_pokemon(void)
{
    ...
    if (gBattleTypeFlags & (1 << 0))
    {
        ...
        position = GetBattlerPosition(battlerId) ^ 2;
...
static void Cmd_get_ability(void)
{
    ...
    if (GetBattlerSide(battlerId) == 0)
    {
        u16 side = ((GetBattlerPosition(battlerId) & 1));

        if ((gBattleResources->battleHistory)->abilities[side] != 0)
        {
            (gBattleResources->ai)->funcResult = (gBattleResources->battleHistory)->abilities[side];
            sAIScriptPtr += 2;
            return;
        }
        ...
        if (gSpeciesInfo[gBattleMons[battlerId].species].abilities[0] != 0)
        {
            if (gSpeciesInfo[gBattleMons[battlerId].species].abilities[1] != 0)
            {

                if (Random() % 2)
                    (gBattleResources->ai)->funcResult = gSpeciesInfo[gBattleMons[battlerId].species].abilities[0];
                else
                    (gBattleResources->ai)->funcResult = gSpeciesInfo[gBattleMons[battlerId].species].abilities[1];
            }
            else
            {
                (gBattleResources->ai)->funcResult = gSpeciesInfo[gBattleMons[battlerId].species].abilities[0];
...
# 1318 "src/battle_ai_script_commands.c"
    statusToCompareTo = ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));
    ...
    for (i = 0; i < 6; i++)
    {
        ...
        if (species != 0 && species != 479 && hp != 0 && status == statusToCompareTo)
        {
            sAIScriptPtr = (u8 *) ((sAIScriptPtr + 6)[0] | ((sAIScriptPtr + 6)[1] << 8) | ((sAIScriptPtr + 6)[2] << 16) | ((sAIScriptPtr + 6)[3] << 24));
            return;
        }
...
static void Cmd_get_weather(void)
{
    if (gBattleWeather & ((1 << 0) | (1 << 1) | (1 << 2)))
        (gBattleResources->ai)->funcResult = WEATHER_TYPE_RAIN;
    if (gBattleWeather & ((1 << 3) | (1 << 4)))
        (gBattleResources->ai)->funcResult = WEATHER_TYPE_SANDSTORM;
    if (gBattleWeather & ((1 << 5) | (1 << 6)))
        (gBattleResources->ai)->funcResult = WEATHER_TYPE_SUNNY;
    if (gBattleWeather & (1 << 7))
        (gBattleResources->ai)->funcResult = WEATHER_TYPE_HAIL;
...
static void Cmd_if_effect(void)
{
    if (gBattleMoves[(gBattleResources->ai)->moveConsidered].effect == sAIScriptPtr[1])
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 2)[0] | ((sAIScriptPtr + 2)[1] << 8) | ((sAIScriptPtr + 2)[2] << 16) | ((sAIScriptPtr + 2)[3] << 24));
    else
        sAIScriptPtr += 6;
}
...
static void Cmd_if_stat_level_less_than(void)
{
    ...
    if (gBattleMons[battlerId].statStages[sAIScriptPtr[2]] < sAIScriptPtr[3])
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 4)[0] | ((sAIScriptPtr + 4)[1] << 8) | ((sAIScriptPtr + 4)[2] << 16) | ((sAIScriptPtr + 4)[3] << 24));
    else
        sAIScriptPtr += 8;
}
...
static void Cmd_if_has_move(void)
{
    ...
    case 2:
        for (i = 0; i < 8; i++)
        {
            if ((gBattleResources->battleHistory)->usedMoves[gBattlerTarget >> 1][i] == *movePtr)
                break;
        }
        if (i == 8)
            sAIScriptPtr += 8;
...
static void Cmd_if_has_move_with_effect(void)
{
    ...
    case 2:
        for (i = 0; i < 8; i++)
        {
            if (gBattleMons[gBattlerAttacker].moves[i] != 0 && gBattleMoves[(gBattleResources->battleHistory)->usedMoves[gBattlerTarget >> 1][i]].effect == sAIScriptPtr[2])
                break;
        }
        sAIScriptPtr = (u8 *) ((sAIScriptPtr + 3)[0] | ((sAIScriptPtr + 3)[1] << 8) | ((sAIScriptPtr + 3)[2] << 16) | ((sAIScriptPtr + 3)[3] << 24));
    }
...
static void Cmd_if_doesnt_have_move_with_effect(void)
{
    ...
    case 2:
        for (i = 0; i < 8; i++)
        {
            if ((gBattleResources->battleHistory)->usedMoves[gBattlerTarget >> 1][i] != 0 && gBattleMoves[(gBattleResources->battleHistory)->usedMoves[gBattlerTarget >> 1][i]].effect == sAIScriptPtr[2])
                break;
        }
        sAIScriptPtr += 7;
    }
...
static void Cmd_flee(void)
{
    (gBattleResources->ai)->aiAction |= (0x0001 | 0x0002 | 0x0008);
}
...
static void Cmd_watch(void)
{
    (gBattleResources->ai)->aiAction |= (0x0001 | 0x0004 | 0x0008);
}
...
static void Cmd_get_hold_effect(void)
{
    ...
    {
        side = ((GetBattlerPosition(battlerId) & 1));
        (gBattleResources->ai)->funcResult = (gBattleResources->battleHistory)->itemEffects[side];
    }
    else
        (gBattleResources->ai)->funcResult = ItemId_GetHoldEffect(gBattleMons[battlerId].item);
...
static void Cmd_get_gender(void)
{
    ...
    (gBattleResources->ai)->funcResult = GetGenderFromSpeciesAndPersonality(gBattleMons[battlerId].species, gBattleMons[battlerId].personality);
...
static void Cmd_is_first_turn_for(void)
{
    ...
    (gBattleResources->ai)->funcResult = gDisableStructs[battlerId].isFirstTurn;
...
static void Cmd_get_stockpile_count(void)
{
    ...
    (gBattleResources->ai)->funcResult = gDisableStructs[battlerId].stockpileCounter;
...
static void Cmd_is_double_battle(void)
{
    (gBattleResources->ai)->funcResult = gBattleTypeFlags & (1 << 0);
...
static void Cmd_get_used_held_item(void)
{
    ...
    (gBattleResources->ai)->funcResult = ((u8 *)gBattleStruct->usedHeldItems)[battlerId * 2];
    sAIScriptPtr += 2;
}
...
static void Cmd_get_protect_count(void)
{
    ...
    (gBattleResources->ai)->funcResult = gDisableStructs[battlerId].protectUses;
