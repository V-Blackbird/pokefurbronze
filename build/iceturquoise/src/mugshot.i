...
void ClearTempFieldEventData(void);
# 7 "src/mugshot.c" 2
# 1 "include/constants/mugshots.h" 1
# 8 "src/mugshot.c" 2
...
static __attribute__((section("ewram_data"))) u8 sMugshotWindow = 0;
...
static void DrawMugshotCore(const struct Mugshot* const mugshot, int x, int y){
    ...
    t = SetWindowTemplateFields(0, x, y, mugshot->width/8, mugshot->height/8, 13, 0x40);

    windowId = AddWindow(&t);
    sMugshotWindow = windowId + 1;

    LoadPalette(mugshot->palette, 16 * 13, 32);
...
void DrawMugshot(void){
    const struct Mugshot* const mugshot = sMugshots + VarGet(0x8000);
    DrawMugshotCore(mugshot, mugshot->x, mugshot->y);
}

void DrawMugshotAtPos(void){
    DrawMugshotCore(sMugshots + VarGet(0x8000), VarGet(0x8001), VarGet(0x8002));
}
