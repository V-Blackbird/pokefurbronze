...
static bool8 ShouldSwitchIfPerishSong(void)
{
    ...
    {
        *(gBattleStruct->AI_monToSwitchIntoId + (GetBattlerPosition(gActiveBattler) >> 1)) = 6;
...
static bool8 ShouldSwitchIfWonderGuard(void)
{
    ...
    if (gBattleTypeFlags & (1 << 0))
        return 0;
    if (gBattleMons[GetBattlerAtPosition(0)].ability == 25)
    {

        for (opposingBattler = GetBattlerAtPosition(0), i = 0; i < 4; ++i)
        {
        ...
        for (i = 0; i < 6; ++i)
        {
            if (GetMonData(&gEnemyParty[i], 57) == 0
             || GetMonData(&gEnemyParty[i], 65) == 0
             || GetMonData(&gEnemyParty[i], 65) == 479
             || i == gBattlerPartyIndexes[gActiveBattler])
                continue;
            GetMonData(&gEnemyParty[i], 11);
            GetMonData(&gEnemyParty[i], 46);
            for (opposingBattler = GetBattlerAtPosition(0), j = 0; j < 4; ++j)
            {
                move = GetMonData(&gEnemyParty[i], 13 + j);
                if (move == 0)
                    continue;
                moveFlags = AI_TypeCalc(move, gBattleMons[opposingBattler].species, gBattleMons[opposingBattler].ability);
...
static bool8 FindMonThatAbsorbsOpponentsMove(void)
{
    ...
    if ((HasSuperEffectiveMoveAgainstOpponents(1) && Random() % 3)
    ...
    for (i = 0; i < 6; ++i)
    {
        ...
        if ((GetMonData(&gEnemyParty[i], 57) == 0)
         ...
         || (i == *(gBattleStruct->monToSwitchIntoId + battlerIn2)))
            continue;
        species = GetMonData(&gEnemyParty[i], 11);
        if (GetMonData(&gEnemyParty[i], 46) != 0)
...
static bool8 ShouldSwitchIfNaturalCure(void)
{
    if (!(gBattleMons[gActiveBattler].status1 & (1 << 0 | 1 << 1 | 1 << 2))
     || (gBattleMons[gActiveBattler].ability != 30)
    ...
    if (FindMonWithFlagsAndSuperEffective((1 << 3), 1)
     || FindMonWithFlagsAndSuperEffective((1 << 2), 1))
        return 1;
    if (Random() & 1)
    {
        *(gBattleStruct->AI_monToSwitchIntoId + (GetBattlerPosition(gActiveBattler) >> 1)) = 6;
...
static bool8 HasSuperEffectiveMoveAgainstOpponents(bool8 noRng)
{
    ...
    opposingBattler = GetBattlerAtPosition(0);
    ...
    if (!(gBattleTypeFlags & (1 << 0)))
        return 0;
    opposingBattler = GetBattlerAtPosition(((0) ^ 2));
    if (!(gAbsentBattlerFlags & gBitTable[opposingBattler]))
    {
        for (i = 0; i < 4; ++i)
        {
...
static bool8 ShouldSwitch(void)
{
    ...
    if ((gBattleMons[gActiveBattler].status2 & ((1 << 13 | 1 << 14 | 1 << 15) | (1 << 26)))
     ...
     || AbilityBattleEffects(12, gActiveBattler, 23, 0, 0)
     || AbilityBattleEffects(12, gActiveBattler, 71, 0, 0))
        return 0;
    if (AbilityBattleEffects(14, 0, 42, 0, 0))
        if ((gBattleMons[gActiveBattler].type1 == 8) || (gBattleMons[gActiveBattler].type2 == 8))
            return 0;
    availableToSwitch = 0;
    if (gBattleTypeFlags & (1 << 0))
    {
        ...
        if (ShouldSwitch())
        {
            if (*(gBattleStruct->AI_monToSwitchIntoId + (GetBattlerPosition(gActiveBattler) >> 1)) == 6)
            {
                ...
                if (monToSwitchId == 6)
                {
                    ...
                    {
                        if ((!GetMonData(&gEnemyParty[monToSwitchId], 57) == 0)
...
u8 GetMostSuitableMonToSwitchInto(void)
{
    ...
    while (invalidMons != 0x3F)
    {
        ...
        for (i = 0; i < 6; ++i)
        {
            u16 species = GetMonData(&gEnemyParty[i], 11);
            if (species != 0
                && GetMonData(&gEnemyParty[i], 57) != 0
        ...
        if (bestMonId != 6)
        {
            for (i = 0; i < 4; ++i)
            {
                move = GetMonData(&gEnemyParty[bestMonId], 13 + i);
    ...
    for (i = 0; i < 6; ++i)
    {
        if (((u16)(GetMonData(&gEnemyParty[i], 11)) == 0)
         || (GetMonData(&gEnemyParty[i], 57) == 0)
         || (gBattlerPartyIndexes[battlerIn1] == i)
         || (gBattlerPartyIndexes[battlerIn2] == i)
         || (i == *(gBattleStruct->monToSwitchIntoId + battlerIn1))
...
static bool8 ShouldUseItem(void)
{
    ...
    for (i = 0; i < 4; ++i)
    {
        ...
        if (item == 0 || gItemEffectTable[item - 14] == ((void *)0))
            continue;
        if (item == 176)
            itemEffects = gSaveBlock1Ptr->enigmaBerry.itemEffect;
        else
            itemEffects = gItemEffectTable[item - 14];
