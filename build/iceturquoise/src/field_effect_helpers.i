...
# 1 "include/constants/field_effects.h" 1
# 12 "src/field_effect_helpers.c" 2
...
LoadObjectEventPalette(0x1102);
        reflectionSprite->oam.paletteNum = IndexOfSpritePaletteTag(0x1102);
...
void LoadSpecialReflectionPalette(struct Sprite *sprite)
{
    ...
    CpuSet(&gPlttBufferUnfaded[0x100 + sprite->oam.paletteNum * 16], gReflectionPaletteBuffer, 0x00000000 | ((32)/(16/8) & 0x1FFFFF));
    TintPalette_CustomTone(gReflectionPaletteBuffer, 16, ((s16)((1.0) * 256)), ((s16)((1.0) * 256)), ((s16)((3.5) * 256)));
...
u32 FldEff_Shadow(void)
{
    ...
    LoadFieldEffectPalette_(gShadowEffectTemplateIds[graphicsInfo->shadowSize], 0);
...
# 939 "src/field_effect_helpers.c"
...
void SetSurfBlob_BobState(u8 spriteId, u8 bobState)
{
    gSprites[spriteId].data[0] = (gSprites[spriteId].data[0] & ~0xF) | (bobState & 0xF);
}
...
void UpdateSurfBlobFieldEffect(struct Sprite *sprite)
{
    ...
    playerObject = &gObjectEvents[sprite->data[2]];
...
void SynchroniseSurfPosition(struct ObjectEvent *playerObject, struct Sprite *surfBlobSprite)
{
    ...
    if (yOffset == 0 && (x != surfBlobSprite->data[6] || y != surfBlobSprite->data[7]))
    {
        ...
        for (i = 1; i <= 4; i++, x = surfBlobSprite->data[6], y = surfBlobSprite->data[7])
        {
            MoveCoords(i, &x, &y);
...
static void CreateBobbingEffect(struct ObjectEvent *objectEvent, struct Sprite *playerSprite, struct Sprite *surfBlobSprite)
{
    ...
    if (bobState != BOB_NONE)
    {
        ...
        if (((u16)(++surfBlobSprite->data[4]) & intervals[surfBlobSprite->data[5]]) == 0)
            surfBlobSprite->y2 += surfBlobSprite->data[3];

        if ((surfBlobSprite->data[4] & 0x1F) == 0)
            surfBlobSprite->data[3] = -surfBlobSprite->data[3];

        if (bobState != BOB_MON_ONLY)
        {
            if (GetSurfBlob_HasPlayerOffset(surfBlobSprite) == 0)
                playerSprite->y2 = surfBlobSprite->y2;
            else
                playerSprite->y2 = surfBlobSprite->data[1] + surfBlobSprite->y2;
...
{
# 1225 "src/field_effect_helpers.c"
    return 0;
}

u32 ShowTreeDisguiseFieldEffect(void)
{
    return ShowDisguiseFieldEffect(28, 24);
}

u32 ShowMountainDisguiseFieldEffect(void)
{
    return ShowDisguiseFieldEffect(29, 25);
}

u32 ShowSandDisguiseFieldEffect(void)
{
    return ShowDisguiseFieldEffect(36, 28);
}
...
void LoadFieldEffectPalette(u8 fieldEffect)
{
    LoadFieldEffectPalette_(fieldEffect, 1);
}
