...
static u8 BikeInputHandler_Normal(u8 *direction_p, u16 newKeys, u16 heldKeys)
{
    ...
    if (MetatileBehavior_IsCyclingRoadPullDownTile(playerObjEvent->currentMetatileBehavior) == 1)
    {
...
static u8 BikeInputHandler_Slope(u8 *direction_p, u16 newKeys, u16 heldKeys)
{
    ...
    if (MetatileBehavior_IsCyclingRoadPullDownTile(playerObjEventId[gObjectEvents].currentMetatileBehavior) == 1)
    {
...
{
    u8 collision = GetBikeCollision(1);
...
bool32 IsRunningDisallowed(u8 metatileBehavior)
{
    if (!gMapHeader.allowRunning)
        return 1;
    if (MetatileBehaviorForbidsBiking(metatileBehavior) != 1)
        return 0;
    else
        return 1;
}
...
bool8 IsBikingDisallowedByPlayer(void)
{
    ...
    if (!(gPlayerAvatar.flags & ((1 << PLAYER_AVATAR_STATE_UNDERWATER) | (1 << PLAYER_AVATAR_STATE_SURFING))))
    {
...
void GetOnOffBike(u8 flags)
{
    ...
    else
    {
        ...
        if (Overworld_MusicCanOverrideMapMusic(282))
        {
...
void BikeClearState(u32 directionHistory, u32 abStartSelectHistory)
{
    ...
    for (i = 0; i < (sizeof(gPlayerAvatar.dirTimerHistory) / sizeof((gPlayerAvatar.dirTimerHistory)[0])); ++i)
...
s16 GetPlayerSpeed(void)
{
    ...
    else if (gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_ACRO_BIKE))
        return SPEED_FASTER;
    else if (gPlayerAvatar.flags & ((1 << PLAYER_AVATAR_STATE_SURFING) | (1 << PLAYER_AVATAR_STATE_DASH)))
        return SPEED_FAST;
    else
        return SPEED_NORMAL;
}
...
void Bike_HandleBumpySlopeJump(void)
{
    ...
    if (gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_ACRO_BIKE))
    {
        PlayerGetDestCoords(&x, &y);
