...
{
    [BALL_POKE] = {gBallGfx_Poke, 384, 55000},
    [BALL_GREAT] = {gBallGfx_Great, 384, 55001},
    [BALL_SAFARI] = {gBallGfx_Safari, 384, 55002},
    [BALL_ULTRA] = {gBallGfx_Ultra, 384, 55003},
    [BALL_MASTER] = {gBallGfx_Master, 384, 55004},
    [BALL_NET] = {gBallGfx_Net, 384, 55005},
    [BALL_DIVE] = {gBallGfx_Dive, 384, 55006},
    [BALL_NEST] = {gBallGfx_Nest, 384, 55007},
    [BALL_REPEAT] = {gBallGfx_Repeat, 384, 55008},
    [BALL_TIMER] = {gBallGfx_Timer, 384, 55009},
    [BALL_LUXURY] = {gBallGfx_Luxury, 384, 55010},
    [BALL_PREMIER] = {gBallGfx_Premier, 384, 55011},
    [BALL_SHADOW] = {gBallGfx_Shadow, 384, 55012},
};

const struct CompressedSpritePalette gBallSpritePalettes[POKEBALL_COUNT] =
{
    [BALL_POKE] = {gBallPal_Poke, 55000},
    [BALL_GREAT] = {gBallPal_Great, 55001},
    [BALL_SAFARI] = {gBallPal_Safari, 55002},
    [BALL_ULTRA] = {gBallPal_Ultra, 55003},
    [BALL_MASTER] = {gBallPal_Master, 55004},
    [BALL_NET] = {gBallPal_Net, 55005},
    [BALL_DIVE] = {gBallPal_Dive, 55006},
    [BALL_NEST] = {gBallPal_Nest, 55007},
    [BALL_REPEAT] = {gBallPal_Repeat, 55008},
    [BALL_TIMER] = {gBallPal_Timer, 55009},
    [BALL_LUXURY] = {gBallPal_Luxury, 55010},
    [BALL_PREMIER] = {gBallPal_Premier, 55011},
    [BALL_SHADOW] = {gBallPal_Shadow, 55012},
};
...
static void Task_DoPokeballSendOutAnim(u8 taskId)
{
    ...
    if (GetBattlerSide(battlerId) != 0)
        itemId = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]], 38);
    else
        itemId = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], 38);
...
static void SpriteCB_BallThrow_StartShrinkMon(struct Sprite *sprite)
{
    if (++sprite->data[5] == 10)
    {
        ...
        StartSpriteAffineAnim(&gSprites[gBattlerSpriteIds[sprite->data[6]]], BATTLER_AFFINE_RETURN);
...
static void Task_PlayCryWhenReleasedFromBall(u8 taskId)
{
    ...
    struct Pokemon *mon = (void *)(u32)((gTasks[taskId].data[3] << 16) | (u16)(gTasks[taskId].data[4]));
...
static void SpriteCB_ReleaseMonFromBall(struct Sprite *sprite)
{
    ...
    if (gMain.inBattle)
    {
        ...
        gTasks[taskId].data[3] = (u32)(mon) >> 16;
        gTasks[taskId].data[4] = (u32)(mon);
        gTasks[taskId].data[15] = 0;
    }

    StartSpriteAffineAnim(&gSprites[gBattlerSpriteIds[sprite->data[6]]], BATTLER_AFFINE_EMERGE);
    AnimateSprite(&gSprites[gBattlerSpriteIds[sprite->data[6]]]);
...
{
    sprite->data[0] = 25;
    sprite->data[2] = GetBattlerSpriteCoord(sprite->data[6], BATTLER_COORD_X);
    sprite->data[4] = GetBattlerSpriteCoord(sprite->data[6], BATTLER_COORD_Y) + 24;
    sprite->data[5] = -30;
    sprite->oam.affineParam = sprite->data[6];
    InitAnimArcTranslation(sprite);
...
static void SpriteCB_OpponentMonSendOut_2(struct Sprite *sprite)
{
    ...
    if ((((sprite->data[7]) >> 8) & 0xFF) >= 35 && (((sprite->data[7]) >> 8) & 0xFF) < 80)
    {
        ...
        if ((sprite->oam.affineParam & 0xFF00) == 0)
        {
            ...
            sprite->data[1] = ((sprite->data[1] / 3) & ~1) | r6;
            sprite->data[2] = ((sprite->data[2] / 3) & ~1) | r7;
            if (GetMonData(&gEnemyParty[1], 11) == 475 || 473)
                StartSpriteAffineAnim(sprite, 2);
            else
                StartSpriteAffineAnim(sprite, 4);
        }
...
void CreatePokeballSpriteToReleaseMon(u8 monSpriteId, u8 monPalNum, u8 x, u8 y, u8 oamPriority, u8 subpriortiy, u8 delay, u32 fadePalettes)
{
    ...
    gSprites[spriteId].data[1] = delay;
    gSprites[spriteId].data[2] = monPalNum;
    gSprites[spriteId].data[3] = fadePalettes;
    gSprites[spriteId].data[4] = fadePalettes >> 16;
...
static void SpriteCB_PokeballReleaseMon(struct Sprite *sprite)
{
    if (sprite->data[1] == 0)
    {
        ...
        sprite->data[1] = LaunchBallFadeMonTaskForPokeball(1, monPalNum, selectedPalettes);
...
static void SpriteCB_ReleasedMonFlyOut(struct Sprite *sprite)
{
    bool8 emergeAnimFinished = 0;
    bool8 atFinalPosition = 0;
    ...
    if (gSprites[monSpriteId].affineAnimEnded)
    {
        ...
        emergeAnimFinished = 1;
    }

    x = (sprite->data[5] - sprite->x) * sprite->data[7] / 128 + sprite->x;
    y = (sprite->data[6] - sprite->y) * sprite->data[7] / 128 + sprite->y;
    gSprites[monSpriteId].x = x;
    gSprites[monSpriteId].y = y;
...
void LoadBallGfx(u8 ballId)
{
    ...
    default:
        ...
        LZDecompressVram(gOpenPokeballGfx, (void *)((void *)(0x6000000 + 0x10000) + 0x100 + var * 32));
        break;
    }
}
...
{
    if (GetBattlerSide(battlerId) == 0)
        return GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], 38);
    else
        return GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerId]], 38);
}
