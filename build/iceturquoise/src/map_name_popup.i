...
# 27 "src/map_name_popup.c"
void ShowMapNamePopup(bool32 palIntoFadedBuffer)
{
    u8 taskId;
    if (FlagGet((0x4000 + 0x0)) != 1 && !(gQuestLogState == 2 || gQuestLogState == 3))
    {
...
static void Task_MapNamePopup(u8 taskId)
{
    ...
    case 4:
        ...
        if (task->data[2] >= 0)
        {
            if (task->data[3])
            {
                ...
                CopyWindowToVram(task->data[4], COPYWIN_GFX);
                task->data[0] = 1;
                task->data[3] = 0;
            }
            else
    ...
    case 6:
        if (task->data[5] && !task->data[6])
        {
            ...
            CopyWindowToVram(task->data[4], COPYWIN_MAP);
            task->data[6] = 1;
        }
        task->data[0] = 7;
        return;
...
{
    return FindTaskIdByFunc(Task_MapNamePopup) != 0xFF ? 1 : 0;
}
...
static u16 MapNamePopupCreateWindow(bool32 palintoFadedBuffer)
{
    ...
    if (palintoFadedBuffer)
        ...
        CpuSet(GetTextWindowPalette(3), &gPlttBufferUnfaded[(0x000 + ((13) * 16))], 0x00000000 | ((((16) * sizeof(u16)))/(16/8) & 0x1FFFFF));
    LoadStdWindowTiles(windowId, tileNum);
    DrawTextBorderOuter(windowId, tileNum, 13);
...
static void MapNamePopupPrintMapNameOnWindow(u16 windowId)
{
    ...
    if (gMapHeader.floorNum != 0)
    {
        ...
        maxWidth = gMapHeader.floorNum != 127 ? 152 : 176;
    }
    xpos = (maxWidth - GetStringWidth(FONT_NORMAL, mapName, -1)) / 2;
    FillWindowPixelBuffer(windowId, ((1) | ((1) << 4)));
    AddTextPrinterParameterized(windowId, FONT_NORMAL, mapName, xpos, 2, 0xFF, ((void *)0));
}
