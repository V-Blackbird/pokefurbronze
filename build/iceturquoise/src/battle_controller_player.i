...
static void PlayerBufferRunCommand(void)
{
    ...
    {
        if (gBattleBufferA[gActiveBattler][0] < (sizeof(sPlayerBufferCommands) / sizeof((sPlayerBufferCommands)[0])))
...
static void HandleInputChooseTarget(void)
{
    ...
    memcpy(identities, sTargetIdentities, (sizeof(sTargetIdentities) / sizeof((sTargetIdentities)[0])));
        ...
        do
        {
            ...
            case 2:
                if (gActiveBattler != gMultiUsePlayerCursor)
                    ++i;
                else if (gBattleMoves[GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], 13 + gMoveSelectionCursor[gActiveBattler])].target & (1 << 1))
                    ++i;
                break;
            case 1:
            case 3:
...
void HandleInputChooseMove(void)
{
    ...
    if (({(gMain.newKeys) & (0x0001);}))
    {
        ...
        {
            if (moveInfo->monType1 != 7 && moveInfo->monType2 != 7)
                moveTarget = (1 << 4);
            else
                moveTarget = 0;
        }
        ...
        BeginNormalPaletteFade(0xF0000, 0, 0, 0, ((31) | ((31) << 5) | ((31) << 10)));
        if (!canSelectTarget)
        {
            BtlController_EmitTwoReturnValues(1, 10, gMoveSelectionCursor[gActiveBattler] | (gMultiUsePlayerCursor << 8));
            PlayerBufferExecCompleted();
...
{
    u8 monIndex;
    s32 battlerId = gTasks[taskId].data[2];

    if (IsBattlerSpriteVisible((u8)battlerId) == 1)
    {
...
static void PlayerHandleDrawTrainerPic(void)
{
    ...
    if (gBattleTypeFlags & (1 << 1))
    {
        if ((gLinkPlayers[GetMultiplayerId()].version & 0xFF) == 2
         || (gLinkPlayers[GetMultiplayerId()].version & 0xFF) == 1
         || (gLinkPlayers[GetMultiplayerId()].version & 0xFF) == 3)
...
static void PlayerHandleStatusXor(void)
{
    u8 val = GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], 55) ^ gBattleBufferA[gActiveBattler][1];
...
static void PlayerHandleFaintingCry(void)
{
    PlayCry_ByMode(GetMonData(&gPlayerParty[gBattlerPartyIndexes[gActiveBattler]], 11), -25, 5);
    PlayerBufferExecCompleted();
}
...
static void PlayerHandleDrawPartyStatusSummary(void)
{
    if (gBattleBufferA[gActiveBattler][1] && GetBattlerSide(gActiveBattler) == 0)
    {
        PlayerBufferExecCompleted();
    }
    else
...
static void PreviewDeterminativeMoveTargets(void)
{
    ...
    if (gBattleTypeFlags & (1 << 0))
    {
        ...
        case (1 << 3):
        case (1 << 6):
            bitMask = (gBitTable[GetBattlerAtPosition(1)]
                     | gBitTable[GetBattlerAtPosition(3)]) << 16;
            startY = 8;
            break;
        case (1 << 4):
            switch (move)
            ...
            case 215:
            case 312:
                bitMask = (gBitTable[GetBattlerAtPosition(0)]
                         | gBitTable[GetBattlerAtPosition(2)]) << 16;
                break;
            case 270:
                bitMask = (gBitTable[GetBattlerAtPosition(GetBattlerPosition(gActiveBattler) ^ 2)]) << 16;
                break;
            default:
        ...
        BeginNormalPaletteFade(bitMask, 8, startY, 0, ((31) | ((31) << 5) | ((31) << 10)));
    }
}
