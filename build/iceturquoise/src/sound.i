...
{
    s32 i;
    for (i = 0; (u32)i < (sizeof(sFanfares) / sizeof((sFanfares)[0])); i++)
    {
...
void PlayCryInternal(u16 species, s8 pan, s8 volume, u8 priority, u8 mode)
{
    ...
    {
    case 0:
        gMPlay_PokemonCry = SetPokemonCryTone(((reverse) ? &gCryTable_Reverse[(128 * (0)) + (index)] : &gCryTable[(128 * (0)) + (index)]));
        break;
    case 1:
        gMPlay_PokemonCry = SetPokemonCryTone(((reverse) ? &gCryTable_Reverse[(128 * (1)) + (index)] : &gCryTable[(128 * (1)) + (index)]));
        break;
    case 2:
        gMPlay_PokemonCry = SetPokemonCryTone(((reverse) ? &gCryTable_Reverse[(128 * (2)) + (index)] : &gCryTable[(128 * (2)) + (index)]));
        break;
    case 3:
        gMPlay_PokemonCry = SetPokemonCryTone(((reverse) ? &gCryTable_Reverse[(128 * (3)) + (index)] : &gCryTable[(128 * (3)) + (index)]));
        break;
    }
...
void SetBGMVolume_SuppressHelpSystemReduction(u16 volume)
{
    gDisableHelpSystemVolumeReduce = 1;
    m4aMPlayVolumeControl(&gMPlayInfo_BGM, 0xFFFF, volume);
}

void BGMVolumeMax_EnableHelpSystemReduction(void)
{
    gDisableHelpSystemVolumeReduce = 0;
    m4aMPlayVolumeControl(&gMPlayInfo_BGM, 0xFFFF, 256);
}
