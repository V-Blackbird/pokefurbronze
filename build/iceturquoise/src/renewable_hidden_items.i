# 0 "src/renewable_hidden_items.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/renewable_hidden_items.c"
...
# 2 "src/renewable_hidden_items.c" 2
...
void ClearTempFieldEventData(void);
# 3 "src/renewable_hidden_items.c" 2
...
# 4 "src/renewable_hidden_items.c" 2
...
# 5 "src/renewable_hidden_items.c" 2
...
void SetAllRenewableItemFlags(void)
{
    ...
    for (i = 0; i < (sizeof(sRenewableHiddenItems) / sizeof((sRenewableHiddenItems)[0])); i++)
    {
        ...
        for (j = 0; j < 8; j++)
        {
            if (rare[j] != 0xFF)
                FlagSet(1000 + rare[j]);
            if (uncommon[j] != 0xFF)
                FlagSet(1000 + uncommon[j]);
            if (common[j] != 0xFF)
                FlagSet(1000 + common[j]);
        }
    }
}
