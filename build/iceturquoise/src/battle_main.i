...
static void BufferPartyVsScreenHealth_AtStart(void)
{
    ...
    for ((i) = 0; (i) < 6; (i)++) { u16 species = GetMonData(&(gPlayerParty)[(i)], 65); u16 hp = GetMonData(&(gPlayerParty)[(i)], 57); u32 status = GetMonData(&(gPlayerParty)[(i)], 55); if (species == 0) continue; if (species != 479 && hp != 0 && status == 0) (flags) |= 1 << (i) * 2; if (species == 0) continue; if (hp != 0 && (species == 479 || status != 0)) (flags) |= 2 << (i) * 2; if (species == 0) continue; if (species != 479 && hp == 0) (flags) |= 3 << (i) * 2; };
...
static void SetPlayerBerryDataInBattleStruct(void)
{
    ...
    {
        const struct Berry *berryData = GetBerryInfo(ItemIdToBerryType(176));
     ...
     && !(gBattleTypeFlags & ((1 << 8) | (1 << 11) | (1 << 19))))
    {
        ...
        for (i = 0; i < gTrainers[trainerNum].partySize; i++)
        {
            ...
            case 0:
            {
                ...
                SetMonData(&party[i], 38, &partyData[i].pokeball);
                break;
            }
            case (1 << 0):
            {
...
static void BufferPartyVsScreenHealth_AtEnd(u8 taskId)
{
    ...
    flags = 0;
    for ((i) = 0; (i) < 6; (i)++) { u16 species = GetMonData(&(party1)[(i)], 65); u16 hp = GetMonData(&(party1)[(i)], 57); u32 status = GetMonData(&(party1)[(i)], 55); if (species == 0) continue; if (species != 479 && hp != 0 && status == 0) (flags) |= 1 << (i) * 2; if (species == 0) continue; if (hp != 0 && (species == 479 || status != 0)) (flags) |= 2 << (i) * 2; if (species == 0) continue; if (species != 479 && hp == 0) (flags) |= 3 << (i) * 2; };
    gTasks[taskId].data[3] = flags;

    flags = 0;
    for ((i) = 0; (i) < 6; (i)++) { u16 species = GetMonData(&(party2)[(i)], 65); u16 hp = GetMonData(&(party2)[(i)], 57); u32 status = GetMonData(&(party2)[(i)], 55); if (species == 0) continue; if (species != 479 && hp != 0 && status == 0) (flags) |= 1 << (i) * 2; if (species == 0) continue; if (hp != 0 && (species == 479 || status != 0)) (flags) |= 2 << (i) * 2; if (species == 0) continue; if (species != 479 && hp == 0) (flags) |= 3 << (i) * 2; };
    ...
    if (gBattleTypeFlags & (1 << 3))
    {
        for (i = 0; i < 6; i++)
        {
            if (GetMonData(&gEnemyParty[i], 65) == 0
             || GetMonData(&gEnemyParty[i], 65) == 479)
            {
        ...
        for (i = 0; i < 6; i++)
        {
            if (GetMonData(&gPlayerParty[i], 65) == 0
             || GetMonData(&gPlayerParty[i], 65) == 479)
            {
                hpStatus[i].hp = 0xFFFF;
                hpStatus[i].status = 0;
            }
            ...
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF0000000) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 0xC))))
            {
                 ...
                 && gBattleBufferB[gActiveBattler][1] == 3)
                {
                    ...
                    gBattleCommunication[gActiveBattler] = STATE_BEFORE_ACTION_CHOSEN;
                      ...
                      && gBattleBufferB[gActiveBattler][1] == 14)
                {
                    BattleScriptExecute(BattleScript_StarterPlayerUsedTackle);
                    gBattleCommunication[gActiveBattler] = STATE_BEFORE_ACTION_CHOSEN;
                }
                else if (gBattleTypeFlags & (1 << 20)
                      && gBattleBufferB[gActiveBattler][1] == 15)
                {
                    BattleScriptExecute(BattleScript_StarterNoMon);
                    gBattleCommunication[gActiveBattler] = STATE_BEFORE_ACTION_CHOSEN;
                }
                else if (gBattleTypeFlags & (1 << 20)
                      && gBattleBufferB[gActiveBattler][1] == 16)
                {
                    BattleScriptExecute(BattleScript_StarterNoRun);
                    gBattleCommunication[gActiveBattler] = STATE_BEFORE_ACTION_CHOSEN;
                }
...
u8 GetWhoStrikesFirst(u8 battler1, u8 battler2, bool8 ignoreChosenMoves)
{
    ...
    if (gBattleMons[battler1].item == 176)
    {
    ...
    if (gBattleMons[battler2].item == 176)
    {
...
static void HandleAction_UseItem(void)
{
    ...
    else if (gLastUsedItem == 81 || gLastUsedItem == 82)
    {
        gBattlescriptCurrInstr = gBattlescriptsForRunningByItem[0];
    }
    else if (gLastUsedItem == 351)
    {
...
bool8 TryRunFromBattle(u8 battler)
{
    ...
    if (gBattleMons[battler].item == 176)
