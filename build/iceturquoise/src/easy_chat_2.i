...
# 10 "src/easy_chat_2.c" 2
# 1 "include/constants/songs.h" 1
# 11 "src/easy_chat_2.c" 2
# 19 "src/easy_chat_2.c"
...
void DoEasyChatScreen(u8 type, u16 *words, MainCallback callback)
{
    ...
    SetWordTaskArg(taskId, 2, (uintptr_t)words);
    SetWordTaskArg(taskId, 4, (uintptr_t)callback);
...
static void Task_InitEasyChat(u8 taskId)
{
    ...
    {
        if (Task_InitEasyChatInternal(taskId) == 1)
            return;
    }
...
static void Task_RunEasyChat(u8 taskId)
{
    ...
    case 3:
        if (!gPaletteFade.active)
        {
            ...
            if (data[1] == 0)
            {
                FlagSet(((((0x4FF + 1) + 768 - 1) + 1) + 0x2D));
                CompareProfileResponseWithPassphrase();
            }
            DismantleEasyChat((MainCallback)GetWordTaskArg(taskId, 4));
        }
        break;
    }
}
...
static bool8 Task_InitEasyChatInternal(u8 taskId)
{
    ...
    case 1:
        ...
        break;
    case 2:
        if (!EasyChat_AllocateResources(data[1], (u16 *)GetWordTaskArg(taskId, 2)))
        {
            DismantleEasyChat((MainCallback)GetWordTaskArg(taskId, 4));
        }
        break;
...
static void CompareProfileResponseWithPassphrase(void)
{
    gSpecialVar_0x8004 = IsPhraseDifferentThanPlayerInput(sECPhrase_MysteryEventIsExciting, (sizeof(sECPhrase_MysteryEventIsExciting) / sizeof((sECPhrase_MysteryEventIsExciting)[0])));
}
...
static void CompareQuestionnaireResponseWithPassphrase(void)
{
    gSpecialVar_0x8004 = IsPhraseDifferentThanPlayerInput(sECPhrase_LinkTogetherWithAll, (sizeof(sECPhrase_LinkTogetherWithAll) / sizeof((sECPhrase_LinkTogetherWithAll)[0])));
}
...
static bool8 EasyChat_AllocateResources(u8 type, u16 *words)
{
    ...
    CpuSet(words, sEasyChatScreen->ecWordBuffer, 0x00000000 | ((sEasyChatScreen->numWords * sizeof(u16))/(16/8) & 0x1FFFFF));
...
static int OpenSelectedGroup(void)
{
    ...
    if (!sEasyChatScreen->isAlphaMode)
    {
        ...
        GetUnlockedECWords(0, groupId);
    }
    else
    {
        GetUnlockedECWords(1, GetSelectedLetter());
    }
...
static int GetSelectedLetter(void)
{
    int col = sEasyChatScreen->selectGroupCursorX < (sizeof(*sAlphabetLayout) / sizeof((*sAlphabetLayout)[0])) ? sEasyChatScreen->selectGroupCursorX : 0;
    int row = sEasyChatScreen->selectGroupCursorY < (sizeof(sAlphabetLayout) / sizeof((sAlphabetLayout)[0])) ? sEasyChatScreen->selectGroupCursorY : 0;
...
{
    if (!sEasyChatScreen->isAlphaMode)
        return GetSelectedGroupIndex() >= GetNumDisplayableGroups() ? 1 : 0;
    else
        return sEasyChatScreen->selectGroupCursorX > GetMaxGroupCursorXinAlphaMode(sEasyChatScreen->selectGroupCursorY) ? 1 : 0;
}

static bool8 WordSelectCursorXPosTooFarRight(void)
{
    return GetSelectWordCursorPos() >= GetNumDisplayedWords() ? 1 : 0;
}
...
static u8 GetEasyChatScreenTemplateId(u8 type)
{
    ...
    for (i = 0; i < (sizeof(sEasyChatScreenTemplates) / sizeof((sEasyChatScreenTemplates)[0])); i++)
    {
