...
extern const u8 gEnemyMonElevation[479];
...
static __attribute__((section("ewram_data"))) u8 sElevatorCurrentFloorWindowId = 0;
static __attribute__((section("ewram_data"))) u16 sElevatorScroll = 0;
static __attribute__((section("ewram_data"))) u16 sElevatorCursorPos = 0;
static __attribute__((section("ewram_data"))) struct ListMenuItem * sListMenuItems = ((void *)0);
static __attribute__((section("ewram_data"))) u16 sListMenuLastScrollPosition = 0;
static __attribute__((section("ewram_data"))) u8 sPCBoxToSendMon = 0;
static __attribute__((section("ewram_data"))) u8 sBrailleTextCursorSpriteID = 0;
...
{
    if (gPlayerAvatar.flags & (1 << PLAYER_AVATAR_STATE_NORMAL))
        SetPlayerAvatarTransitionFlags((1 << PLAYER_AVATAR_STATE_MACH_BIKE));
    Overworld_SetSavedMusic(282);
    Overworld_ChangeMusicTo(282);
}
...
u8 GetPlayerAvatarBike(void)
{
    if (TestPlayerAvatarFlags((1 << PLAYER_AVATAR_STATE_ACRO_BIKE)))
        return 1;
    else if (TestPlayerAvatarFlags((1 << PLAYER_AVATAR_STATE_MACH_BIKE)))
        return 2;
    else
        return 0;
}
...
u8 GetLeadMonFriendship(void)
{
    ...
    if (GetMonData(pokemon, 32) == 255)
        return 6;
    else if (GetMonData(pokemon, 32) >= 200)
        return 5;
    else if (GetMonData(pokemon, 32) >= 150)
        return 4;
    else if (GetMonData(pokemon, 32) >= 100)
        return 3;
    else if (GetMonData(pokemon, 32) >= 50)
        return 2;
    else if (GetMonData(pokemon, 32) > 0)
        return 1;
    else
        return 0;
}
...
void AnimatePcTurnOn(void)
{
    ...
    if (FuncIsActiveTask(Task_AnimatePcTurnOn) != 1)
    {
...
static void PcTurnOnUpdateMetatileId(bool16 flickerOff)
{
    ...
    MapGridSetMetatileIdAt(gSaveBlock1Ptr->pos.x + deltaX + 7, gSaveBlock1Ptr->pos.y + deltaY + 7, metatileId | 0x0C00);
}
...
{
    u8 objectEventId = SpawnSpecialObjectEventParameterized(18, 8, 0x7F, gSaveBlock1Ptr->pos.x + 7, gSaveBlock1Ptr->pos.y + 7, 3);
...
{
    u16 rval = Random() % (sizeof(sSlotMachineIndices) / sizeof((sSlotMachineIndices)[0]));
...
bool8 LeadMonHasEffortRibbon(void)
{
    ...
    return GetMonData(&gPlayerParty[leadMonIdx], 71, ((void *)0));
}
...
{
    u16 species = GetStarterSpeciesById(VarGet(0x4031));
    u8 partyCount = CalculatePlayerPartyCount();
    u8 i;
    for (i = 0; i < partyCount; i++)
    {
        if (GetMonData(&gPlayerParty[i], 65, ((void *)0)) == species)
            return 1;
    }
    return 0;
}
...
u8 GetLeadMonIndex(void)
{
    ...
    {
        pokemon = &gPlayerParty[i];
        if (GetMonData(pokemon, 65, ((void *)0)) != 479 && GetMonData(pokemon, 65, ((void *)0)) != 0)
            return i;
    }
    return 0;
}
...
{
    u8 t = 0xFF;
    AddTextPrinterParameterized(0, FONT_NORMAL, &t, 0, 1, 0, ((void *)0));
    PicboxCancel();
}
...
void IncrementResortGorgeousStepCounter(void)
{
    u16 var4035 = VarGet(0x4035);
    if (VarGet(0x4036) != 0)
    {
        var4035++;
        if (var4035 >= 250)
        {
            VarSet(0x4036, 0xFFFF);
            VarSet(0x4035, 0);
        }
        else
        {
            VarSet(0x4035, var4035);
        }
    }
}
...
{
    u16 requestedSpecies = VarGet(0x4036);
    if (requestedSpecies == 0 || requestedSpecies == 0xFFFF)
    {
        VarSet(0x4036, SampleResortGorgeousMon());
        VarSet(0x403B, SampleResortGorgeousReward());
        VarSet(0x4035, 0);
    }
    StringCopy(gStringVar1, gSpeciesNames[VarGet(0x4036)]);
}
...
static u16 SampleResortGorgeousMon(void)
{
    ...
    for (i = 0; i < 100; i++)
    {
        species = (Random() % (479 - 1)) + 1;
        if (GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), 0) == 1)
            return species;
    }
    while (GetSetPokedexFlag(SpeciesToNationalPokedexNum(species), 0) != 1)
    {
        if (species == 1)
            species = 479 - 1;
        else
...
static u16 SampleResortGorgeousReward(void)
{
    if ((Random() % 100) >= 30)
        ...
        return sResortGorgeousDeluxeRewards[Random() % (sizeof(sResortGorgeousDeluxeRewards) / sizeof((sResortGorgeousDeluxeRewards)[0]))];
}
...
{
    u16 floor = 4;
    if (gSaveBlock1Ptr->dynamicWarp.mapGroup == ((42 | (1 << 8)) >> 8))
    {
    ...
    if (gSaveBlock1Ptr->dynamicWarp.mapGroup == ((0 | (11 << 8)) >> 8))
    {
        switch (gSaveBlock1Ptr->dynamicWarp.mapNum)
        {
        case ((0 | (11 << 8)) & 0xFF):
    ...
    if (gSaveBlock1Ptr->dynamicWarp.mapGroup == ((1 | (2 << 8)) >> 8))
    {
        switch (gSaveBlock1Ptr->dynamicWarp.mapNum)
        {
        case ((1 | (2 << 8)) & 0xFF):
...
void DrawElevatorCurrentFloorWindow(void)
{
    ...
    if (QuestLog_SchedulePlaybackCB(QLPlaybackCB_DestroyScriptMenuMonPicSprites) != 1)
    {
        ...
        LoadStdWindowGfx(sElevatorCurrentFloorWindowId, 0x21D, (0x000 + ((13) * 16)));
        DrawStdFrameWithCustomTileAndPalette(sElevatorCurrentFloorWindowId, 0, 0x21D, 13);
        AddTextPrinterParameterized(sElevatorCurrentFloorWindowId, FONT_NORMAL, gText_NowOn, 0, 2, 0xFF, ((void *)0));
        floorname = sFloorNamePointers[gSpecialVar_0x8005];
        strwidth = GetStringWidth(FONT_NORMAL, floorname, 0);
        AddTextPrinterParameterized(sElevatorCurrentFloorWindowId, FONT_NORMAL, floorname, 56 - strwidth, 16, 0xFF, ((void *)0));
...
{
    ClearStdWindowAndFrameToTransparent(sElevatorCurrentFloorWindowId, 1);
...
{
    u8 taskId;
    if (FuncIsActiveTask(Task_AnimateElevatorWindowView) != 1)
    {
...
static void Task_AnimateElevatorWindowView(u8 taskId)
{
    ...
    if (data[1] == 6)
    {
        ...
        if (data[2] == 0)
        {
            ...
            {
                for (j = 0; j < 3; j++)
                    MapGridSetMetatileIdAt(j + 1 + 7, i + 7, sElevatorWindowMetatilesGoingUp[i][data[0] % 3] | 0x0C00);
            }
        }
        else
        {
            ...
            {
                for (j = 0; j < 3; j++)
                    MapGridSetMetatileIdAt(j + 1 + 7, i + 7, sElevatorWindowMetatilesGoingDown[i][data[0] % 3] | 0x0C00);
            }
        }
        DrawWholeMapView();
        data[1] = 0;
...
static void CreateScriptListMenu(void)
{
    ...
    sFieldSpecialsListMenuTemplate.itemPrintFunc = ((void *)0);
...
static void ScriptListMenuMoveCursorFunction(s32 nothing, bool8 is, struct ListMenu * used)
{
    ...
    {
        task = &gTasks[taskId];
        ListMenuGetScrollAndRow(task->data[14], &sFieldSpecialsListMenuScrollBuffer, ((void *)0));
...
{
    if (idx >= (sizeof(sStarterSpecies) / sizeof((sStarterSpecies)[0])))
        idx = 0;
...
u16 GetStarterSpecies(void)
{
    return GetStarterSpeciesById(VarGet(0x4031));
}
...
void ResetContextNpcTextColor(void)
{
    ...
    gSpecialVar_TextColor = 255;
}

u8 ContextNpcGetTextColor(void)
{
    u8 gfxId;
    if (gSpecialVar_TextColor != 255)
    {
    ...
    else
    {
        ...
        if (gfxId >= (240 + 0x0))
            gfxId = VarGetObjectEventGraphicsId(gfxId - (240 + 0x0));
...
static bool8 HasMonBeenRenamed(u8 idx)
{
    ...
    if (language != 2)
        return 1;
    else if (StringCompare(gSpeciesNames[GetMonData(pokemon, 11, ((void *)0))], gStringVar1) != 0)
        return 1;
    else
        return 0;
}
...
void ChangeBoxPokemonNickname(void)
{
    ...
    GetBoxMonData(pokemon, 2, gStringVar3);
    GetBoxMonData(pokemon, 2, gStringVar2);
    species = GetBoxMonData(pokemon, 11, ((void *)0));
    gender = GetBoxMonGender(pokemon);
    personality = GetBoxMonData(pokemon, 0, ((void *)0));
...
void ChangePokemonNickname(void)
{
    ...
    species = GetMonData(&gPlayerParty[gSpecialVar_0x8004], 11, ((void *)0));
    gender = GetMonGender(&gPlayerParty[gSpecialVar_0x8004]);
    personality = GetMonData(&gPlayerParty[gSpecialVar_0x8004], 0, ((void *)0));
...
{
    if (GetPlayerTrainerId() == GetMonData(&gPlayerParty[gSpecialVar_0x8004], 1, ((void *)0)))
...
{
    u8 result = 0;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x9B)) == 1)
        result |= 1 << 0;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x9C)) == 1)
        result |= 1 << 1;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x9D)) == 1)
        result |= 1 << 2;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x9E)) == 1)
        result |= 1 << 3;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x9F)) == 1)
        result |= 1 << 4;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0xA1)) == 1)
        result |= 1 << 5;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0xA0)) == 1)
        result |= 1 << 6;
    return result;
}
...
void UpdateTrainerCardPhotoIcons(void)
{
    ...
    for (i = 0; i < partyCount; i++)
    {
        species[i] = GetMonData(&gPlayerParty[i], 65, ((void *)0));
        personality[i] = GetMonData(&gPlayerParty[i], 0, ((void *)0));
    }
    VarSet(0x4043, SpeciesToMailSpecies(species[0], personality[0]));
    VarSet(0x4044, SpeciesToMailSpecies(species[1], personality[1]));
    VarSet(0x4045, SpeciesToMailSpecies(species[2], personality[2]));
    VarSet(0x4046, SpeciesToMailSpecies(species[3], personality[3]));
    VarSet(0x4047, SpeciesToMailSpecies(species[4], personality[4]));
    VarSet(0x4048, SpeciesToMailSpecies(species[5], personality[5]));
    VarSet(0x4042, gSpecialVar_0x8004);
}
...
u16 StickerManGetBragFlags(void)
{
    u16 result = 0;
    u32 numEggs;
    gSpecialVar_0x8004 = GetGameStat(10);
    numEggs = GetGameStat(13);
    gSpecialVar_0x8006 = GetGameStat(23);
    if (numEggs > 0xFFFF)
...
u16 GetHiddenItemAttr(u32 hiddenItem, u8 attr)
{
    if (attr == HIDDEN_ITEM_ITEM)
        return (((hiddenItem) >> 0) & ((1 << 16) - 1));
    else if (attr == HIDDEN_ITEM_FLAG)
        return (((hiddenItem) >> (0 + 16)) & ((1 << 8) - 1)) + 1000;
    else if (attr == HIDDEN_ITEM_QUANTITY)
        return (((hiddenItem) >> ((0 + 16) + 8)) & ((1 << 7) - 1));
    else if (attr == HIDDEN_ITEM_UNDERFOOT)
        return (((hiddenItem) >> (((0 + 16) + 8) + 7)) & ((1 << 1) - 1));
    else
        return 1;
}
...
bool8 DoesPlayerPartyContainSpecies(void)
{
    ...
    for (i = 0; i < partyCount; i++)
    {
        if (GetMonData(&gPlayerParty[i], 65, ((void *)0)) == gSpecialVar_0x8004)
            return 1;
    }
    return 0;
}
...
static const u8 sMartMaps[][3] = {
    ...
    {((3 | (7 << 8)) >> 8), ((3 | (7 << 8)) & 0xFF), 3},
    {((7 | (8 << 8)) >> 8), ((7 | (8 << 8)) & 0xFF), 1},
    {((5 | (9 << 8)) >> 8), ((5 | (9 << 8)) & 0xFF), 1},
    {((5 | (10 << 8)) >> 8), ((5 | (10 << 8)) & 0xFF), 1},
    {((1 | (12 << 8)) >> 8), ((1 | (12 << 8)) & 0xFF), 1},
    {((7 | (13 << 8)) >> 8), ((7 | (13 << 8)) & 0xFF), 1},
    {((5 | (15 << 8)) >> 8), ((5 | (15 << 8)) & 0xFF), 1},
    {((3 | (35 << 8)) >> 8), ((3 | (35 << 8)) & 0xFF), 1},
    {((7 | (36 << 8)) >> 8), ((7 | (36 << 8)) & 0xFF), 1},
    {((2 | (32 << 8)) >> 8), ((2 | (32 << 8)) & 0xFF), 1},
    {((4 | (38 << 8)) >> 8), ((4 | (38 << 8)) & 0xFF), 1}
};

u8 GetMartClerkObjectId(void)
{
    u8 i;
    for (i = 0; i < (sizeof(sMartMaps) / sizeof((sMartMaps)[0])); i++)
    {
...
static const struct {
    ...
    [0] = {((0 | (5 << 8)) >> 8), ((0 | (5 << 8)) & 0xFF), ((0 | (3 << 8)) >> 8), ((0 | (3 << 8)) & 0xFF)},
    [1] = {((3 | (5 << 8)) >> 8), ((3 | (5 << 8)) & 0xFF), ((0 | (3 << 8)) >> 8), ((0 | (3 << 8)) & 0xFF)},
    [2] = {((1 | (6 << 8)) >> 8), ((1 | (6 << 8)) & 0xFF), ((1 | (3 << 8)) >> 8), ((1 | (3 << 8)) & 0xFF)},
    [3] = {((0 | (29 << 8)) >> 8), ((0 | (29 << 8)) & 0xFF), ((41 | (3 << 8)) >> 8), ((41 | (3 << 8)) & 0xFF)},
    [4] = {((0 | (29 << 8)) >> 8), ((0 | (29 << 8)) & 0xFF), ((42 | (3 << 8)) >> 8), ((42 | (3 << 8)) & 0xFF)},
    [5] = {((0 | (1 << 8)) >> 8), ((0 | (1 << 8)) & 0xFF), ((0 | (16 << 8)) >> 8), ((0 | (16 << 8)) & 0xFF)},
    [6] = {((0 | (1 << 8)) >> 8), ((0 | (1 << 8)) & 0xFF), ((3 | (16 << 8)) >> 8), ((3 | (16 << 8)) & 0xFF)},
    [7] = {((0 | (7 << 8)) >> 8), ((0 | (7 << 8)) & 0xFF), ((2 | (3 << 8)) >> 8), ((2 | (3 << 8)) & 0xFF)},
    [8] = {((2 | (7 << 8)) >> 8), ((2 | (7 << 8)) & 0xFF), ((2 | (3 << 8)) >> 8), ((2 | (3 << 8)) & 0xFF)},
    [9] = {((1 | (1 << 8)) >> 8), ((1 | (1 << 8)) & 0xFF), ((22 | (3 << 8)) >> 8), ((22 | (3 << 8)) & 0xFF)},
    [10] = {((2 | (1 << 8)) >> 8), ((2 | (1 << 8)) & 0xFF), ((22 | (3 << 8)) >> 8), ((22 | (3 << 8)) & 0xFF)},
    [11] = {((5 | (8 << 8)) >> 8), ((5 | (8 << 8)) & 0xFF), ((3 | (3 << 8)) >> 8), ((3 | (3 << 8)) & 0xFF)},
    [12] = {((6 | (8 << 8)) >> 8), ((6 | (8 << 8)) & 0xFF), ((3 | (3 << 8)) >> 8), ((3 | (3 << 8)) & 0xFF)},
    [13] = {((0 | (31 << 8)) >> 8), ((0 | (31 << 8)) & 0xFF), ((44 | (3 << 8)) >> 8), ((44 | (3 << 8)) & 0xFF)},
    [14] = {((0 | (18 << 8)) >> 8), ((0 | (18 << 8)) & 0xFF), ((23 | (3 << 8)) >> 8), ((23 | (3 << 8)) & 0xFF)},
    [15] = {((30 | (1 << 8)) >> 8), ((30 | (1 << 8)) & 0xFF), ((23 | (3 << 8)) >> 8), ((23 | (3 << 8)) & 0xFF)},
    [16] = {((32 | (1 << 8)) >> 8), ((32 | (1 << 8)) & 0xFF), ((24 | (3 << 8)) >> 8), ((24 | (3 << 8)) & 0xFF)},
    [17] = {((3 | (10 << 8)) >> 8), ((3 | (10 << 8)) & 0xFF), ((5 | (3 << 8)) >> 8), ((5 | (3 << 8)) & 0xFF)},
    [18] = {((6 | (10 << 8)) >> 8), ((6 | (10 << 8)) & 0xFF), ((5 | (3 << 8)) >> 8), ((5 | (3 << 8)) & 0xFF)},
    [19] = {((5 | (1 << 8)) >> 8), ((5 | (1 << 8)) & 0xFF), ((5 | (3 << 8)) >> 8), ((5 | (3 << 8)) & 0xFF)},
    [20] = {((36 | (1 << 8)) >> 8), ((36 | (1 << 8)) & 0xFF), ((20 | (3 << 8)) >> 8), ((20 | (3 << 8)) & 0xFF)},
    [21] = {((38 | (1 << 8)) >> 8), ((38 | (1 << 8)) & 0xFF), ((29 | (3 << 8)) >> 8), ((29 | (3 << 8)) & 0xFF)},
    [22] = {((81 | (1 << 8)) >> 8), ((81 | (1 << 8)) & 0xFF), ((28 | (3 << 8)) >> 8), ((28 | (3 << 8)) & 0xFF)},
    [23] = {((81 | (1 << 8)) >> 8), ((81 | (1 << 8)) & 0xFF), ((28 | (3 << 8)) >> 8), ((28 | (3 << 8)) & 0xFF)},
    [24] = {((95 | (1 << 8)) >> 8), ((95 | (1 << 8)) & 0xFF), ((28 | (3 << 8)) >> 8), ((28 | (3 << 8)) & 0xFF)},
    [25] = {((88 | (1 << 8)) >> 8), ((88 | (1 << 8)) & 0xFF), ((4 | (3 << 8)) >> 8), ((4 | (3 << 8)) & 0xFF)},
    [26] = {((2 | (9 << 8)) >> 8), ((2 | (9 << 8)) & 0xFF), ((4 | (3 << 8)) >> 8), ((4 | (3 << 8)) & 0xFF)},
    [27] = {((4 | (9 << 8)) >> 8), ((4 | (9 << 8)) & 0xFF), ((4 | (3 << 8)) >> 8), ((4 | (3 << 8)) & 0xFF)},
    [28] = {((35 | (1 << 8)) >> 8), ((35 | (1 << 8)) & 0xFF), ((26 | (3 << 8)) >> 8), ((26 | (3 << 8)) & 0xFF)},
    [29] = {((33 | (1 << 8)) >> 8), ((33 | (1 << 8)) & 0xFF), ((25 | (3 << 8)) >> 8), ((25 | (3 << 8)) & 0xFF)},
    [30] = {((0 | (11 << 8)) >> 8), ((0 | (11 << 8)) & 0xFF), ((6 | (3 << 8)) >> 8), ((6 | (3 << 8)) & 0xFF)},
    [31] = {((7 | (11 << 8)) >> 8), ((7 | (11 << 8)) & 0xFF), ((6 | (3 << 8)) >> 8), ((6 | (3 << 8)) & 0xFF)},
    [32] = {((14 | (11 << 8)) >> 8), ((14 | (11 << 8)) & 0xFF), ((6 | (3 << 8)) >> 8), ((6 | (3 << 8)) & 0xFF)},
    [33] = {((16 | (11 << 8)) >> 8), ((16 | (11 << 8)) & 0xFF), ((6 | (3 << 8)) >> 8), ((6 | (3 << 8)) & 0xFF)},
    [34] = {((17 | (11 << 8)) >> 8), ((17 | (11 << 8)) & 0xFF), ((6 | (3 << 8)) >> 8), ((6 | (3 << 8)) & 0xFF)},
    [35] = {((42 | (1 << 8)) >> 8), ((42 | (1 << 8)) & 0xFF), ((14 | (11 << 8)) >> 8), ((14 | (11 << 8)) & 0xFF)},
    [36] = {((63 | (1 << 8)) >> 8), ((63 | (1 << 8)) & 0xFF), ((0 | (12 << 8)) >> 8), ((0 | (12 << 8)) & 0xFF)},
    [37] = {((3 | (12 << 8)) >> 8), ((3 | (12 << 8)) & 0xFF), ((7 | (3 << 8)) >> 8), ((7 | (3 << 8)) & 0xFF)},
    [38] = {((7 | (12 << 8)) >> 8), ((7 | (12 << 8)) & 0xFF), ((7 | (3 << 8)) >> 8), ((7 | (3 << 8)) & 0xFF)},
    [39] = {((2 | (15 << 8)) >> 8), ((2 | (15 << 8)) & 0xFF), ((10 | (3 << 8)) >> 8), ((10 | (3 << 8)) & 0xFF)},
    [40] = {((3 | (15 << 8)) >> 8), ((3 | (15 << 8)) & 0xFF), ((10 | (3 << 8)) >> 8), ((10 | (3 << 8)) & 0xFF)},
    [41] = {((47 | (1 << 8)) >> 8), ((47 | (1 << 8)) & 0xFF), ((10 | (3 << 8)) >> 8), ((10 | (3 << 8)) & 0xFF)},
    [42] = {((83 | (1 << 8)) >> 8), ((83 | (1 << 8)) & 0xFF), ((38 | (3 << 8)) >> 8), ((38 | (3 << 8)) & 0xFF)},
    [43] = {((83 | (1 << 8)) >> 8), ((83 | (1 << 8)) & 0xFF), ((38 | (3 << 8)) >> 8), ((38 | (3 << 8)) & 0xFF)},
    [44] = {((59 | (1 << 8)) >> 8), ((59 | (1 << 8)) & 0xFF), ((8 | (3 << 8)) >> 8), ((8 | (3 << 8)) & 0xFF)},
    [45] = {((0 | (13 << 8)) >> 8), ((0 | (13 << 8)) & 0xFF), ((8 | (3 << 8)) >> 8), ((8 | (3 << 8)) & 0xFF)},
    [46] = {((1 | (13 << 8)) >> 8), ((1 | (13 << 8)) & 0xFF), ((8 | (3 << 8)) >> 8), ((8 | (3 << 8)) & 0xFF)},
    [47] = {((39 | (1 << 8)) >> 8), ((39 | (1 << 8)) & 0xFF), ((42 | (3 << 8)) >> 8), ((42 | (3 << 8)) & 0xFF)},
    [48] = {((40 | (1 << 8)) >> 8), ((40 | (1 << 8)) & 0xFF), ((42 | (3 << 8)) >> 8), ((42 | (3 << 8)) & 0xFF)},
    [49] = {((0 | (14 << 8)) >> 8), ((0 | (14 << 8)) & 0xFF), ((9 | (3 << 8)) >> 8), ((9 | (3 << 8)) & 0xFF)},
    [50] = {((72 | (1 << 8)) >> 8), ((72 | (1 << 8)) & 0xFF), ((3 | (3 << 8)) >> 8), ((3 | (3 << 8)) & 0xFF)}
};

void QuestLog_CheckDepartingIndoorsMap(void)
{
    u8 i;
    for (i = 0; i < (sizeof(sInsideOutsidePairs) / sizeof((sInsideOutsidePairs)[0])); i++)
    {
...
void QuestLog_TryRecordDepartedLocation(void)
{
    ...
    u16 ql_entrance_id = VarGet(0x404D);
    event_buffer.map_section_id = 0;
    event_buffer.entrance_id = 0;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x8)))
    {
        if (ql_entrance_id == 5)
        {
            if (gSaveBlock1Ptr->location.mapGroup == ((0 | (16 << 8)) >> 8) && (gSaveBlock1Ptr->location.mapNum == ((0 | (16 << 8)) & 0xFF) || gSaveBlock1Ptr->location.mapNum == ((3 | (16 << 8)) & 0xFF)))
            {
                event_buffer.map_section_id = 0x66;
                if (gSaveBlock1Ptr->location.mapNum == ((0 | (16 << 8)) & 0xFF))
                    event_buffer.entrance_id = ql_entrance_id;
                else
                    event_buffer.entrance_id = ql_entrance_id + 1;
                SetQuestLogEvent(35, (void *)&event_buffer);
                FlagClear(((((0x4FF + 1) + 768 - 1) + 1) + 0x8));
                return;
            }
        }
        else if (ql_entrance_id == 3)
        {
            if (gSaveBlock1Ptr->location.mapGroup == ((41 | (3 << 8)) >> 8) && (gSaveBlock1Ptr->location.mapNum == ((41 | (3 << 8)) & 0xFF) || gSaveBlock1Ptr->location.mapNum == ((42 | (3 << 8)) & 0xFF)))
            {
                ...
                if (gSaveBlock1Ptr->location.mapNum == ((41 | (3 << 8)) & 0xFF))
                    event_buffer.entrance_id = ql_entrance_id;
                else
                    event_buffer.entrance_id = ql_entrance_id + 1;
                SetQuestLogEvent(35, (void *)&event_buffer);
        ...
        if (gSaveBlock1Ptr->location.mapGroup == sInsideOutsidePairs[ql_entrance_id].outside_grp && gSaveBlock1Ptr->location.mapNum == sInsideOutsidePairs[ql_entrance_id].outside_num)
        {
            event_buffer.map_section_id = Overworld_GetMapHeaderByGroupAndId(sInsideOutsidePairs[ql_entrance_id].inside_grp, sInsideOutsidePairs[ql_entrance_id].inside_num)->regionMapSectionId;
            event_buffer.entrance_id = ql_entrance_id;
            if (ql_entrance_id == 22)
            {
                PlayerGetDestCoords(&x, &y);
                if (x != 15 || y != 26)
                    event_buffer.entrance_id++;
            }
            else if (ql_entrance_id == 42)
            {
                PlayerGetDestCoords(&x, &y);
                if (x != 67 || y != 15)
                    event_buffer.entrance_id++;
...
u16 GetMysteryGiftCardStat(void)
{
    ...
    default:
        (0) ? ((void *)0) : AGBAssert(("C:/WORK/POKeFRLG/src/pm_lgfr_ose/source/" "scr_tool.c"), 3873, "0", 1);;
        return 0;
    }
}
...
bool8 ShouldShowBoxWasFullMessage(void)
{
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x43)))
        return 0;
    if (StorageGetCurrentBox() == VarGet(0x4037))
        return 0;
    FlagSet(((((0x4FF + 1) + 768 - 1) + 1) + 0x43));
    return 1;
}
...
bool8 IsDestinationBoxFull(void)
{
    s32 i;
    s32 j;
    SetPCBoxToSendMon(VarGet(0x4037));
        ...
        for (j = 0; j < (5 * 6); j++)
        {
            if (GetBoxMonData(GetBoxedMonPtr(i, j), 11, ((void *)0)) == 0)
            {
                if (GetPCBoxToSendMon() != i)
                    FlagClear(((((0x4FF + 1) + 768 - 1) + 1) + 0x43));
                VarSet(0x4037, i);
...
bool8 UsedPokemonCenterWarp(void)
{
    ...
    for (i = 0; sPokeCenter1FMaps[i] != (0xFF | (0xFF << 8)); i++)
    {
...
bool8 BufferTMHMMoveName(void)
{
    ...
    if (gSpecialVar_0x8004 >= 290 && gSpecialVar_0x8004 <= 347)
    {
...
void RunMassageCooldownStepCounter(void)
{
    u16 count = VarGet(0x4025);
    if (count < 500)
        VarSet(0x4025, count + 1);
}
...
void DoPokemonLeagueLightingEffect(void)
{
    ...
    else
    {
        if (gSaveBlock1Ptr->location.mapGroup == ((79 | (1 << 8)) >> 8) && gSaveBlock1Ptr->location.mapNum == ((79 | (1 << 8)) & 0xFF))
        {
            ...
            data[2] = 8;
            LoadPalette(sChampionRoomLightingPalettes[0], (0x000 + ((7) * 16)), ((16) * sizeof(u16)));
        }
        else
        {
            data[0] = sEliteFourLightingTimers[0];
            data[2] = 11;
            LoadPalette(sEliteFourLightingPalettes[0], (0x000 + ((7) * 16)), ((16) * sizeof(u16)));
        }
        data[1] = 0;
...
static void Task_RunPokemonLeagueLightingEffect(u8 taskId)
{
    ...
    )
    {
        ...
        if (gSaveBlock1Ptr->location.mapGroup == ((79 | (1 << 8)) >> 8) && gSaveBlock1Ptr->location.mapNum == ((79 | (1 << 8)) & 0xFF))
        {
            ...
            LoadPalette(sChampionRoomLightingPalettes[data[1]], (0x000 + ((7) * 16)), ((16) * sizeof(u16)));
        }
        else
        {
            data[0] = sEliteFourLightingTimers[data[1]];
            LoadPalette(sEliteFourLightingPalettes[data[1]], (0x000 + ((7) * 16)), ((16) * sizeof(u16)));
        }
        ApplyGlobalTintToPaletteSlot(7, 1);
    }
}
...
{
    s16 *data = gTasks[taskId].data;
    if (FlagGet((0x0 + 0x04)) != 0)
    {
        if (gSaveBlock1Ptr->location.mapGroup == ((79 | (1 << 8)) >> 8) && gSaveBlock1Ptr->location.mapNum == ((79 | (1 << 8)) & 0xFF))
            LoadPalette(sChampionRoomLightingPalettes[8], (0x000 + ((7) * 16)), ((16) * sizeof(u16)));
        else
            LoadPalette(sEliteFourLightingPalettes[11], (0x000 + ((7) * 16)), ((16) * sizeof(u16)));
        ApplyGlobalTintToPaletteSlot(7, 1);
        if (gPaletteFade.active)
        {
            BlendPalettes(0x00000080, 16, ((0) | ((0) << 5) | ((0) << 10)));
        }
        DestroyTask(taskId);
    }
}
...
void StopPokemonLeagueLightingEffectTask(void)
{
    if (FuncIsActiveTask(Task_RunPokemonLeagueLightingEffect) == 1)
    {
...
bool8 CapeBrinkGetMoveToTeachLeadPokemon(void)
{
    ...
    for (i = 0; i < (sizeof(sCapeBrinkCompatibleSpecies) / sizeof((sCapeBrinkCompatibleSpecies)[0])); i++)
    {
        if (GetMonData(&gPlayerParty[leadMonSlot], 65, ((void *)0)) == sCapeBrinkCompatibleSpecies[i])
        {
            tutorMonId = i;
            break;
        }
    }
    if (i == (sizeof(sCapeBrinkCompatibleSpecies) / sizeof((sCapeBrinkCompatibleSpecies)[0])) || GetMonData(&gPlayerParty[leadMonSlot], 32) != 255)
        return 0;
    if (tutorMonId == 0)
    {
        StringCopy(gStringVar2, gMoveNames[338]);
    ...
    {
        StringCopy(gStringVar2, gMoveNames[307]);
    ...
    {
        StringCopy(gStringVar2, gMoveNames[308]);
        gSpecialVar_0x8005 = 17;
        if (FlagGet(0x2E0) == 1)
            return 0;
    }
    if (GetMonData(&gPlayerParty[leadMonSlot], 13) != 0)
        numMovesKnown++;
    if (GetMonData(&gPlayerParty[leadMonSlot], 14) != 0)
        numMovesKnown++;
    if (GetMonData(&gPlayerParty[leadMonSlot], 15) != 0)
        numMovesKnown++;
    if (GetMonData(&gPlayerParty[leadMonSlot], 16) != 0)
        numMovesKnown++;
...
{
    if (FlagGet(0x2E3) != 1
     && gSaveBlock1Ptr->location.mapGroup == ((61 | (3 << 8)) >> 8)
     && gSaveBlock1Ptr->location.mapNum == ((61 | (3 << 8)) & 0xFF)
     && gSaveBlock1Ptr->pos.x == 24
     && gSaveBlock1Ptr->pos.y == 25
     && GetPlayerFacingDirection() == 2
    )
        return 1;
    else
        return 0;
...
static void Task_DoDeoxysTriangleInteraction(u8 taskId)
{
    u16 r5;
    u16 r6;
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x48)) == 1)
    {
...
{
    u8 mapObjId;
    LoadPalette(sDeoxysObjectPals[num], (0x100 + ((10) * 16)), ((4) * sizeof(u16)));
...
static void Task_WaitDeoxysFieldEffect(u8 taskId)
{
    if (!FieldEffectActiveListContains(67))
    {
        ScriptContext_Enable();
        DestroyTask(taskId);
    }
...
void IncrementBirthIslandRockStepCount(void)
{
    u16 count = VarGet(0x4026);
    if (gSaveBlock1Ptr->location.mapGroup == ((56 | (2 << 8)) >> 8) && gSaveBlock1Ptr->location.mapNum == ((56 | (2 << 8)) & 0xFF))
    {
        count++;
        if (count > 99)
            VarSet(0x4026, 0);
...
{
    if (gSaveBlock1Ptr->location.mapGroup == ((10 | (2 << 8)) >> 8) && gSaveBlock1Ptr->location.mapNum == ((10 | (2 << 8)) & 0xFF))
        return 0;
    else
        return 1;
}
...
bool8 PlayerPartyContainsSpeciesWithPlayerID(void)
{
    ...
    {
        if (GetMonData(&gPlayerParty[i], 65, ((void *)0)) == gSpecialVar_0x8004
            && GetPlayerTrainerId() == GetMonData(&gPlayerParty[i], 1, ((void *)0)))
            return 1;
    }
    return 0;
}
...
{
    u32 numHofClears = GetGameStat(10);
    if (numHofClears >= 25)
    {
        FlagClear(0x0A5);
        if (numHofClears >= 50)
