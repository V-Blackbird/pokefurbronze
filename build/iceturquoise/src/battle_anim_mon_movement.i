...
# 1 "include/task.h" 1
# 5 "src/battle_anim_mon_movement.c" 2
# 1 "include/trig.h" 1
...
# 146 "src/battle_anim_mon_movement.c"
...
# 232 "src/battle_anim_mon_movement.c"
...
# 333 "src/battle_anim_mon_movement.c"
...
# 377 "src/battle_anim_mon_movement.c"
...
# 500 "src/battle_anim_mon_movement.c"
static void SlideMonToOffset(struct Sprite *sprite)
{
    u8 battlerId;
    u8 monSpriteId;
...
# 579 "src/battle_anim_mon_movement.c"
...
static void AnimTask_SlideOffScreen_Step(u8 taskId)
{
    ...
    if (gSprites[spriteId].x2 + gSprites[spriteId].x < -32 || gSprites[spriteId].x2 + gSprites[spriteId].x > 240 + 32)
        DestroyAnimVisualTask(taskId);
}
# 680 "src/battle_anim_mon_movement.c"
...
static void AnimTask_SwayMon_Step(u8 taskId)
{
    ...
    else if (GetBattlerSide(gTasks[taskId].data[5]) == 0)
    {
...
void AnimTask_RotateMonSpriteToSide(u8 taskId)
{
    ...
    else
    {
        if (gBattleAnimArgs[2] == 0)
            gTasks[taskId].data[7] = GetBattlerSide(gBattleAnimAttacker) == 0 ? 1 : 0;
        else
            gTasks[taskId].data[7] = GetBattlerSide(gBattleAnimTarget) == 0 ? 1 : 0;
    }
