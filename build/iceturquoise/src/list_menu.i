...
static __attribute__((section("ewram_data"))) struct MysteryGiftLinkMenuStruct sMysteryGiftLinkMenu = {0};
...
static s32 ListMenuTestInput(struct ListMenuTemplate *template, u32 cursorPos, u32 itemsAbove, u16 keys, u16 *newCursorPos, u16 *newItemsAbove)
{
    ...
    if (newCursorPos != ((void *)0))
        *newCursorPos = list.cursorPos;
    if (newItemsAbove != ((void *)0))
...
static void ListMenuPrint(struct ListMenu *list, const u8 *str, u8 x, u8 y)
{
    ...
    if (gListMenuOverride.enabled)
    {
        ...
        AddTextPrinterParameterized4(list->template.windowId, gListMenuOverride.fontId, x, y, gListMenuOverride.lettersSpacing, 0, colors, 0xFF, str);
        gListMenuOverride.enabled = 0;
    }
    else
    {
        ...
        AddTextPrinterParameterized4(list->template.windowId, list->template.fontId, x, y, list->template.lettersSpacing, 0, colors, 0xFF, str);
    }
}
...
static void ListMenuPrintEntries(struct ListMenu *list, u16 startIndex, u16 yOffset, u16 count)
{
    ...
    for (i = 0; i < count; i++)
    {
        if (list->template.items[startIndex].index != -3)
...
static u8 ListMenuUpdateSelectedRowIndexAndScrollOffset(struct ListMenu *list, bool8 movingDown)
{
    ...
    if (!movingDown)
    {
        ...
        if (cursorPos == 0)
        {
            ...
            {
                itemsAbove--;
                if (list->template.items[cursorPos + itemsAbove].index != -3)
                {
                    list->itemsAbove = itemsAbove;
                    return 1;
                }
...
static bool8 ListMenuChangeSelection(struct ListMenu *list, bool8 updateCursorAndCallCallback, u8 count, bool8 movingDown)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        while (list->template.items[list->cursorPos + list->itemsAbove].index == -3);
    }
...
void ListMenu_LoadMonIconPalette(u8 palOffset, u16 speciesId)
{
    LoadPalette(GetValidMonIconPalettePtr(speciesId), palOffset, ((16) * sizeof(u16)));
}
...
void ListMenuLoadStdPalAt(u8 palOffset, u8 palId)
{
    ...
    LoadPalette(palette, palOffset, ((16) * sizeof(u16)));
}
