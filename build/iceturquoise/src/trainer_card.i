...
struct TrainerCardData
{
    ...
    bool8 var_F;
    bool8 hasTrades;
    bool8 hasBadge[(1 + ((((0x4FF + 1) + 768 - 1) + 1) + 0x27) - ((((0x4FF + 1) + 768 - 1) + 1) + 0x20))];
    ...
    u16 backTilemap[600];
    u16 bgTilemap[600];
    u8 badgeTiles[0x80 * (1 + ((((0x4FF + 1) + 768 - 1) + 1) + 0x27) - ((((0x4FF + 1) + 768 - 1) + 1) + 0x20))];
...
__attribute__((section("ewram_data"))) static struct TrainerCardData *sTrainerCardDataPtr = ((void *)0);
...
static const u8 sTimeColonInvisibleTextColors[] = {0x0, 0x0, 0x0};
static const u8 sTrainerCardFontIds[] = {FONT_SMALL, FONT_NORMAL, FONT_SMALL};

static const u8 sTrainerPicOffsets[2][2][2] =
{
...
static const u8 sLinkTrainerPicFacilityClasses[2][8] =
{
    [0] =
    {
        116,
...
static const struct TrainerCard sLinkPlayerTrainerCardTemplate1 =
{
    ...
    .version = 4,
    .hasAllFrontierSymbols = 0,
    .berryCrushPoints = 5555,
    .unionRoomNum = 8500,
    .berriesPicked = 5456,
    .jumpsInRow = 6300,
    .shouldDrawStickers = 1,
    .hasAllMons = 1,
    .monIconTint = 2,
    .facilityClass = 0,
    .stickers = {1, 2, 3},
...
static void CloseTrainerCard(u8 taskId)
{
    ...
    { Free(sTrainerCardDataPtr); sTrainerCardDataPtr = ((void *)0); };
    DestroyTask(taskId);
}
# 491 "src/trainer_card.c"
static void Task_TrainerCard(u8 taskId)
{
    ...
    case 8:
        ...
        {
            PlaySE(244);
            sTrainerCardDataPtr->mainState = 10;
        }
        break;
    case 9:
        if (!IsSEPlaying())
    ...
    case 10:
        ...
        if (!gReceivedRemoteLinkPlayers && sTrainerCardDataPtr->timeColonNeedDraw)
        {
            PrintTimeOnCard();
            DrawTrainerCardWindow(1);
            sTrainerCardDataPtr->timeColonNeedDraw = 0;
        }
        ...
        if (({(gMain.newKeys) & (0x0001);}))
        {
            ...
            FlipTrainerCard();
            PlaySE(242);
            sTrainerCardDataPtr->mainState = 12;
        }
        else if (({(gMain.newKeys) & (0x0002);}))
        {
            if (gReceivedRemoteLinkPlayers && sTrainerCardDataPtr->isLink && InUnionRoom() == 1)
            {
                sTrainerCardDataPtr->mainState = 15;
            }
            else
            {
                BeginNormalPaletteFade((0x0000FFFF | 0xFFFF0000), 0, 0, 16, ((0) | ((0) << 5) | ((0) << 10)));
                sTrainerCardDataPtr->mainState = 14;
            }
        }
        break;
    case 12:
        if (IsCardFlipTaskActive() && Overworld_LinkRecvQueueLengthMoreThan2() != 1)
        {
            PlaySE(244);
            sTrainerCardDataPtr->mainState = 11;
        }
        break;
    case 11:
        if (({(gMain.newKeys) & (0x0002);}))
        {
            ...
            else
            {
                SetHelpContext(10);
                FlipTrainerCard();
                sTrainerCardDataPtr->mainState = 13;
                PlaySE(242);
            }
        }
    ...
    case 15:
        ...
        AddTextPrinterParameterized(0, FONT_NORMAL, gText_WaitingTrainerFinishReading, 0, 1, 0xFF, 0);
        CopyWindowToVram(0, COPYWIN_FULL);
        sTrainerCardDataPtr->mainState = 16;
        break;
    case 16:
...
static bool8 LoadCardGfx(void)
{
    ...
    case 4:
        ...
        break;
    case 5:
        if (sTrainerCardDataPtr->cardType == 0)
...
static void SetPlayerCardData(struct TrainerCard *trainerCard, u8 cardType)
{
    ...
    trainerCard->rse.hasPokedex = FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x29));
    ...
    trainerCard->rse.linkBattleWins = GetCappedGameStat(23, 9999);
    trainerCard->rse.linkBattleLosses = GetCappedGameStat(24, 9999);
    trainerCard->rse.pokemonTrades = GetCappedGameStat(21, 0xFFFF);
    ...
    trainerCard->rse.hasAllPaintings = 0;
...
void TrainerCard_GenerateCardForLinkPlayer(struct TrainerCard *trainerCard)
{
    ...
    trainerCard->version = 5;
    SetPlayerCardData(trainerCard, 1);
    if (GetCardType() != 0)
        return;
    ...
    trainerCard->berryCrushPoints = GetCappedGameStat(51, 0xFFFF);
    trainerCard->unionRoomNum = GetCappedGameStat(50, 0xFFFF);
    trainerCard->shouldDrawStickers = 1;
    ...
    if (trainerCard->rse.gender == 1)
        trainerCard->facilityClass = sLinkTrainerPicFacilityClasses[1][id];
    else
        trainerCard->facilityClass = sLinkTrainerPicFacilityClasses[0][id];

    trainerCard->stickers[0] = VarGet(0x4049);
    trainerCard->stickers[1] = VarGet(0x404A);
    trainerCard->stickers[2] = VarGet(0x404B);

    trainerCard->monIconTint = VarGet(0x4042);

    trainerCard->monSpecies[0] = MailSpeciesToIconSpecies(VarGet(0x4043));
    trainerCard->monSpecies[1] = MailSpeciesToIconSpecies(VarGet(0x4044));
    trainerCard->monSpecies[2] = MailSpeciesToIconSpecies(VarGet(0x4045));
    trainerCard->monSpecies[3] = MailSpeciesToIconSpecies(VarGet(0x4046));
    trainerCard->monSpecies[4] = MailSpeciesToIconSpecies(VarGet(0x4047));
    trainerCard->monSpecies[5] = MailSpeciesToIconSpecies(VarGet(0x4048));
}
...
static void SetDataFromTrainerCard(void)
{
    ...
    sTrainerCardDataPtr->hasPokedex = 0;
    sTrainerCardDataPtr->hasHofResult = 0;
    sTrainerCardDataPtr->hasLinkResults = 0;
    sTrainerCardDataPtr->hasBattleTowerWins = 0;
    sTrainerCardDataPtr->var_E = 0;
    sTrainerCardDataPtr->var_F = 0;
    sTrainerCardDataPtr->hasTrades = 0;

    memset(sTrainerCardDataPtr->hasBadge, 0, sizeof(sTrainerCardDataPtr->hasBadge));
    ...
    for (i = 0, badgeFlag = ((((0x4FF + 1) + 768 - 1) + 1) + 0x20); badgeFlag <= ((((0x4FF + 1) + 768 - 1) + 1) + 0x27); badgeFlag++, i++)
    {
...
static void HandleGpuRegs(void)
{
    ...
    ShowBg(2);
    ShowBg(3);
    SetGpuReg(0x0, 0x2000 | 0x1000 | 0x0040 | 0x0F00);
    SetGpuReg(0x50, (1 << 0) | (3 << 6));
    SetGpuReg(0x54, 0);
    SetGpuReg(0x48, ((1 << 0) | (1 << 1) | (1 << 2) | (1 << 3)) | (1 << 4) | (1 << 5));
    SetGpuReg(0x4a, (1 << 1) | (1 << 2) | (1 << 3) | (1 << 4));
    SetGpuReg(0x44, (1 << 5) | (((0) << 8) | (0x80)));
    SetGpuReg(0x40, (1 << 5) | (1 << 4) | (((0) << 8) | (0xC0)));
    if (gReceivedRemoteLinkPlayers)
...
static void UpdateCardFlipRegs(u16 cardTop)
{
    ...
    SetGpuReg(0x54, sTrainerCardDataPtr->flipBlendY);
    SetGpuReg(0x44, (((sTrainerCardDataPtr->cardTop) << 8) | (160 - sTrainerCardDataPtr->cardTop)));
}
...
static void InitBgsAndWindows(void)
{
    ...
    InitBgsFromTemplates(0, sTrainerCardBgTemplates, (sizeof(sTrainerCardBgTemplates) / sizeof((sTrainerCardBgTemplates)[0])));
    ChangeBgX(0, 0, 0);
    ChangeBgY(0, 0, 0);
    ChangeBgX(1, 0, 0);
    ChangeBgY(1, 0, 0);
...
static void PrintNameOnCardFront(void)
{
    ...
    AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardFrontNameXPositions[sTrainerCardDataPtr->cardType], sTrainerCardFrontNameYPositions[sTrainerCardDataPtr->cardType], sTrainerCardTextColors, 0xFF, buffer[0]);
}
...
static void PrintIdOnCard(void)
{
    ...
    AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardIdXPositions[sTrainerCardDataPtr->cardType], sTrainerCardIdYPositions[sTrainerCardDataPtr->cardType], sTrainerCardTextColors, 0xFF, buffer);
}
...
static void PrintMoneyOnCard(void)
{
    ...
    if (sTrainerCardDataPtr->cardType != 1)
    {
        ...
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 20, 56, sTrainerCardTextColors, 0xFF, gText_TrainerCardMoney);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], x, 56, sTrainerCardTextColors, 0xFF, buffer);
    }
    else
    {
        x = 118 - 6 * StringLength(buffer);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 16, 57, sTrainerCardTextColors, 0xFF, gText_TrainerCardMoney);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], x, 57, sTrainerCardTextColors, 0xFF, buffer);
    }
}
...
static void PrintPokedexOnCard(void)
{
    ...
    if (FlagGet(((((0x4FF + 1) + 768 - 1) + 1) + 0x29)))
    {
        ...
        if (sTrainerCardDataPtr->cardType != 1)
        {
            ...
            AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 20, 72, sTrainerCardTextColors, 0xFF, gText_TrainerCardPokedex);
            AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], x, 72, sTrainerCardTextColors, 0xFF, buffer);
            AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 138, 72, sTrainerCardTextColors, 0xFF, gText_TrainerCardNull);
        }
        else
        {
            x = 120 - 6 * StringLength(buffer);
            AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 16, 73, sTrainerCardTextColors, 0xFF, gText_TrainerCardPokedex);
            AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], x, 73, sTrainerCardTextColors, 0xFF, buffer);
            AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 138, 73, sTrainerCardTextColors, 0xFF, gText_TrainerCardNull);
        }
    }
}
...
static void PrintTimeOnCard(void)
{
    ...
    FillWindowPixelRect(1, ((0) | ((0) << 4)), sTrainerCardTimeHoursXPositions[sTrainerCardDataPtr->cardType], sTrainerCardTimeMinutesYPositions[sTrainerCardDataPtr->cardType], 50, 12);
    if (sTrainerCardDataPtr->cardType != 1)
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 20, 88, sTrainerCardTextColors, 0xFF, gText_TrainerCardTime);
    else
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 16, 89, sTrainerCardTextColors, 0xFF, gText_TrainerCardTime);
    ...
    AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardTimeHoursXPositions[sTrainerCardDataPtr->cardType],
        sTrainerCardTimeMinutesYPositions[sTrainerCardDataPtr->cardType], sTrainerCardTextColors, 0xFF, buffer);
    AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardTimeHoursYPositions[sTrainerCardDataPtr->cardType],
        sTrainerCardTimeMinutesYPositions[sTrainerCardDataPtr->cardType], sTimeColonTextColors[sTrainerCardDataPtr->timeColonInvisible], 0xFF, gText_Colon2);

    ConvertIntToDecimalStringN(buffer, minutes, STR_CONV_MODE_LEADING_ZEROS, 2);
    AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardTimeMinutesXPositions[sTrainerCardDataPtr->cardType], sTrainerCardTimeMinutesYPositions[sTrainerCardDataPtr->cardType], sTrainerCardTextColors, 0xFF, buffer);
}
...
static void PrintProfilePhraseOnCard(void)
{
    if (sTrainerCardDataPtr->isLink)
    {
        AddTextPrinterParameterized3(1, FONT_NORMAL, 10, sTrainerCardProfilePhraseXPositions[sTrainerCardDataPtr->cardType],
            sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->easyChatProfile[0]);
        ...
        AddTextPrinterParameterized3(1, FONT_NORMAL, GetStringWidth(FONT_NORMAL, sTrainerCardDataPtr->easyChatProfile[0], 0) + 16, sTrainerCardProfilePhraseXPositions[sTrainerCardDataPtr->cardType],
            sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->easyChatProfile[1]);

        AddTextPrinterParameterized3(1, FONT_NORMAL, 10, sTrainerCardProfilePhraseYPositions[sTrainerCardDataPtr->cardType],
            sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->easyChatProfile[2]);
        ...
        AddTextPrinterParameterized3(1, FONT_NORMAL, GetStringWidth(FONT_NORMAL, sTrainerCardDataPtr->easyChatProfile[2], 0) + 16, sTrainerCardProfilePhraseYPositions[sTrainerCardDataPtr->cardType],
            sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->easyChatProfile[3]);
    }
}
...
static void PrintNameOnCardBack(void)
{
    ...
    if (sTrainerCardDataPtr->cardType == 0)
    {
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardBackNameXPositions[sTrainerCardDataPtr->cardType],
            sTrainerCardBackNameYPositions[sTrainerCardDataPtr->cardType], sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_NAME]);
    }
    else
    {
        ...
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], x, sTrainerCardBackNameYPositions[sTrainerCardDataPtr->cardType],
            sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_NAME]);
    }
}
...
static void PrintHofDebutTimeOnCard(void)
{
    if (sTrainerCardDataPtr->hasHofResult)
    {
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardHofDebutXPositions[sTrainerCardDataPtr->cardType], 35, sTrainerCardTextColors, 0xFF, gText_HallOfFameDebut);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 164, 35, sTrainerCardStatColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_HOF_TIME]);
    }
}
...
static void PrintLinkBattleResultsOnCard(void)
{
    if (sTrainerCardDataPtr->hasLinkResults)
    {
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardHofDebutXPositions[sTrainerCardDataPtr->cardType], 51,
            sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_LINK_RECORD]);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 130, 51, sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_WIN_LOSS]);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 144, 51, sTrainerCardStatColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_LINK_WINS]);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 192, 51, sTrainerCardStatColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_LINK_LOSSES]);
    }
}
...
static void PrintTradesStringOnCard(void)
{
    if (sTrainerCardDataPtr->hasTrades)
    {
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardHofDebutXPositions[sTrainerCardDataPtr->cardType], 67, sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_TRADES]);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 186, 67, sTrainerCardStatColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_TRADE_COUNT]);
    }
}
...
static void PrintBerryCrushStringOnCard(void)
{
    if (sTrainerCardDataPtr->cardType != 1 && sTrainerCardDataPtr->trainerCard.berryCrushPoints)
    {
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardHofDebutXPositions[sTrainerCardDataPtr->cardType], 99, sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_BERRY_CRUSH]);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 186, 99, sTrainerCardStatColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_BERRY_CRUSH_COUNT]);
    }
}
...
static void PrintUnionStringOnCard(void)
{
    if (sTrainerCardDataPtr->cardType != 1 && sTrainerCardDataPtr->trainerCard.unionRoomNum)
    {
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], sTrainerCardHofDebutXPositions[sTrainerCardDataPtr->cardType], 83, sTrainerCardTextColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_UNION_ROOM]);
        AddTextPrinterParameterized3(1, sTrainerCardFontIds[1], 186, 83, sTrainerCardStatColors, 0xFF, sTrainerCardDataPtr->strings[TRAINER_CARD_STRING_UNION_ROOM_NUM]);
    }
}
...
static void LoadMonIconGfx(void)
{
    ...
    CpuSet(gMonIconPalettes, sTrainerCardDataPtr->monIconPals, 0x00000000 | ((2 * (sizeof(sTrainerCardDataPtr->monIconPals) / sizeof((sTrainerCardDataPtr->monIconPals)[0])))/(16/8) & 0x1FFFFF));
    ...
    LoadPalette(sTrainerCardDataPtr->monIconPals, (0x000 + ((5) * 16)), sizeof(sTrainerCardDataPtr->monIconPals));
    for (i = 0; i < 6; i++)
    {
...
static void PrintStickersOnCard(void)
{
    ...
    if (sTrainerCardDataPtr->cardType == 0 && sTrainerCardDataPtr->trainerCard.shouldDrawStickers == 1)
    {
        for (i = 0; i < 3; i++)
        {
...
static void LoadStickerGfx(void)
{
    LoadPalette(sTrainerCardStickerPal1, (0x000 + ((11) * 16)), sizeof(sTrainerCardStickerPal1));
    LoadPalette(sTrainerCardStickerPal2, (0x000 + ((12) * 16)), sizeof(sTrainerCardStickerPal2));
    LoadPalette(sTrainerCardStickerPal3, (0x000 + ((13) * 16)), sizeof(sTrainerCardStickerPal3));
    LoadPalette(sTrainerCardStickerPal4, (0x000 + ((14) * 16)), sizeof(sTrainerCardStickerPal4));
...
static bool8 SetTrainerCardBgsAndPals(void)
{
    ...
    {
    case 0:
        LoadBgTiles(3, sTrainerCardDataPtr->badgeTiles, (sizeof(sTrainerCardDataPtr->badgeTiles) / sizeof((sTrainerCardDataPtr->badgeTiles)[0])), 0);
        break;
    case 1:
    ...
    case 2:
        if (sTrainerCardDataPtr->cardType == 1)
            LoadPalette(sHoennTrainerCardPals[sTrainerCardDataPtr->trainerCard.rse.stars], (0x000 + ((0) * 16)), 3 * ((16) * sizeof(u16)));
        else
            LoadPalette(sKantoTrainerCardPals[sTrainerCardDataPtr->trainerCard.rse.stars], (0x000 + ((0) * 16)), 3 * ((16) * sizeof(u16)));
        break;
    case 3:
        if (sTrainerCardDataPtr->cardType == 1)
            LoadPalette(sHoennTrainerCardBadges_Pal, (0x000 + ((3) * 16)), sizeof(sHoennTrainerCardBadges_Pal));
        else
            LoadPalette(sKantoTrainerCardBadges_Pal, (0x000 + ((3) * 16)), sizeof(sKantoTrainerCardBadges_Pal));
        break;
    case 4:
        if (sTrainerCardDataPtr->cardType == 1 && sTrainerCardDataPtr->trainerCard.rse.gender != 0)
            LoadPalette(sHoennTrainerCardFemaleBg_Pal, (0x000 + ((1) * 16)), sizeof(sHoennTrainerCardFemaleBg_Pal));
        else if (sTrainerCardDataPtr->trainerCard.rse.gender != 0)
            LoadPalette(sKantoTrainerCardFemaleBg_Pal, (0x000 + ((1) * 16)), sizeof(sKantoTrainerCardFemaleBg_Pal));
        break;
    case 5:
        LoadPalette(sTrainerCardStar_Pal, (0x000 + ((4) * 16)), sizeof(sTrainerCardStar_Pal));
        break;
    case 6:
...
static void DrawStarsAndBadgesOnCard(void)
{
    ...
    {
        x = 4;
        for (i = 0; i < (1 + ((((0x4FF + 1) + 768 - 1) + 1) + 0x27) - ((((0x4FF + 1) + 768 - 1) + 1) + 0x20)); i++, tileNum += 2, x += 3)
        {
...
static void BlinkTimeColon(void)
{
    if (++sTrainerCardDataPtr->timeColonBlinkTimer > 60)
    {
        ...
        sTrainerCardDataPtr->timeColonNeedDraw = 1;
    }
}
...
static bool8 Task_AnimateCardFlipDown(struct Task* task)
{
    ...
    sTrainerCardDataPtr->allowDMACopy = 0;
    if (task->data[1] >= 77)
        task->data[1] = 77;
    else
        task->data[1] += 7;
    ...
    sTrainerCardDataPtr->allowDMACopy = 1;
    if (task->data[1] >= 77)
        task->data[0]++;
...
void ShowPlayerTrainerCard(void (*callback)(void))
{
    ...
    if (InUnionRoom() == 1)
        sTrainerCardDataPtr->isLink = 1;
    else
        sTrainerCardDataPtr->isLink = 0;

    sTrainerCardDataPtr->language = (2);
...
static void InitTrainerCardData(void)
{
    ...
    sTrainerCardDataPtr->timeColonInvisible = 0;
    sTrainerCardDataPtr->onBack = 0;
    sTrainerCardDataPtr->flipBlendY = 0;
    if (GetCardType() == 1)
        sTrainerCardDataPtr->cardType = 1;
    else
        sTrainerCardDataPtr->cardType = 0;
...
static u8 GetCardType(void)
{
    ...
    {
        if (sTrainerCardDataPtr->trainerCard.version == 4 || sTrainerCardDataPtr->trainerCard.version == 5)
            return 0;
        else
            return 1;
    }
...
static void CreateTrainerCardTrainerPic(void)
{
    ...
    if (InUnionRoom() == 1 && gReceivedRemoteLinkPlayers == 1)
    {
        ...
        CreateTrainerCardTrainerPicSprite(FacilityClassToPicIndex(facilityClass), 1, sTrainerPicOffsets[sTrainerCardDataPtr->cardType][sTrainerCardDataPtr->trainerCard.rse.gender][0],
    ...
    else
    {
        ...
        {
            CreateTrainerCardTrainerPicSprite(PlayerGenderToFrontTrainerPicId(sTrainerCardDataPtr->trainerCard.rse.gender, 1), 1,
