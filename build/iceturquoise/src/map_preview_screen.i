# 0 "src/map_preview_screen.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/map_preview_screen.c"
...
# 13 "src/map_preview_screen.c" 2
# 1 "include/constants/region_map_sections.h" 1
# 14 "src/map_preview_screen.c" 2

static __attribute__((section("ewram_data"))) bool8 sHasVisitedMapBefore = 0;
static __attribute__((section("ewram_data"))) bool8 sAllocedBg0TilemapBuffer = 0;
...
bool8 MapHasPreviewScreen(u8 mapsec, u8 type)
{
    ...
    if (idx != MPS_COUNT)
    {
        ...
        {
            return sMapPreviewScreenData[idx].type == type ? 1 : 0;
        }
    }
    else
    {
...
void MapPreview_InitBgs(void)
{
    InitBgsFromTemplates(0, sMapPreviewBgTemplate, (sizeof(sMapPreviewBgTemplate) / sizeof((sMapPreviewBgTemplate)[0])));
    ShowBg(0);
}
...
void MapPreview_LoadGfx(u8 mapsec)
{
    ...
    if (idx != MPS_COUNT)
    {
       ...
       LoadPalette(sMapPreviewScreenData[idx].palptr, (0x000 + ((13) * 16)), 3 * ((16) * sizeof(u16)));
       DecompressAndCopyTileDataToVram(0, sMapPreviewScreenData[idx].tilesptr, 0, 0, 0);
       if (GetBgTilemapBuffer(0) == ((void *)0))
       {
...
void MapPreview_StartForestTransition(u8 mapsec)
{
    ...
    gTasks[taskId].data[4] = GetGpuReg(0x50);
    gTasks[taskId].data[5] = GetGpuReg(0x52);
    gTasks[taskId].data[3] = GetGpuReg(0x0);
    gTasks[taskId].data[6] = GetGpuReg(0x48);
    gTasks[taskId].data[7] = GetGpuReg(0x4a);
    ...
    SetGpuRegBits(0x48, (1 << 5) | (1 << 13));
    SetGpuRegBits(0x4a, (1 << 5));
...
bool32 ForestMapPreviewScreenIsRunning(void)
{
    if (FuncIsActiveTask(Task_RunMapPreviewScreenForest) == 1)
    {
        return 0;
    }
    else
...
static void Task_RunMapPreviewScreenForest(u8 taskId)
{
    ...
    case 4:
        ...
        data[1] = (data[1] + 1) % 3;
        SetGpuReg(0x52, (((data[9]) << 8) | (data[8])));
    ...
    case 5:
        if (!IsDma3ManagerBusyWithBgCopy())
        {
            ...
            SetGpuReg(0x0, data[3]);
            SetGpuReg(0x50, data[4]);
            SetGpuReg(0x52, data[5]);
            SetGpuReg(0x48, data[6]);
            SetGpuReg(0x4a, data[7]);
            DestroyTask(taskId);
        }
        break;
    }
...
u16 MapPreview_GetDuration(u8 mapsec)
{
    ...
    if (sMapPreviewScreenData[idx].type == 0)
    {
        if (!FlagGet(flagId))
        {
            return 120;
...
void MapPreview_SetFlag(u16 flagId)
{
    if (!FlagGet(flagId))
    {
        sHasVisitedMapBefore = 1;
    }
    else
    {
        sHasVisitedMapBefore = 0;
    }
    FlagSet(flagId);
}
