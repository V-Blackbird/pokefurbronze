...
static __attribute__((section("ewram_data"))) union AffineAnimCmd *sAnimTaskAffineAnim = ((void *)0);
...
const struct MonCoords gCastformFrontSpriteCoords[4] =
{
    [0] = { .size = ((((32) / (8)) + (((32) % (8)) ? 1 : 0)) << 4 | (((32) / (8)) + (((32) % (8)) ? 1 : 0))), .y_offset = 17 },
    [1] = { .size = ((((48) / (8)) + (((48) % (8)) ? 1 : 0)) << 4 | (((48) / (8)) + (((48) % (8)) ? 1 : 0))), .y_offset = 9 },
    [2] = { .size = ((((32) / (8)) + (((32) % (8)) ? 1 : 0)) << 4 | (((48) / (8)) + (((48) % (8)) ? 1 : 0))), .y_offset = 9 },
    [3] = { .size = ((((64) / (8)) + (((64) % (8)) ? 1 : 0)) << 4 | (((48) / (8)) + (((48) % (8)) ? 1 : 0))), .y_offset = 8 },
};

static const u8 sCastformElevations[4] =
{
    [0] = 13,
    [1] = 14,
    [2] = 13,
...
static const u8 sCastformBackSpriteYCoords[4] =
{
    [0] = 0,
    [1] = 0,
    [2] = 0,
...
{
    { gMiscBlank_Gfx, (64 * 64 / 2), 55125 },
    { gMiscBlank_Gfx, (64 * 64 / 2), 55126 },
};
...
static u8 GetBattlerYDelta(u8 battlerId, u16 species)
{
    ...
    if (GetBattlerSide(battlerId) == 0)
    {
        if (species == 201)
        {
            ...
            letter = (( (((personality) & 0x03000000) >> 18) | (((personality) & 0x00030000) >> 12) | (((personality) & 0x00000300) >> 6) | (((personality) & 0x00000003) >> 0) ) % 28);
            if (!letter)
                coordSpecies = species;
            else
                coordSpecies = letter + (479 + 1) - 1;
...
void InitBattleAnimBg(u32 bgId)
{
    ...
    { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, animBgData.bgTiles, 0x04000000 | 0x01000000 | ((0x2000)/(32/8) & 0x1FFFFF)); };
...
void AnimLoadCompressedBgGfx(u32 bgId, const u32 *src, u32 tilesOffset)
{
    { vu32 tmp = (vu32)(0); CpuSet((void *)&tmp, gBattleAnimBgTileBuffer, 0x04000000 | 0x01000000 | ((0x2000)/(32/8) & 0x1FFFFF)); };
...
void SetGreyscaleOrOriginalPalette(u16 paletteNum, bool8 restoreOriginalColor)
{
    ...
    {
        CpuSet(&gPlttBufferUnfaded[paletteNum], &gPlttBufferFaded[paletteNum], 0x04000000 | ((((16) * sizeof(u16)))/(32/8) & 0x1FFFFF));
    }
}
...
void AnimTask_BlendMonInAndOut(u8 task)
{
    ...
    gTasks[task].data[0] = (0x100 + ((gSprites[spriteId].oam.paletteNum) * 16)) + 1;
...
u8 CreateAdditionalMonSpriteForMoveAnim(u16 species, bool8 isBackpic, u8 templateId, s16 x, s16 y, u8 subpriority, u32 personality, u32 trainerId, u32 battlerId, bool32 ignoreDeoxys)
{
    ...
    if (gMonSpritesGfxPtr != ((void *)0) && gMonSpritesGfxPtr->multiUseBuffer == ((void *)0))
        gMonSpritesGfxPtr->multiUseBuffer = AllocZeroed(0x2000);
    if (!isBackpic)
    {
        LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(species, trainerId, personality), (0x100 + ((palette) * 16)), ((16) * sizeof(u16)));
        if (ignoreDeoxys == 1 || ShouldIgnoreDeoxysForm(5, battlerId) == 1 || gBattleSpritesDataPtr->battlerData[battlerId].transformSpecies != 0)
    ...
    RequestDma3Copy(gMonSpritesGfxPtr->multiUseBuffer, (void *)((void *)(0x6000000 + 0x10000) + (sheet * 0x20)), 0x800, 1);
...
s16 GetBattlerSpriteCoordAttr(u8 battlerId, u8 attr)
{
    ...
    case BATTLER_COORD_ATTR_LEFT:
        return GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X_2) - (((coords->size >> 4) * 8) / 2);
    case BATTLER_COORD_ATTR_RIGHT:
        return GetBattlerSpriteCoord(battlerId, BATTLER_COORD_X_2) + (((coords->size >> 4) * 8) / 2);
    case BATTLER_COORD_ATTR_TOP:
        return GetBattlerSpriteCoord(battlerId, BATTLER_COORD_Y_PIC_OFFSET) - (((coords->size & 0xF) * 8) / 2);
...
void AnimTask_AttackerPunchWithTrace(u8 taskId)
{
    ...
    task->data[4] = AllocSpritePalette((10000 + 97));
    task->data[5] = 0;

    dest = ((((task->data[4]) + 16) * 16));
    ...
    CpuSet(&gPlttBufferUnfaded[src], &gPlttBufferFaded[dest], 0x04000000 | ((((16) * sizeof(u16)))/(32/8) & 0x1FFFFF));
