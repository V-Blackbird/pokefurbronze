# 0 "src/mystery_gift_show_card.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/mystery_gift_show_card.c"
...
# 2 "src/mystery_gift_show_card.c" 2
# 1 "include/gflib.h" 1
...
# 3 "src/mystery_gift_show_card.c" 2
...
# 4 "src/mystery_gift_show_card.c" 2
# 1 "include/menu.h" 1
# 18 "include/menu.h"
struct MenuAction
{
...
# 5 "src/mystery_gift_show_card.c" 2
...
# 6 "src/mystery_gift_show_card.c" 2
...
# 7 "src/mystery_gift_show_card.c" 2
...
# 8 "src/mystery_gift_show_card.c" 2
...
# 9 "src/mystery_gift_show_card.c" 2
...
# 10 "src/mystery_gift_show_card.c" 2
# 1 "include/constants/mystery_gift.h" 1
# 11 "src/mystery_gift_show_card.c" 2
...
struct WonderCardData
{
             ...
             u8 monIconSpriteId;
             u8 stampSpriteIds[7][2];
             u8 titleText[40 + 1];
             u8 subtitleText[40 + 1];
             u8 idNumberText[7];
             u8 bodyText[4][40 + 1];
             u8 footerLine1Text[40 + 1];
             u8 footerLine2Text[40 + 1];
             struct CardStatTextData statTextData[8];
             u8 bgTilemapBuffer[0x1000];
};

static __attribute__((section("ewram_data"))) struct WonderCardData * sWonderCardData = ((void *)0);
...
static const u8 __attribute__((aligned(4))) sFooterTextOffsets[3] =
{
    [0] = 7,
    [1] = 4,
    [2] = 7
...
static const struct CompressedSpriteSheet sSpriteSheet_StampShadow = {
    sStampShadowGfx, 0x100, 0x8000
};

static const struct SpritePalette sSpritePalettes_StampShadow[] = {
    {sStampShadowPal0, 0x8000},
    {sStampShadowPal1, 0x8000},
    {sStampShadowPal2, 0x8000},
    {sStampShadowPal3, 0x8000},
    {sStampShadowPal4, 0x8000},
    {sStampShadowPal5, 0x8000},
    {sStampShadowPal6, 0x8000},
    {sStampShadowPal7, 0x8000}
};
...
static const struct WonderGraphics sCardGraphics[8] = {
...
bool32 WonderCard_Init(struct WonderCard * card, struct WonderCardMetadata * metadata)
{
    if (card == ((void *)0) || metadata == ((void *)0))
        return 0;
...
void WonderCard_Destroy(void)
{
    if (sWonderCardData != ((void *)0))
    {
        *sWonderCardData = (struct WonderCardData){};
        { Free(sWonderCardData); sWonderCardData = ((void *)0); };
    }
}
...
s32 WonderCard_Enter(void)
{
    ...
    switch(sWonderCardData->enterExitState)
    {
        ...
        case 3:
            ...
            LoadPalette(sWonderCardData->gfx->pal, (0x000 + ((1) * 16)), ((16) * sizeof(u16)));
...
static void BufferCardText(void)
{
    ...
    {
    case 0:
        memcpy(sWonderCardData->footerLine2Text, sWonderCardData->card.footerLine2Text, 40);
        sWonderCardData->footerLine2Text[40] = 0xFF;
        break;
    case 1:
        sWonderCardData->footerLine2Text[0] = 0xFF;
        break;
    case 2:
        sWonderCardData->footerLine2Text[0] = 0xFF;
        ...
        for (i = 0; i < (sizeof(sWonderCardData->statTextData) / sizeof((sWonderCardData->statTextData)[0])); i++)
        {
        ...
        for (i = 0, charsUntilStat = 0; i < 40; i++)
        {
            ...
            else
            {
                ...
                if (id >= (sizeof(stats) / sizeof((stats)[0])))
                {
                ...
                else
                {
                    ...
                    if (sWonderCardData->recordIdx >= (sizeof(sWonderCardData->statTextData) / sizeof((sWonderCardData->statTextData)[0])))
                        break;
                    charsUntilStat = 0;
                    i += 2;
                }
...
static void CreateCardSprites(void)
{
    ...
    {
        sWonderCardData->monIconSpriteId = CreateMonIcon_HandleDeoxys(MailSpeciesToIconSpecies(sWonderCardData->cardMetadata.iconSpecies), SpriteCallbackDummy, 220, 20, 0, 0);
