# 0 "src/pokemon_storage_system_graphics.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokemon_storage_system_graphics.c"
...
# 2 "src/pokemon_storage_system_graphics.c" 2
# 1 "include/gflib.h" 1
...
# 3 "src/pokemon_storage_system_graphics.c" 2
...
void ClearTempFieldEventData(void);
# 4 "src/pokemon_storage_system_graphics.c" 2
...
# 5 "src/pokemon_storage_system_graphics.c" 2
...
# 6 "src/pokemon_storage_system_graphics.c" 2
...
# 7 "src/pokemon_storage_system_graphics.c" 2
...
# 8 "src/pokemon_storage_system_graphics.c" 2
...
extern const u8 gText_PokeSum_EggOrigin_Trade[];
# 9 "src/pokemon_storage_system_graphics.c" 2
# 1 "include/task.h" 1
# 10 "src/pokemon_storage_system_graphics.c" 2
# 1 "include/trig.h" 1
...
s16 Sin2(u16 angle);
s16 Cos2(u16 angle);
# 11 "src/pokemon_storage_system_graphics.c" 2
...
static const u16 sUnusedColor = ((26) | ((29) << 5) | ((8) << 10));
...
void InitMonIconFields(void)
{
    ...
    LoadMonIconPalettes();
    for (i = 0; i < ((5 * 6) + 6 + 1 >= 40 ? (5 * 6) + 6 + 1 : 40); i++)
        gStorage->numIconsPerSpecies[i] = 0;
    for (i = 0; i < ((5 * 6) + 6 + 1 >= 40 ? (5 * 6) + 6 + 1 : 40); i++)
        gStorage->iconSpeciesList[i] = 0;
    for (i = 0; i < 6; i++)
        gStorage->partySprites[i] = ((void *)0);
    for (i = 0; i < (5 * 6); i++)
        gStorage->boxMonsSprites[i] = ((void *)0);

    gStorage->movingMonSprite = ((void *)0);
...
void CreateMovingMonIcon(void)
{
    u32 personality = GetMonData(&gStorage->movingMon, 0);
    u16 species = GetMonData(&gStorage->movingMon, 65);
    ...
    for (i = 0; i < 5; i++)
    {
        for (j = 0; j < 6; j++)
        {
            ...
            if (species != 0)
            {
                ...
                gStorage->boxMonsSprites[count] = ((void *)0);
    ...
    if (gStorage->boxOption == OPTION_MOVE_ITEMS)
    {
        for (boxPosition = 0; boxPosition < (5 * 6); boxPosition++)
        {
            if (GetBoxMonDataAt(boxId, boxPosition, 12) == 0)
                gStorage->boxMonsSprites[boxPosition]->oam.objMode = 1;
        }
    }
}
...
void CreateBoxMonIconAtPos(u8 boxPosition)
{
    ...
    if (species != 0)
    {
        ...
        gStorage->boxMonsSprites[boxPosition] = CreateMonIconSprite(species, personality, x, y, 2, 19 - (boxPosition % 6));
        if (gStorage->boxOption == OPTION_MOVE_ITEMS)
            gStorage->boxMonsSprites[boxPosition]->oam.objMode = 1;
    }
}
...
static void StartBoxMonIconsScrollOut(s16 speed)
{
    ...
    for (i = 0; i < (5 * 6); i++)
    {
        if (gStorage->boxMonsSprites[i] != ((void *)0))
        {
            gStorage->boxMonsSprites[i]->data[2] = speed;
            gStorage->boxMonsSprites[i]->data[4] = 1;
...
static void DestroyAllIconsInColumn(u8 column)
{
    ...
    for (row = 0; row < 5; row++)
    {
        if (gStorage->boxMonsSprites[boxPosition] != ((void *)0))
        {
            DestroyBoxMonIcon(gStorage->boxMonsSprites[boxPosition]);
            gStorage->boxMonsSprites[boxPosition] = ((void *)0);
        }
        boxPosition += 6;
    }
}
...
static u8 CreateBoxMonIconsInColumn(u8 column, u16 distance, s16 speed)
{
    ...
    if (gStorage->boxOption != OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < 5; i++)
        {
            if (gStorage->boxSpecies[boxPosition] != 0)
            {
                ...
                if (gStorage->boxMonsSprites[boxPosition] != ((void *)0))
                {
                    gStorage->boxMonsSprites[boxPosition]->data[1] = distance;
                    gStorage->boxMonsSprites[boxPosition]->data[2] = speed;
                    gStorage->boxMonsSprites[boxPosition]->data[3] = xDest;
        ...
        for (i = 0; i < 5; i++)
        {
            if (gStorage->boxSpecies[boxPosition] != 0)
            {
                ...
                if (gStorage->boxMonsSprites[boxPosition] != ((void *)0))
                {
                    ...
                    gStorage->boxMonsSprites[boxPosition]->data[3] = xDest;
                    gStorage->boxMonsSprites[boxPosition]->callback = SpriteCB_BoxMonIconScrollIn;
                    if (GetBoxMonDataAt(gStorage->incomingBoxId, boxPosition, 12) == 0)
                        gStorage->boxMonsSprites[boxPosition]->oam.objMode = 1;
                    count++;
                }
            }
...
static void InitBoxMonIconScroll(u8 boxId, s8 direction)
{
    ...
    if (direction > 0)
        ...
        gStorage->iconScrollCurColumn = 6 - 1;
...
static bool8 UpdateBoxMonIconScroll(void)
{
    ...
    case 1:
        ...
        if ((gStorage->iconScrollDirection > 0 && gStorage->iconScrollCurColumn == 6 - 1)
...
static void SetBoxSpeciesAndPersonalities(u8 boxId)
{
    ...
    for (i = 0; i < 5; i++)
    {
        for (j = 0; j < 6; j++)
        {
            gStorage->boxSpecies[boxPosition] = GetBoxMonDataAt(boxId, boxPosition, 65);
            if (gStorage->boxSpecies[boxPosition] != 0)
                gStorage->boxPersonalities[boxPosition] = GetBoxMonDataAt(boxId, boxPosition, 0);
            boxPosition++;
        }
    }
...
void CreatePartyMonsSprites(bool8 visible)
{
    ...
    if (!visible)
    {
        for (i = 0; i < count; i++)
        {
            ...
            gStorage->partySprites[i]->invisible = 1;
        }
    }
    ...
    if (gStorage->boxOption == OPTION_MOVE_ITEMS)
    {
        for (i = 0; i < 6; i++)
        {
            if (gStorage->partySprites[i] != ((void *)0) && GetMonData(&gPlayerParty[i], 12) == 0)
                gStorage->partySprites[i]->oam.objMode = 1;
        }
    }
}
...
void CompactPartySprites(void)
{
    ...
    for (i = 0, targetPartyId = 0; i < 6; i++)
    {
        if (gStorage->partySprites[i] != ((void *)0))
        {
            if (i != targetPartyId)
            {
...
# 669 "src/pokemon_storage_system_graphics.c"
...
static void SpriteCB_MovePartySpriteToNextSlot(struct Sprite *sprite)
{
    ...
    else
    {
        ...
        sprite->callback = SpriteCallbackDummy;
        gStorage->partySprites[sprite->data[1]] = sprite;
        gStorage->numPartySpritesToCompact--;
    }
}
# 722 "src/pokemon_storage_system_graphics.c"
void DestroyMovingMonIcon(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (gStorage->partySprites[i] != ((void *)0))
        {
            ...
            if (posY > 192)
                ...
                gStorage->partySprites[i]->invisible = 0;
        }
    }
}
...
void DestroyPartyMonIcon(u8 partyId)
{
    if (gStorage->partySprites[partyId] != ((void *)0))
    {
        DestroyBoxMonIcon(gStorage->partySprites[partyId]);
        gStorage->partySprites[partyId] = ((void *)0);
    }
}
...
void SetMovingMonSprite(u8 mode, u8 id)
{
    if (mode == MODE_PARTY)
    {
        ...
        gStorage->partySprites[id] = ((void *)0);
    }
    else if (mode == MODE_BOX)
    {
        gStorage->movingMonSprite = gStorage->boxMonsSprites[id];
        gStorage->boxMonsSprites[id] = ((void *)0);
    }
    else
        return;
...
void SetPlacedMonSprite(u8 boxId, u8 position)
{
    ...
    else
    {
        ...
        gStorage->boxMonsSprites[position]->subpriority = 19 - (position % 6);
    }
...
void DoReleaseMonAnim(u8 mode, u8 position)
{
    ...
    if (*gStorage->releaseMonSpritePtr != ((void *)0))
    {
        InitSpriteAffineAnim(*gStorage->releaseMonSpritePtr);
        (*gStorage->releaseMonSpritePtr)->oam.affineMode = 1;
...
bool8 TryHideReleaseMonSprite(void)
{
    if (*gStorage->releaseMonSpritePtr == ((void *)0) || (*gStorage->releaseMonSpritePtr)->invisible)
        return 0;

    if ((*gStorage->releaseMonSpritePtr)->affineAnimEnded)
        (*gStorage->releaseMonSpritePtr)->invisible = 1;
...
void DestroyReleaseMonIcon(void)
{
    if (*gStorage->releaseMonSpritePtr != ((void *)0))
    {
        ...
        *gStorage->releaseMonSpritePtr = ((void *)0);
    }
}
...
void DoReleaseMonComeBackAnim(void)
{
    if (*gStorage->releaseMonSpritePtr != ((void *)0))
    {
        (*gStorage->releaseMonSpritePtr)->invisible = 0;
...
static u16 TryLoadMonIconTiles(u16 species)
{
    ...
    if (i == ((5 * 6) + 6 + 1 >= 40 ? (5 * 6) + 6 + 1 : 40))
    {

        for (i = 0; i < ((5 * 6) + 6 + 1 >= 40 ? (5 * 6) + 6 + 1 : 40); i++)
        {
            if (gStorage->iconSpeciesList[i] == 0)
                break;
        }
        if (i == ((5 * 6) + 6 + 1 >= 40 ? (5 * 6) + 6 + 1 : 40))
            return 0xFFFF;
    ...
    offset = 16 * i;
    CpuSet(GetMonIconTiles(species, 1), (void *)((void *)(0x6000000 + 0x10000)) + offset * 32, 0x04000000 | ((0x200)/(32/8) & 0x1FFFFF));
...
static void Task_InitBox(u8 taskId)
{
    ...
    case 0:
        ...
        gStorage->bg2_X = 0;
        task->data[1] = RequestDma3Fill(0, gStorage->wallpaperBgTilemapBuffer, sizeof(gStorage->wallpaperBgTilemapBuffer), 1);
        break;
    case 1:
...
bool8 DoWallpaperGfxChange(void)
{
    ...
    {
    case 0:
        BeginNormalPaletteFade(gStorage->wallpaperPalBits, 1, 0, 16, (((31) | ((31) << 5) | ((31) << 10)) | (1 << 15)));
    ...
    case 2:
        ...
        {
            CycleBoxTitleColor();
            BeginNormalPaletteFade(gStorage->wallpaperPalBits, 1, 16, 0, (((31) | ((31) << 5) | ((31) << 10)) | (1 << 15)));
            gStorage->wallpaperChangeState++;
        }
        break;
    case 3:
...
static void LoadWallpaperGfx(u8 boxId, s8 direction)
{
    ...
    if (gStorage->wallpaperLoadDir != 0)
        LoadPalette(wallpaper->palettes, (0x000 + ((4) * 16)) + (0x000 + ((gStorage->wallpaperOffset * 2) * 16)), 2 * ((16) * sizeof(u16)));
    else
        CpuSet(wallpaper->palettes, &gPlttBufferUnfaded[(0x000 + ((4) * 16)) + (0x000 + ((gStorage->wallpaperOffset * 2) * 16))], 0x00000000 | ((2 * ((16) * sizeof(u16)))/(16/8) & 0x1FFFFF));
...
static void InitBoxTitle(u8 boxId)
{
    ...
    gStorage->boxTitlePalOffset = (0x100 + ((tagIndex) * 16)) + 14;
    gStorage->wallpaperPalBits |= (1 << 16) << tagIndex;

    tagIndex = IndexOfSpritePaletteTag(PALTAG_BOX_TITLE);
    gStorage->boxTitleAltPalOffset = (0x100 + ((tagIndex) * 16)) + 14;
    gStorage->wallpaperPalBits |= (1 << 16) << tagIndex;
...
static void CycleBoxTitleColor(void)
{
    ...
    if (gStorage->boxTitleCycleId == 0)
        CpuSet(sBoxTitleColors[wallpaperId], &gPlttBufferUnfaded[gStorage->boxTitlePalOffset], 0x00000000 | ((((2) * sizeof(u16)))/(16/8) & 0x1FFFFF));
    else
        CpuSet(sBoxTitleColors[wallpaperId], &gPlttBufferUnfaded[gStorage->boxTitleAltPalOffset], 0x00000000 | ((((2) * sizeof(u16)))/(16/8) & 0x1FFFFF));
}

static s16 GetBoxTitleBaseX(const u8 *string)
{
    return 240 - 64 - GetStringWidth(FONT_NORMAL_COPY_1, string, 0) / 2;
}
...
static void StartBoxScrollArrowsSlide(s8 direction)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        gStorage->arrowSprites[i]->data[0] = 2;
    }
    if (direction < 0)
    {
        gStorage->arrowSprites[0]->data[1] = 29;
        gStorage->arrowSprites[1]->data[1] = 5;
        gStorage->arrowSprites[0]->data[2] = 72;
        gStorage->arrowSprites[1]->data[2] = 72;
    }
    else
    {
        gStorage->arrowSprites[0]->data[1] = 5;
        gStorage->arrowSprites[1]->data[1] = 29;
        gStorage->arrowSprites[0]->data[2] = 240 + 8;
        gStorage->arrowSprites[1]->data[2] = 240 + 8;
    }
...
static void StopBoxScrollArrowsSlide(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        gStorage->arrowSprites[i]->x2 = 0;
        gStorage->arrowSprites[i]->invisible = 0;
    }
    AnimateBoxScrollArrows(1);
}
...
void AnimateBoxScrollArrows(bool8 animate)
{
    ...
    if (animate)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            gStorage->arrowSprites[i]->data[0] = 1;
            gStorage->arrowSprites[i]->data[1] = 0;
    ...
    else
    {
        ...
        for (i = 0; i < 2; i++)
            gStorage->arrowSprites[i]->data[0] = 0;
    }
}
