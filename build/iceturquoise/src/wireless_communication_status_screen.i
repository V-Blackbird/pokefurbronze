...
static const u8 sActivityGroupInfo[][3] = {
    {1, GROUPTYPE_BATTLE, 2},
    {2, GROUPTYPE_BATTLE, 2},
    {3, GROUPTYPE_BATTLE, 4},
    {4, GROUPTYPE_TRADE, 2},
    {21, GROUPTYPE_TOTAL, 2},
    {22, GROUPTYPE_TOTAL, 2},
    {9, NUM_GROUPTYPES, 0},
    {10, NUM_GROUPTYPES, 0},
    {11, NUM_GROUPTYPES, 0},
    {12, GROUPTYPE_NONE, 0},
    {13, GROUPTYPE_TRADE, 0},
    {14, GROUPTYPE_NONE, 0},
    {15, NUM_GROUPTYPES, 0},
    {16, GROUPTYPE_NONE, 0},
    {0 | (1 << 6), GROUPTYPE_UNION, 1},
    {1 | (1 << 6), GROUPTYPE_UNION, 2},
    {4 | (1 << 6), GROUPTYPE_UNION, 2},
    {5 | (1 << 6), GROUPTYPE_UNION, 0},
    {8 | (1 << 6), GROUPTYPE_UNION, 2},
    {20 | (1 << 6), GROUPTYPE_UNION, 1},
    {19 | (1 << 6), GROUPTYPE_UNION, 2},
    {17 | (1 << 6), GROUPTYPE_UNION, 1},
    {18 | (1 << 6), GROUPTYPE_UNION, 1}
};
...
static void CB2_InitWirelessCommunicationScreen(void)
{
    ...
    LoadPalette(sPalettes, (0x000 + ((0) * 16)), ((16) * sizeof(u16)));
...
static void ExitWirelessCommunicationStatusScreen(void)
{
    ...
    for (i = 0; i < (int)(sizeof(sBgTemplates) / sizeof((sBgTemplates)[0])); i++)
...
static void CyclePalette(s16 * counter, s16 * palIdx)
{
    ...
    if (++(*counter) > 5)
    {
        if (++(*palIdx) == (int)(sizeof(sPalettes) / sizeof((sPalettes)[0])) - 2)
            *palIdx = 0;
        *counter = 0;
    }
    idx = *palIdx + 2;
    LoadPalette(sPalettes[idx], (0x000 + ((0) * 16)), ((8) * sizeof(u16)));
}
...
static void WCSS_AddTextPrinterParameterized(u8 windowId, u8 fontId, const u8 * str, u8 x, u8 y, u8 mode)
{
    ...
    AddTextPrinterParameterized4(windowId, fontId, x, y, fontId == FONT_SMALL ? 0 : 1, 0, textColor, 0xFF, str);
}
...
static u32 CountPlayersInGroupAndGetActivity(struct RfuPlayer * player, u32 * groupCounts)
{
    ...
    for (i = 0; i < (sizeof(sActivityGroupInfo) / sizeof((sActivityGroupInfo)[0])); i++)
    {
