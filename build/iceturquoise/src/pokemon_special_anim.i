# 0 "src/pokemon_special_anim.c"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "src/pokemon_special_anim.c"
...
# 2 "src/pokemon_special_anim.c" 2
# 1 "include/gflib.h" 1
...
# 3 "src/pokemon_special_anim.c" 2
# 1 "include/data.h" 1
# 9 "include/data.h"
struct MonCoords
{
...
# 4 "src/pokemon_special_anim.c" 2
...
# 5 "src/pokemon_special_anim.c" 2
...
# 6 "src/pokemon_special_anim.c" 2
...
# 7 "src/pokemon_special_anim.c" 2
# 1 "include/task.h" 1
# 8 "src/pokemon_special_anim.c" 2
# 1 "include/constants/songs.h" 1
# 9 "src/pokemon_special_anim.c" 2
# 1 "include/constants/items.h" 1
# 10 "src/pokemon_special_anim.c" 2




static __attribute__((section("ewram_data"))) bool32 sCancelDisabled = 0;
static __attribute__((section("ewram_data"))) u8 sPSATaskId = 0;
static __attribute__((section("ewram_data"))) struct PokemonSpecialAnim * sPSAWork = ((void *)0);
...
static struct PokemonSpecialAnim * AllocPSA(u8 slotId, u16 itemId, MainCallback callback)
{
    ...
    ptr->species = GetMonData(pokemon, 11);
    ptr->closeness = GetClosenessFromFriendship(GetMonData(pokemon, 32));
    ptr->personality = GetMonData(pokemon, 0);
    ptr->slotId = slotId;
    ptr->itemId = itemId;
    ptr->animType = GetAnimTypeByItemId(itemId);
    ptr->pokemon = *pokemon;
    ptr->field_00a4 = 0;
    GetMonData(pokemon, 2, ptr->nickname);
    if (ptr->animType == 4)
    {
...
{
    struct PokemonSpecialAnim * ptr = (void *)GetWordTaskArg(taskId, 0);
    if (!ptr->cancelDisabled && ({(gMain.heldKeys) & (0x0001 | 0x0002);}))
    {
    ...
    case 5:
        ...
        break;
    case 6:
        PSA_SetUpItemUseOnMonAnim(ptr->itemId, ptr->closeness, 1);
        ptr->state++;
        break;
    case 7:
        if (!PSA_IsItemUseOnMonAnimActive())
        {
            ptr->cancelDisabled = 1;
            if (ptr->closeness == 3)
            {
    ...
    case 12:
        ...
        {
            if (CheckIfItemIsTMHMOrEvolutionStone(ptr->itemId) != 2)
            {
...
static void Task_EvoStone_CantEvolve(u8 taskId)
{
    ...
    if (!ptr->cancelDisabled && ({(gMain.heldKeys) & (0x0002);}))
    {
        SetUseItemAnimCallback(taskId, Task_CleanUp);
        return;
    }
    ...
    case 3:
        ...
        break;
    case 4:
        PSA_SetUpItemUseOnMonAnim(ptr->itemId, ptr->closeness, 0);
        ptr->state++;
        break;
    case 5:
        if (!PSA_IsItemUseOnMonAnimActive())
...
static void Task_MachineSet(u8 taskId)
{
    ...
    if (!ptr->cancelDisabled && ({(gMain.newKeys) & (0x0002);}))
    {
...
static u16 GetAnimTypeByItemId(u16 itemId)
{
    ...
    for (i = 0; i < (sizeof(sItemAnimMap) / sizeof((sItemAnimMap)[0])); i++)
    {
        if (sItemAnimMap[i].itemId == itemId)
            return sItemAnimMap[i].animType;
    }

    if (itemId >= 290 && itemId <= 347)
    {
        return 4;
    }
...
{
    data[0] = GetMonData(pokemon, 58);
    data[1] = GetMonData(pokemon, 59);
    data[2] = GetMonData(pokemon, 60);
    data[3] = GetMonData(pokemon, 61);
    data[4] = GetMonData(pokemon, 62);
    data[5] = GetMonData(pokemon, 63);
}
