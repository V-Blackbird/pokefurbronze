...
static bool8 CheckPartyMonHasHeldItem(u16 item)
{
    ...
    for(i = 0; i < 6; i++)
    {
        ...
        if (species != 0 && species != 479 && GetMonData(&gPlayerParty[i], 12) == item)
            return 1;
    }
    return 0;
}
...
{
    bool8 hasItem = CheckPartyMonHasHeldItem(176);
    if (hasItem == 1)
        GetBerryNameByBerryType(ItemIdToBerryType(176), gStringVar1);
...
void CreateScriptedWildMon(u16 species, u8 level, u16 item)
{
    ...
    ZeroEnemyPartyMons();
    CreateMon(&gEnemyParty[0], species, level, 32, 0, 0, 0, 0);
    if (item)
    {
        heldItem[0] = item;
        heldItem[1] = item >> 8;
