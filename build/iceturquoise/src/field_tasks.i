...
static const TaskFunc sPerStepCallbacks[] =
{
    ...
    [2] = DummyPerStepCallback,
    [3] = DummyPerStepCallback,
    [4] = IcefallCaveIcePerStepCallback,
    [5] = DummyPerStepCallback,
    [6] = DummyPerStepCallback,
...
void ActivatePerStepCallback(u8 callbackId)
{
    ...
    if (taskId != 0xff)
    {
        ...
        if (callbackId >= (sizeof(sPerStepCallbacks) / sizeof((sPerStepCallbacks)[0])))
        {
            data[0] = 0;
        }
        else
...
{
    u8 i = 0;
    for (; i < (sizeof(sIcefallCaveIceTileCoords) / sizeof((sIcefallCaveIceTileCoords)[0])); ++i)
    {
...
{
    FlagSet(((((0x4FF + 1) + 768 - 1) + 1) + 0x29));
    FlagSet(((((0x4FF + 1) + 768 - 1) + 1) + 0x28));
}
