...
static void Cmd_critcalc(void)
{
    ...
    if ((gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     ...
     && !(gBattleTypeFlags & BATTLE_TYPE_STARTER)
     && !(gBattleTypeFlags & BATTLE_TYPE_POKEDUDE))
        gCritMultiplier = 2;
    else
        gCritMultiplier = 1;
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        else // mon may be caught, calculate shakes
        {
            ...
            {
                if (gBattleTypeFlags & BATTLE_TYPE_STARTER) // first try catch success
                {
                    gBattlescriptCurrInstr = BattleScript_OldMan_Pokedude_CaughtMessage; 
                } else {
                    gBattlescriptCurrInstr = BattleScript_SuccessBallThrow;
            ...
            else if (VarGet(VAR_TEMP_0) == 1) // second try catch always success
            {
                BtlController_EmitBallThrowAnim(BUFFER_A, BALL_3_SHAKES_SUCCESS);
                MarkBattlerForControllerExec(gActiveBattler);
                gBattlescriptCurrInstr = BattleScript_OldMan_Pokedude_CaughtMessage;
            ...
            {
                if (gBattleTypeFlags & BATTLE_TYPE_STARTER) // first try catch fail
                {
                    VarSet(VAR_TEMP_0, 1); 
