            ...
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF0000000) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 0xC))))
            {
                 ...
                 && gBattleBufferB[gActiveBattler][1] == B_ACTION_RUN)
                {
                    ...
                    gBattleCommunication[gActiveBattler] = STATE_BEFORE_ACTION_CHOSEN;
                      ...
                      && gBattleBufferB[gActiveBattler][1] == B_ACTION_STARTER_USE_MOVE) 
                {
                    BattleScriptExecute(BattleScript_StarterPlayerUsedTackle);
                    gBattleCommunication[gActiveBattler] = STATE_BEFORE_ACTION_CHOSEN;
                }
                else if (gBattleTypeFlags & BATTLE_TYPE_STARTER
                      && gBattleBufferB[gActiveBattler][1] == B_ACTION_STARTER_SWITCH)
                {
                    BattleScriptExecute(BattleScript_StarterNoMon);
                    gBattleCommunication[gActiveBattler] = STATE_BEFORE_ACTION_CHOSEN;
                }
                else if (gBattleTypeFlags & BATTLE_TYPE_STARTER
                      && gBattleBufferB[gActiveBattler][1] == B_ACTION_STARTER_RUN)
                {
