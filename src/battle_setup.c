...
{
    CreateMaleMon(&gEnemyParty[0], SPECIES_VILL, 48);
    LockPlayerFieldControls();
...
void StartStarterBattle(void)
{
    ...
    // Choose starter species based on PLAYER_STARTER_NUM
    switch (VarGet(VAR_STARTER_MON)) // Or however you access the variable
    {
    case SPECIES_MARIE:
        starterSpecies = SPECIES_MARIE;
        break;
    case SPECIES_GILAN:
        starterSpecies = SPECIES_GILAN;
        break;
    case SPECIES_EVEREST:
        starterSpecies = SPECIES_EVEREST;
        break;
    default:
        starterSpecies = SPECIES_MARIE; // Fallback
        break;
    }

    // Create the enemy Pokémon
    CreateMaleMon(&gEnemyParty[0], starterSpecies, 5); // Adjust level/nature as needed
    gMain.savedCallback = CB2_ReturnToFieldContinueScriptPlayMapMusic; // Or your own callback if needed
    gBattleTypeFlags = BATTLE_TYPE_STARTER; // Your custom battle type
    CreateBattleStartTask(GetWildBattleTransition(), 0);
    IncrementGameStat(GAME_STAT_TOTAL_BATTLES);
    IncrementGameStat(GAME_STAT_WILD_BATTLES);
}
